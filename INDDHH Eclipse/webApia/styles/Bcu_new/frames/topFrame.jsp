...
<form name="frmLogin" id="frmLogin" method="post">
	<table width="100%" height="100%" cellpadding=0 cellspacing=0>
		...
		<tr>
			<td class="degree1" valign="top"><img src="<%=Parameters.ROOT_PATH%>/styles/<%= styleDirectory %>/images/header.jpg"></td>
			<td valign="top" >
			<div>
			...
			<td align=right vAlign="center" class="degree2">
				...
				<!--  agregado 
				<!-- <%if (TestUrlImg.executeTest(bLogin.getUserId(request),environmentId,Integer.valueOf(currentLanguage)) ){%>-->
				<span class="spanName"	title = "Datos Usuario"> 
				| <img src="<%=new Helper().getFotoUsr(bLogin.getUserId(request),environmentId,Integer.valueOf(currentLanguage))%>" border=1 width="50" height="30" onClick="mostrarDatosUsuario('<%= bLogin.fmtHTML(bLogin.getUserId(request))%>',<%=environmentId %>,<%=currentLanguage%>)">
				</span>		
				<!-- <%} %>-->
				<!--  finagregado -->	
...
function mostrarDatosUsuario(usr,envId,currentLang){
	var URL = "<%=Parameters.ROOT_PATH%>" + "/expedientes/jsp/datosUsuario.jsp?usuario="+usr+"&langId="+currentLang+"&environmentId="+envId;
	window.status = URL;		
	var modal = xShowModalDialog( URL,"","dialogWidth:850px; dialogHeight:500px;status=no" );
