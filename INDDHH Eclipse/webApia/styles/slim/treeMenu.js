	...
	function sizeToc(){
		...
		for(var i=0;i<elements.length;i++){
			if(elements[i].parentNode.style.display!="none"){
				if(elements[i].getElementsByTagName("SPAN")[0]){
					var thisWidth=elements[i].getElementsByTagName("SPAN")[0].offsetWidth+30;
					if(width<thisWidth){
						width=thisWidth;
						max=elements[i];
					}
		...
		document.getElementById("tocHolder").style.width=(width)+"px";
		document.getElementById("tocContainer").style.width=(width)+"px";
		adjustBack(width);
		document.getElementById("tocContainer").style.height=(window.parent.document.getElementById("tocArea").clientHeight)+"px";
		if(navigator.userAgent.indexOf("MSIE")<0){
			document.getElementById("tocContainer").style.overflow="hidden";
