	function doLIAction(aEvent){		
		...
		var toAddClass;
		if(MSIE)
			toAddClass = window.event.srcElement.parentNode;
		else
			toAddClass = element.parentNode;
		
		if(toAddClass.tagName == "TD")
			toAddClass = toAddClass.parentNode;
		
		if(toAddClass.tagName == "TR") {		
			if(toAddClass.className.indexOf("menuSelected") >= 0)
				toAddClass.className = 'tdInnerTitleAcord';
			else
				toAddClass.className += ' menuSelected';
		}
		...
		for(var i=0;i<elements.length;i++){
			if(elements[i].parentNode.style.display!="none"){
				if(elements[i].getElementsByTagName("SPAN")[0]){
					var thisWidth=elements[i].getElementsByTagName("SPAN")[0].offsetWidth+40;
					if(width<thisWidth){
						width=thisWidth;
						max=elements[i];
					}
