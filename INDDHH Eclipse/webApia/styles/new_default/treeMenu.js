	...
	function closeAll(){
		var tds=document.getElementById("tocHolder").getElementsByTagName("TABLE")[0].getElementsByTagName("TD");
		for(var i=0;i<tds.length;i++){
			if(tds[i].className=="tdBodyAcord"){
				tds[i].style.display="none";
			}
		}
		document.getElementById("tocHolder").style.display="none";
		document.getElementById("tocHolder").style.display="block";
	}
	...
	function sizeToc(){
		//var width=document.getElementById("tocHolder").getElementsByTagName("UL")[0].offsetWidth;
		var elements=document.getElementById("tocHolder").getElementsByTagName("LI");
		var width=10;
		var max;
		for(var i=0;i<elements.length;i++){
			if(elements[i].parentNode.style.display!="none"){
				//var thisWidth=elements[i].childNodes[1].offsetWidth+60;
				var thisWidth=elements[i].childNodes[0].offsetWidth+60;
				if(width<thisWidth){
					width=thisWidth;
					max=elements[i];
				}
		...
		document.getElementById("tocContainer").style.width=(width-4)+"px";
		adjustBack(width);
	...
	function setTocWidth(width){
		...
		adjustBack(width);
		document.getElementById("tocContainer").style.height=(window.parent.document.getElementById("tocArea").clientHeight-5)+"px";
		document.getElementById("tocContainerBack").style.height=(window.parent.document.getElementById("tocArea").clientHeight-5)+"px";
		if(navigator.userAgent.indexOf("MSIE")<0){
			document.getElementById("tocContainer").style.overflow="hidden";
			if(document.getElementById("tocContainer").offsetHeight<document.getElementById("tocHolder").clientHeight){
				document.getElementById("tocContainer").style.overflow="-moz-scrollbars-vertical";
