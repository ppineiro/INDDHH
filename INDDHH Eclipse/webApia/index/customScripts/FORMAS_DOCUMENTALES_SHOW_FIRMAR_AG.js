...
function fnc_1001_1659(evtSource, par_nombFormulario, par_fldAccion, par_fldResultado, par_btnFirma) { 
	...
	if(strAccion != ""){
		if (strAccion.search(/^firmar_.+/i) == 0){
			...
			if(name == 'chrome' && version >= 42)
				modal =  ModalController.openWinModal(getUrlApp() + "/expedientes/firma/firmarchromeSignature.jsp?" + TAB_ID_REQUEST, 460, 280, null, null, true, true, true);
			else
				modal =  ModalController.openWinModal(getUrlApp() + "/expedientes/firma/firmar.jsp?" + TAB_ID_REQUEST, 460, 280, null, null, true, true, true);
