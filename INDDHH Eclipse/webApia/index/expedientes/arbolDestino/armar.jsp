...
function getModalReturnValue(modal) {
	...
	destino = document.getElementById("textOficinaDestino").value 
		...
		+ sep1 + document.getElementById("textNombUsuarioDestino").value;
...
<BODY onload="whenLoad()">
	<form name="frmMain" id="frmMain" method="post">
		...
		<input id=textNombUsuarioDestino name=textNombUsuarioDestino type="hidden">
		...
		<input readonly id="waittoload" value="Espere un momento.." style="background-color: yellow; 
				color: navy; position: absolute; top: 120px; left: 130px; display: none">

		<table border="0" width="100%">
			<tr>
				<td colspan=2>
				<font style="FONT-FAMILY: verdana; FONT-SIZE: 8pt;">
				<legend><%=mensajeDao.obtenerMensajePorCodigo(
					"VAL_LBL_BUSCAR_OFICINA_JSP_TXT", currentLanguage,
					environmentId)%></legend>
						<input type=text id='busqueda'
						onkeyup='return busquedaOnKeyPress();'>
					<button onclick='return buscarNodo("sig");'><%=mensajeDao.obtenerMensajePorCodigo(
					"VAL_BTN_SIGUIENTE_BUSQUEDA_JSP_TXT", currentLanguage,
					environmentId)%></button>
						<button onclick='return buscarNodo("ant");'><%=mensajeDao.obtenerMensajePorCodigo(
					"VAL_LBL_ANTERIOR_BUSQUEDA_JSP_TXT", currentLanguage,
					environmentId)%></button></font>
				</td>
			</tr>
			<tr>
				<td>
					<div id="tree" onclick="treeClick(event)">
						<fieldset id="treefield" name="treefield"
							style="HEIGHT: 287px; WIDTH: 379px">
							<legend>
								<font style="FONT-FAMILY: verdana; FONT-SIZE: 8pt;"><%=mensajeDao.obtenerMensajePorCodigo(
					"VAL_LBL_SELECCIONAR_OFICINA_JSP_TXT", currentLanguage,
					environmentId)%>
								</font>
							</legend>
							<BLOCKQUOTE CLASS="body">
								<DIV id="oTocDiv"
									style="BACKGROUND-COLOR: <%=Mensajes.com_st_apia_expedientes_style_arbol_destino%>; BORDER-BOTTOM: #505050 1px solid; BORDER-LEFT: #505050 1px solid; BORDER-RIGHT: #505050 1px solid; BORDER-TOP: #505050 1px solid; FONT-FAMILY: verdana; FONT-SIZE: 8pt; HEIGHT: 238px; OVERFLOW: auto; WIDTH: 341px">
									<UL ID="oPrimaryTOC">
										<%
											try {
												if (uData != null) {
				...
				<td>
					...
					<fieldset id="treefieldusr" name="treefieldusr"
						...
						<legend>
							<font style="FONT-FAMILY: verdana; FONT-SIZE: 8pt;"><%=mensajeDao.obtenerMensajePorCodigo(
					"VAL_LBL_USUARIO_DESTINO_JSP_TXT", currentLanguage,
					environmentId)%></font>
						</legend>
						<BLOCKQUOTE CLASS="body">
							<DIV id="oTocDivUsr"
				...
				<td>
					...
					<fieldset id="usrImg" name="usrImg"
						style="HEIGHT: 287px; WIDTH: 219px">
						<legend>
							<font style="FONT-FAMILY: verdana; FONT-SIZE: 8pt;"><%=mensajeDao.obtenerMensajePorCodigo(
					"VAL_LBL_FOTO_JSP_TXT", currentLanguage, environmentId)%></font>
						</legend>
						<BLOCKQUOTE CLASS="body">
							<img onload="resizeImg(this, 150, 150)" id="imgFoto"
								name="imgFoto"
								src="<%=Parameters.ROOT_PATH%>/images/fotos/NoPicture.gif">
						</BLOCKQUOTE>
					</fieldset>
