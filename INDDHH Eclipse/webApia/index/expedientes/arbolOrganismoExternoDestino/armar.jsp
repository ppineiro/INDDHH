...
<%
	...
	boolean arteeDeshabilitado = ConfigurationManager
			.estaDeshabilitadoFIEEARTEE(null, environmentId,
					currentLanguage, false);
	boolean electronicSendingIsOff = ConfigurationManager
			.apagarEnvioElectronicoAOrganismoExterno(environmentId,
					currentLanguage, false);
	...
	String nroDocumento = (String) uData.getUserAttributes().get(
			"EXP_NRO_EXPEDIENTE_STR");
	Boolean isReceivedManually = Helper.checkIsReceivedManually(
			nroDocumento, environmentId);
%>
...
<title><%=mensajeDao.obtenerMensajePorCodigo(
					"VAL_TITULO_SELECCIONAR_DESTINO_PASE_JSP_TXT",
					currentLanguage, environmentId)%></title>
...
function getModalReturnValue(modal) {
	...
	if (destino == "" ){
		alert("<%=mensajeDao.obtenerMensajePorCodigo("MSG_ELEGIR_UN_DESTINO_JSP",
							currentLanguage, environmentId)%>"); //alert("Debe elegir un destino");
		...
		alert("<%=mensajeDao.obtenerMensajePorCodigo(
					"MSG_ELEGIR_MESA_ENTRADA_JSP", currentLanguage,
					environmentId)%>"); //alert("Debe elegir una mesa de entrada.");
	}else{
		if (document.getElementById("modoEnvio").value == '<%=Values.VAL_FORMA_ENVIO_ARTEE_OE_STR%>' && document.getElementById("textHayMEenOE").value == "0"){
			destino = "";
			alert("<%=mensajeDao.obtenerMensajePorCodigo(
						"MSG_ORGANISMO_SIN_MESA_ENTRADA_JSP", currentLanguage,
						environmentId)%>"); //alert("No hay mesa de entrada para ese organismo!");
		}else{
...
function cargarUsuarios(flag){
	...
	var URL = "<%=ConfigurationManager.getServerAddress(environmentId,
					currentLanguage) + Configuration.ROOT_PATH%>/expedientes/arbolDestino/cargarUsuarios.jsp?unidad=" + document.getElementById("textIdOficinaDestino").value + "&flag=" + flag;					
	//alert(URL);
...
function cargarMesasEntrada(flag){
	...
	// Enviamos la peticion	
	var URL = "<%=ConfigurationManager.getServerAddress(environmentId,
					currentLanguage) + Configuration.ROOT_PATH%>/expedientes/arbolOrganismoExternoDestino/cargarMesasEntrada.jsp?orgext=" + document.getElementById("organismoDestino").value + "&flag=" + flag;					
	//alert(URL);
					...
					currentLanguage, environmentId)%>
							...
							<BLOCKQUOTE CLASS="body">
								...
								<BUTTON type="BUTTON" onclick="displayARTEE()" ID=btnARTEE
									NAME=btnARTEE disabled="true"><%=mensajeDao
						.obtenerMensajePorCodigo("VAL_BTN_ARTEE_JSP_TXT",
								currentLanguage, environmentId)%></BUTTON>
								<BUTTON type="BUTTON" onclick="displayEntOE();" id=btnEntOE
									name=btnEntOE><%=mensajeDao.obtenerMensajePorCodigo(
						"VAL_BTN_ENT_OE_JSP_TXT", currentLanguage,
				...
				<td>
					...
					<div id="oTocDivMesaEntradaG" style="width: 100%; HEIGHT: 100%">
						<fieldset id="treefieldusr" name="treefieldusr">
							...
							<legend>
								<font style="FONT-FAMILY: verdana; FONT-SIZE: 8pt;"><%=mensajeDao.obtenerMensajePorCodigo(
						"VAL_LBL_MESA_ENTRADA_DESTINO_JSP_TXT",
						currentLanguage, environmentId)%>
