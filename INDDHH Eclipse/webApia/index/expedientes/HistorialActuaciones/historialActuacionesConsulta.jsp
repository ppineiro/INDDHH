...
<body class="listBody">
		...
		<COL class="col1"><COL class="col2">
			<TR>
				<TD><%= mensajeDao.obtenerMensajePorCodigo("MSG_LBL_TITULO_BUSQUEDA_EXPEDIENTE_HIST_JSP", langId, environmentId)%><!--  Búsqueda de Expedientes--></TD>
				<TD></TD>
			</TR>
		</TABLE>
...
<table class="frmSubTit">
	<tr>
		<td><%= mensajeDao.obtenerMensajePorCodigo("MSG_LBL_TITULO_INFORMACION_EXPEDIENTE_HIST_JSP", langId, environmentId)%><!--  Información del Expediente--></td>
	</tr>
</table>
<TABLE width="100%">
	<TR>
		<TD align="RIGHT" width=15%>
		<%= mensajeDao.obtenerMensajePorCodigo("MSG_LBL_NRO_EXPEDIENTE_HIST_JSP", langId, environmentId)%><!--  Número de expediente: -->
		</TD>
		<TD align="left" width=75%>
		<input type="text" id="nroExp"
			name="nroExp" value=<%=nroExp%>>
		<button type="button" value="Buscar expediente" onclick="buscar()">
		<%= mensajeDao.obtenerMensajePorCodigo("MSG_VAL_BTN_BUSCAR_HIST_JSP", langId, environmentId)%><!--Buscar expediente--></button>
	...
	<TR>
		<TD align="RIGHT">
		<%= mensajeDao.obtenerMensajePorCodigo("MSG_LBL_ASUNTO_EXPEDIENTE_HIST_JSP", langId, environmentId)%><!--Asunto: -->
		</TD>
	...
	<TR>
		<TD align="RIGHT">
		<%= mensajeDao.obtenerMensajePorCodigo("MSG_LBL_OFICINA_ACTUAL_HIST_JSP", langId, environmentId)%><!--Oficina Actual: -->
		</TD>
		<td>
	...
	<TR>
		<TD align="RIGHT">
		<%= mensajeDao.obtenerMensajePorCodigo("MSG_LBL_FECHA_ULTIMO_HIST_JSP", langId, environmentId)%><!--Fecha de último pase: -->
		</TD>
		<td>
		<input type="text"
	...
	<TR>
		...
		<table rules="all" cellpadding="0"
			...
			<thead id="tHeadList">
				<tr>
					<th style='width: 250px' title='Tipo de titular'><%= mensajeDao.obtenerMensajePorCodigo("MSG_LBL_TIPO_TITULAR_HIST_JSP", langId, environmentId)%><!--Tipo de Titular--></th>
					<th style='width: 250px' title='Titular'><%= mensajeDao.obtenerMensajePorCodigo("MSG_LBL_TITULAR_HIST_JSP", langId, environmentId)%><!--Titular--></th>
					<th style='width: 250px' title='Nombre del titular'><%= mensajeDao.obtenerMensajePorCodigo("MSG_LBL_NOMBRE_TITULAR_HIST_JSP", langId, environmentId)%><!--Nombre del titular--></th>
					<th style='width: 250px' title='Documento del titular'> <%= mensajeDao.obtenerMensajePorCodigo("MSG_LBL_DOCUMENTO_TITULAR_HIST_JSP", langId, environmentId)%><!--Documento del titular--></th>
				</tr>
			</thead>
	...
	<table class="frmSubTit">
		<tr>
			<td><%= mensajeDao.obtenerMensajePorCodigo("MSG_LBL_TITULO_HSTORIAL_EXPEDIENTE_HIST_JSP", langId, environmentId)%><!--Historial del Expediente--></td>
		</tr>
	</table>
	<BR>
	<TR>
		...
		<table cellpadding="0" cellspacing="0" hasFiles='false'>
			<thead>
				<tr>
					<th style='width: 350px' title='Oficina Actuante'><%= mensajeDao.obtenerMensajePorCodigo("MSG_LBL_OFICINA_ACTUANTE_HIST_JSP", langId, environmentId)%><!--Oficina Actuante--></th>
					<th style='width: 250px' title='Fecha Actuación'><%= mensajeDao.obtenerMensajePorCodigo("MSG_LBL_FECHA_ACTUACION_HIST_JSP", langId, environmentId)%><!--Fecha	Actuación--></th>
				</tr>
			</thead>
