...
function cargarUsuarios(flag){
	...
	var URL = "<%=ConfigurationManager.getServerAddress(environmentId,currentLanguage) + Configuration.ROOT_PATH%>/expedientes/arbolDestinoMultipleConUsuarios/cargarUsuarios.jsp?unidad=" + document.getElementById("textIdOficinaDestino").value + "&flag=" + flag;					
	//alert(URL);
...
<tr>
<td colspan =2><font style="FONT-FAMILY: verdana; FONT-SIZE: 8pt;"><legend><%=mensajeDao.obtenerMensajePorCodigo("VAL_LBL_BUSCAR_OFICINA_JSP_TXT", currentLanguage, environmentId)%> </legend> <input type=text id='busqueda' onkeyup='return busquedaOnKeyPress();'><button onclick='return buscarNodo("sig");'><%=mensajeDao.obtenerMensajePorCodigo("VAL_BTN_SIGUIENTE_BUSQUEDA_JSP_TXT", currentLanguage, environmentId)%></button><button onclick='return buscarNodo("ant");'><%=mensajeDao.obtenerMensajePorCodigo("VAL_LBL_ANTERIOR_BUSQUEDA_JSP_TXT", currentLanguage, environmentId)%></button></font>
</td>
</tr>
<tr>
	<td>
