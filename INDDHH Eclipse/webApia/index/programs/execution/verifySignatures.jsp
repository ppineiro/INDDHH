...
function checkSign(signId){
	...
	http_request.open('POST', "execution.FormAction.do?action=verifySignature", false);
	http_request.setRequestHeader("content-type","application/x-www-form-urlencoded; charset=utf-8");
    http_request.send("signId=" + signId + "&frmId=<%=request.getParameter("frmId")%>&frmParent=<%=request.getParameter("frmParent")%>" );
...
function btnExit_click() {
	...
	function downloadPKCS7(signId){
		document.getElementById("ifrDownload").src="execution.FormAction.do?action=downloadPKCS7&signId=" + signId + "&frmId=<%=fBean.getFormId()%>";
	}
