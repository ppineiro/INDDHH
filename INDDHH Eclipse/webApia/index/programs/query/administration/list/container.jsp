<%@page import="com.dogma.*"%><%@page import="com.dogma.vo.*"%><%@page import="java.util.*"%><!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd"><%@include file="../../../../components/scripts/server/startInc.jsp" %><jsp:useBean id="qBeanReady" scope="session" class="com.dogma.bean.query.TaskListBean"></jsp:useBean><jsp:useBean id="qBeanInproc" scope="session" class="com.dogma.bean.query.TaskListBean"></jsp:useBean><%
com.dogma.bean.query.TaskListBean dBean = qBeanInproc;
%><HTML><head><%@include file="../../../../components/scripts/server/headInclude.jsp" %><SCRIPT DEFER=true>
function tabSwitch(){
	...
	<%if( "R".equals(request.getParameter("workMode")) ){%> 
...
function init(){
	document.getElementById("frameContent0").src="<%=Parameters.ROOT_PATH%>/programs/query/administration/list/client.jsp?listType=<%=com.dogma.bean.query.TaskListBean.WORKING_MODE_INPROCESS%>"+windowId;
	document.getElementById("frameContent1").src="<%=Parameters.ROOT_PATH%>/programs/query/administration/list/client.jsp?listType=<%=com.dogma.bean.query.TaskListBean.WORKING_MODE_READY%>"+windowId;
	<%
