...
%><HTML><head><%@include file="../../../../../components/scripts/server/headInclude.jsp" %></head><body onload="checkFilter()"><TABLE class="pageTop"><COL class="col1"><COL class="col2"><TR><TD><%=LabelManager.getName(labelSet,"titQryMonEnt")%>: <%=queryVo.getQryTitle()%></TD><TD></TD></TR></TABLE><DIV id="divContent" class="divContent"><form id="frmMain" name="frmMain" method="POST"><% if (queryVo.getWhereUserColumns() != null && queryVo.getWhereUserColumns().size() > 0) { %><DIV class="subTit"><table width="100%"><tr class="subTit"><td width="100%" align="left"><%=LabelManager.getName(labelSet,"sbtFil")%></td><td align="right"><button type="button" id="toggleFilter" title="<%=LabelManager.getToolTip(labelSet,"lblMonButFil")%>" class="btn" onclick="toggleFilterSection(<%=Parameters.SCREEN_LIST_SIZE - Parameters.FILTER_LIST_SIZE%>,<%=(Parameters.SCREEN_LIST_SIZE)%>)"><img src="<%=Parameters.ROOT_PATH%>/styles/<%= styleDirectory %>/images/openToc.gif" width="8" height="7"></button></td></tr></table></DIV><DIV id="listFilterArea"  style="display:none"><DIV style="OVERFLOW:AUTO;HEIGHT:<%= Parameters.FILTER_LIST_SIZE - 32 %>px;"><table class="tblFormLayout"><COL class="col1"><COL class="col2"><COL class="col3"><COL class="col4"><%
						...
						if (! addedEndLine) {
							...
							if (qBean.queryHasEmptyRequieredFilters()) { %><tr><td colspan="<%= colsToShow %>"><br><br><%=LabelManager.getName(labelSet,"lblQryMustFil")%><br><br><br></td></tr><% 
	  							...
	  							if (iteratorFilas != null) {
									...
									boolean odd = false;
		 			  				while (iteratorFilas.hasNext()) {
	 		 	  						...
	 		 	  						rowString = row.getBusEntityMonitorQueryString(posBusEntInstId); 
	 		 	  						odd = (count % 2 == 0); %><tr row_id="<%=rowString%>" <%=odd?"class=\"trOdd\"":""%> id=LIST><%
		 									columnas = row.getColumnas().iterator();
	 										pos = 0;
	 										while (columnas.hasNext()) { 
	 											value = columnas.next();
	 	 					...
	 	 					} %></tbody></table><IFRAME name="idResult" id="idResult" height="0" width="0" frameborder="0"></IFRAME></div><table class="navBar"><COL class="col1"><COL class="col2"><tr><% if (! queryVo.isFirstTime()) { %><%@include file="../../../../includes/navButtonsQuery.jsp" %><% } else { %><td></td><% } %><td><button type="button" onclick="btnExport_click()" <%= qBean.queryHasEmptyRequieredFilters() ? "disabled":"" %> accesskey="<%=LabelManager.getAccessKey(labelSet,"btnExport")%>" title="<%=LabelManager.getToolTip(labelSet,"btnExport")%>"><%=LabelManager.getNameWAccess(labelSet,"btnExport")%></button><button type="button" onclick="btnDet_click()" <%= qBean.queryHasEmptyRequieredFilters() ? "disabled":"" %> accesskey="<%=LabelManager.getAccessKey(labelSet,"btnMonHis")%>" title="<%=LabelManager.getToolTip(labelSet,"btnMonHis")%>"><%=LabelManager.getNameWAccess(labelSet,"btnMonHis")%></button></td></tr></table></form></div><TABLE class="pageBottom"><COL class="col1"><COL class="col2"><TR><TD></TD><TD><button type="button" onclick="splash()" accesskey="<%=LabelManager.getAccessKey(labelSet,"btnSal")%>" title="<%=LabelManager.getToolTip(labelSet,"btnSal")%>"><%=LabelManager.getNameWAccess(labelSet,"btnSal")%></button></TD></TR></TABLE></body></html><script src="<%=Parameters.ROOT_PATH%>/programs/query/administration/monitor/entity/list.js"></script><script language="javascript">
