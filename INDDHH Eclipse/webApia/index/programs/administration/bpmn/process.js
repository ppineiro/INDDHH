function lnkDownDeps_click(){
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=downDepsTxt";
...
function btnNew_click(){
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=new";
	...
	if(cant != 0) {
		if(cant == 1 && canWrite()==false) {
			...
			if (confirm(GNR_DELETE_RECORD)) {
				document.getElementById("frmMain").action = "administration.BPMNAction.do?action=remove";
				submitForm(document.getElementById("frmMain"));
			}
		}
	} else {
	...
	if(cant != 0) {
		if(cant == 1 && canWrite()==false) {
			...
			if (confirm(GNR_INIT_RECORD)) {
				var usedInScenario = verifieIfUsedInScenario();
				if (usedInScenario=="true"){
					if (cant==1){
						if (confirm(MSG_PRO_WILL_BE_DEL_FROM_ALL_SCE1)){
							submit = true;
						}
					}else{
						if (confirm(MSG_PRO_WILL_BE_DEL_FROM_ALL_SCE2)){
							submit = true;
						}
					}
				}else{
			...
			if (submit){
				document.getElementById("frmMain").action = "administration.BPMNAction.do?action=initPro";
				submitForm(document.getElementById("frmMain"));
			}
		}
	} else {
...
function verifieIfUsedInScenario(){
	var	http_request = getXMLHttpRequest();
	http_request.open('POST', "administration.BPMNAction.do?action=verifieSceUsed"+windowId, false);
	http_request.setRequestHeader("content-type","application/x-www-form-urlencoded");
	
	var selectedProcess = "";
	var rows=document.getElementById("gridList").selectedItems;
	for(var i=0;i<rows.length;i++){
		var proId = rows[i].getElementsByTagName("TD")[1].getElementsByTagName("INPUT")[1].value;
		if (selectedProcess == ""){
			selectedProcess = "proId=" + proId;
		}else{
			selectedProcess = selectedProcess  + "&proId=" + proId;
		}
	}
	http_request.send(selectedProcess);
		...
		if (canRead()==true){ 
			if (canWrite()==true){ //verificamos si el usuario tiene permisos de escritura
				if (canUpdate()){ //verificamos si bloqueo el proceso
					...
					var isBPMNVal = trows[selItem].getElementsByTagName("TD")[1].getElementsByTagName("INPUT")[4].value;
					IS_BPMN = isBPMNVal;
					//----------------------------------------------------------------------------
					
					document.getElementById("frmMain").action = "administration.BPMNAction.do?action=update&bpmn="+isBPMNVal;
					submitForm(document.getElementById("frmMain"));	
				}
				...
				if (resp){
					document.getElementById("frmMain").action = "administration.BPMNAction.do?action=update";
					submitForm(document.getElementById("frmMain"));	
				}
			}
		}else{
...
function btnDeb_click(){
	var cant = chksChecked(document.getElementById("gridList"));
	if(cant == 1) {
		if (canRead()==true){
			document.getElementById("frmMain").action = "administration.BPMNAction.do?action=debuggerInit";
			submitForm(document.getElementById("frmMain"));	
		}else{
			alert(MSG_PRO_CANT_READ);
		}
...
function btnVers_click(){
	...
	if(cant == 1) {
		if (canRead()==true){ 
			document.getElementById("frmMain").action = "administration.BPMNAction.do?action=versions";
			submitForm(document.getElementById("frmMain"));	
		}else{
			alert(MSG_INSUF_PERMISSIONS);
		}
...
function btnRegenCond_click(){
	...
	if(cant != 0) {
		if(cant == 1 && canWrite()==false) {
			...
			document.getElementById("frmMain").action = "administration.BPMNAction.do?action=regenCond";
			submitForm(document.getElementById("frmMain"));
		}
	} else {
		alert(GNR_CHK_AT_LEAST_ONE);
...
function btnClo_click(){
	...
	if(cant == 1) {
		if (canRead()==true){ 
			...
			document.getElementById("frmMain").action = "administration.BPMNAction.do?action=clone&toClone="+procId;
			submitForm(document.getElementById("frmMain"));	
		}else{
			alert(MSG_INSUF_PERMISSIONS);
		}
...
function btnConfClo_click(){
	if (verifyRequiredObjects()) {
		if(isValidName(document.getElementById("txtName").value)){
			document.getElementById("frmMain").action = "administration.BPMNAction.do?action=confClone";
			submitForm(document.getElementById("frmMain"));
		}
	}
}
...
function btnDep_click(){
	var cant = chksChecked(document.getElementById("gridList"));
	if(cant == 1) {
		document.getElementById("frmMain").action = "administration.BPMNAction.do?action=viewDeps";
		submitForm(document.getElementById("frmMain"));	
	} else if (cant > 1) {
		alert(GNR_CHK_ONLY_ONE);
	} else {
...
function btnSearch_click() {
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=search";
	submitForm(document.getElementById("frmMain"));
}

function orderBy(order){
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=order&orderBy=" + order;
	submitForm(document.getElementById("frmMain"));
}

function first() {
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=first";
	submitForm(document.getElementById("frmMain"));
}

function prev() {
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=prev";
	submitForm(document.getElementById("frmMain"));
}

function next() {
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=next";
	submitForm(document.getElementById("frmMain"));
}

function last() { 
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=last";
	submitForm(document.getElementById("frmMain"));
}

function goToPage(){
	document.getElementById("frmMain").action="administration.BPMNAction.do?action=page";
	submitForm(document.getElementById("frmMain"));
}
	...
	if (verifyRequiredObjects()){
		if (canModifyProcess()=='true'){
			if (verifyCubeData() && verifyPermissions()){ //&& verifyActualUserCanModify()
				var result = askAjax("administration.BPMNAction.do","action=checkWarning&txtMap="+escape(document.getElementById("txtMap").value) + "&windowId=" + windowId,
					function(result){
					if (result == "true"){
...
function doConfirm(){
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=confirm&firstConf=true&bpmn="+IS_BPMN;
	submitFormFrame(document.getElementById("frmMain"));
}

function doSave(){
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=save";
	submitFormFrame(document.getElementById("frmMain"));
}


function removeProEleIds(){
	var model=document.getElementById("txtMap").value;
	while(model.indexOf("apia:ProEleId=")>0){
		var startIndex=model.indexOf("apia:ProEleId=");
		var id="apia:ProEleId=\"";
		var index=startIndex+15;
		var char=model.charAt(index);
		while(char!='"'){
			index++;
			id=id+char;
			char=model.charAt(index);
		}
		id=id+"\"";
		model=model.split(id).join("");
	}
	document.getElementById("txtMap").value=model;
}

function btnVal_click(){
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=validate";
	submitFormFrame(document.getElementById("frmMain"));
}
...
function btnRecover() {
	...
	if(cant == 1) {
		if (confirm(RECOVER_RECORD)) {
			document.getElementById("frmMain").action = "administration.BPMNAction.do?action=recover";
			submitForm(document.getElementById("frmMain"));	
		}
	} else if (cant > 1) {
		alert(GNR_CHK_ONLY_ONE);
...
function btnVerVersion() {
	var cant = chksChecked(document.getElementById("gridList"));
	if(cant == 1) {
		document.getElementById("frmMain").action = "administration.BPMNAction.do?action=viewVersion";
		submitForm(document.getElementById("frmMain"));	
	} else if (cant > 1) {
		alert(GNR_CHK_ONLY_ONE);
	} else {
...
function btnBackVer_click() {
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=backToVer";
	document.getElementById("frmMain").target = "_self";
	submitForm(document.getElementById("frmMain"));
}
...
function btnBackPro_click() {
	...
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=backToPro&defaultTab="+defaultTab;
	document.getElementById("frmMain").target = "_self";
	submitForm(document.getElementById("frmMain"));
}
...
function proTypeChange() {
	try{
	if (flashLoaded) {
		var cmb=document.getElementById("txtProAction");
		var type="";
		if(cmb.tagName=="SELECT"){
...
function entChange() {
	...
	//	setEntity();
	//}
	//mandar por ajax el id al server
	var result = askAjax("administration.BPMNAction.do","action=setEntity&busEntId="+document.getElementById("selBusEnt").value + "&windowId=" + windowId,	function(result){} );
...
function loadQuerys() {
	...
	//var sXMLSourceUrl = URL_ROOT_PATH + "/programs/administration/BPMN/querysXML.jsp?proAction=" + proAction + "&busEntId=" + busEntId;;
	var sXMLSourceUrl = "administration.BPMNAction.do?action=querys&proAction=" + proAction + "&busEntId=" + busEntId;
...
function btnConfPools_click(event) {
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=addPools&event=" + EVENT;
...
function alterPools(event) {
	openModal("/programs/administration/bpmn/pools.jsp?event=" + event,500,500);
}

function alterMessage(event) {
	openModal("/programs/administration/bpmn/message.jsp?event=" + event,500,300);
}
...
function btnConfMessage_click() {
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=alterMessage&event=" + EVENT;
	submitFormModal(document.getElementById("frmMain"));
}	
...
function lockUnlockProcess(lock,proId){
	...
	http_request.open('POST', "administration.BPMNAction.do?action=lockUnlockProcess"+windowId, false);
	http_request.setRequestHeader("content-type","application/x-www-form-urlencoded");
...
function canModifyProcess(){
	var	http_request = getXMLHttpRequest();
	http_request.open('POST', "administration.BPMNAction.do?action=canModifyProcess"+windowId, false);
	http_request.setRequestHeader("content-type","application/x-www-form-urlencoded");
...
function getUsrLockProcess(proId){
	var	http_request = getXMLHttpRequest();
	http_request.open('POST', "administration.BPMNAction.do?action=getUsrLockProcess"+windowId, false);
	http_request.setRequestHeader("content-type","application/x-www-form-urlencoded");
...
function btnImportXPDL_click(){
	document.getElementById("frmMain").action = "administration.BPMNAction.do?action=importXPDL";
	submitForm(document.getElementById("frmMain"));
}


function btnExportXPDL_click(){
	var cant = chksChecked(document.getElementById("gridList"));
	if(cant == 1) {
		exportXPDL();	
	} else if (cant > 1) {
...
function exportXPDL(){
	var rets = openModal("/programs/modals/export.jsp?hiddeHtml=true&hiddeTXT=true&hiddePdf=true&hiddeExcel=true&hiddeCSV=true&hiddeCount=true",500,220);
	var doAfter=function(rets){
		if (rets != null) {
			if (rets[0] == "xpdl") {
				document.getElementById("frmMain").action = "administration.BPMNAction.do?action=exportXPDL";
			}
	
			document.getElementById("frmMain").target = "iframeDownload";
			document.getElementById("frmMain").submit();
			document.getElementById("frmMain").target = "_self";
		}
	}
...
function loadProyectPerms(){
	...
	var sXMLSourceUrl = "administration.BPMNAction.do?action=getProjPermssions&prjId=" + prjId;
	var xmlLoad=new xmlLoader();
	...
	if(cant != 0) {
		if(cant == 1 && canWrite()==false) {
			...
			if (confirm(MSG_REG_CUBE)) {
				document.getElementById("frmMain").action = "administration.BPMNAction.do?action=regCbe";
