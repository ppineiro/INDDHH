...
function openData(element,type){
	...
	if (type=="proRedAtts"){ //Atributos redundantes
		sXMLSourceUrl =  "<%=Parameters.ROOT_PATH%>/administration.ProcessAction.do?action=getTreeXMLForAddAttDim&opt=1" + windowId;
	}else if (type=="processTasks"){ //Tareas y subprocesos utilizados por el proceso (todos en un mismo nivel)
		sXMLSourceUrl =  "<%=Parameters.ROOT_PATH%>/administration.ProcessAction.do?action=getTreeXMLForAddAttDim&opt=2" + "&proId=" + objId + windowId;
	}else if (type=="entityForms"){ //Formularios de entidad
		var proEleId = obj.getElementsByTagName("INPUT")[1].value;
		var proId = obj.getElementsByTagName("INPUT")[2].value;
		sXMLSourceUrl =  "<%=Parameters.ROOT_PATH%>/administration.ProcessAction.do?action=getTreeXMLForAddAttDim&opt=3" + "&proId=" + proId + "&proEleId=" + proEleId + windowId;
		...
		sXMLSourceUrl =  "<%=Parameters.ROOT_PATH%>/administration.ProcessAction.do?action=getTreeXMLForAddAttDim&opt=5" + "&frmId=" + objId + windowId;
	}else if (type =="processForm"){ //Atributos de un formulario de proceso
		sXMLSourceUrl =  "<%=Parameters.ROOT_PATH%>/administration.ProcessAction.do?action=getTreeXMLForAddAttDim&opt=6" + "&frmId=" + objId + windowId;
	}
...
function getSelected(){
	...
	if (selRedundantsAtts.length > 0){
		...
		if (str != ""){
			...
			http_request.open('POST', "administration.ProcessAction.do?action=getAttsInfo"+windowId, false);
