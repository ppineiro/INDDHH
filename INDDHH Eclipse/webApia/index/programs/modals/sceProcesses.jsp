...
function btnSearch_click(){
	document.getElementById("gridProcess").clearTable();
	doXMLLoad();
}

function doXMLLoad(){
	var sXMLSourceUrl = "<%=Parameters.ROOT_PATH%>/programs/modals/sceProcessesXML.jsp?name=" + escape(document.getElementById("txtNom").value) + "&showAll=" + document.getElementById("showAll").value;
		...
		if (xmlRoot.childNodes.length == 0) {
			...
			for(i=0;i<xmlRoot.childNodes.length;i++){
				if (i >= <%=Parameters.MAX_RESULT_MODAL %>) {
					...
					oTd0.setAttribute("proId",xRow.childNodes[0].firstChild.nodeValue);
					oTd0.setAttribute("proName",xRow.childNodes[1].firstChild.nodeValue);
					oTd0.setAttribute("proVerId",xRow.childNodes[2].firstChild.nodeValue);
					if (xRow.childNodes[3].firstChild != null){
						oTd0.setAttribute("proFathersId",xRow.childNodes[3].firstChild.nodeValue);
					}else{
						oTd0.setAttribute("proFathersId","");
					}
					...
					document.getElementById("gridProcess").addRow(oTr);
				}
			}
		}
	}else{
...
function getSelected(){
	var oRows = document.getElementById("gridProcess").selectedItems;
	if (oRows != null) {
		...
		for (i = 0; i < oRows.length; i++) {
			...
			arr = new Array();
			arr[0] = oTd.getAttribute("proId");
			arr[1] = oTd.getAttribute("proName");
			arr[2] = oTd.getAttribute("proVerId");
			arr[3] = oTd.getAttribute("proFathersId");
