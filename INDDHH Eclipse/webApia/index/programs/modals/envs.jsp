...
function btnSearch_click(){
	document.getElementById("gridEnvs").clearTable();
	doXMLLoad();
}

function doXMLLoad(){
	var sXMLSourceUrl = "envsXML.jsp?name=" + escape(document.getElementById("txtNom").value) ;
		...
		if (xmlRoot.childNodes.length == 0) {
			...
			for(i=0;i<xmlRoot.childNodes.length;i++){
				if (i >= <%=Parameters.MAX_RESULT_MODAL %>) {
					...
					if(xRow.childNodes[1].firstChild){
					...
					document.getElementById("gridEnvs").addRow(oTr);
				}
			}
		}
	}else{
...
function getSelected(){
	var oRows = document.getElementById("gridEnvs").selectedItems;
	if (oRows != null) {
