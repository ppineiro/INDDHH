...
function doXMLLoad(){
	var sXMLSourceUrl = "<%=Parameters.ROOT_PATH%>/programs/modals/dataDictionaryXML.jsp?name=" + escape(document.getElementById("txtNom").value);
		...
		if (xmlRoot.childNodes.length == 0) {
			...
			for(i=0;i<xmlRoot.childNodes.length;i++){
				if (i >= <%=Parameters.MAX_RESULT_MODAL %>) {
					...
					oTd0.setAttribute("attName",xRow.childNodes[1].firstChild.nodeValue);
					oTd0.setAttribute("attLabel",xRow.childNodes[2].firstChild.nodeValue);
					oTd0.setAttribute("attType",xRow.childNodes[3].firstChild.nodeValue);
...
function getSelected(){
	...
	if (oRows != null) {
		...
		for (i = 0; i < oRows.length; i++) {
			...
			arr = new Array();
			arr[0] = oTd.getAttribute("attId");
			arr[1] = oTd.getAttribute("attName");
			arr[2] = oTd.getAttribute("attLabel");
			arr[3] = oTd.getAttribute("attType");
