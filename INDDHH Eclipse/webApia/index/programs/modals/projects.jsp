...
function btnSearch_click(){
	document.getElementById("gridEntities").clearTable();
	doXMLLoad();
}


function doXMLLoad(){
	var sXMLSourceUrl = "projectsXML.jsp?name=" + escape(document.getElementById("txtNom").value) + "&envId=<%=request.getParameter("envId")%>";
		...
		if (xmlRoot.childNodes.length == 0) {
			...
			for(i=0;i<xmlRoot.childNodes.length;i++){
				if (i >= <%=Parameters.MAX_RESULT_MODAL %>) {
					...
					oTd0.projectId = xRow.childNodes[0].firstChild.nodeValue;
					oTd0.projectName = xRow.childNodes[1].firstChild.nodeValue;
					if(xRow.childNodes[2].firstChild){
						oTd0.entityDesc = xRow.childNodes[2].firstChild.nodeValue;
					}
					...
					document.getElementById("gridEntities").addRow(oTr);
				}
			}
		}
	}else{
...
function getSelected(){
	var oRows = document.getElementById("gridEntities").selectedItems;
	if (oRows != null) {
