...
<%@page import="com.dogma.business.querys.factory.QueryColumns"%>
<%@page import="com.dogma.vo.BusEntityVo"%>
<%@page import="biz.statum.sdk.util.FlagUtil"%>
<%@page import="biz.statum.sdk.util.StringUtil"%>
<%@page import="com.apia.security.SecurityFacade"%>
<%@page import="com.dogma.vo.QueryVo"%>
...
<html>
<head>
	<title>Apia 3.0 Functionality URL Verifier</title>
	<style type="text/css">
...
<%
	...
	if ("all".equals(doUpdate) || "query".equals(doUpdate)) { %>
		<hr>
		<b>Update of query functionalities</b><br><%
	    ...
	    try {
			...
			String error = StringUtil.toString(e, true); %>
			<hr>
			<font color="red">Error found: <%= e.getMessage() %></font><br>
			<%= error %><%
	...
	if ("all".equals(doUpdate) || "entity".equals(doUpdate)) {
	    %><hr>
	    <b>Update of business entities functionalities</b><br><%
	    try {
			dbConn = DBManagerUtil.getApiaConnection();
			Test test = new Test();
			con =  test.getDBConnection2(dbConn);
	...
	if ("all".equals(doUpdate) || "report".equals(doUpdate)) {
	    %><hr>
	    <b>Update of reports functionalities</b><br><%
	    try {
			dbConn = DBManagerUtil.getApiaConnection();
			Test test = new Test();
			con =  test.getDBConnection2(dbConn);
	...
	if ("all".equals(doUpdate) || "process".equals(doUpdate)) {
	    %><hr>
	    <b>Update of process functionalities</b><br><%
	    try {
			dbConn = DBManagerUtil.getApiaConnection();
			Test test = new Test();
			con =  test.getDBConnection2(dbConn);
	...
	if ("all".equals(doUpdate) || "bi".equals(doUpdate)) {%>
		<hr>
	    <b>Update of BI Navigator functionalities</b><br><%
	    try {
			dbConn = DBManagerUtil.getApiaConnection();
			Test test = new Test();
			con =  test.getDBConnection2(dbConn);
		...
		<hr>
	    <b>Update of BI Viewer functionalities</b><br><%
	    try {
			dbConn = DBManagerUtil.getApiaConnection();
			Test test = new Test();
			con =  test.getDBConnection2(dbConn);
