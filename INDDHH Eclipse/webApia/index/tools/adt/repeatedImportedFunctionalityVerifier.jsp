...
<%@page import="biz.statum.sdk.util.StringUtil"%><%@page import="java.util.ArrayList"%><%@page import="java.sql.ResultSet"%><%@page import="java.sql.PreparedStatement"%><%@page import="com.dogma.vo.FunctionalityVo"%><%@page import="java.util.Collection"%><%@page import="java.sql.SQLException"%><%@page import="com.st.db.dataAccess.StatementFactory"%><%@page import="com.st.db.dataAccess.DBManager"%><%@page import="com.st.db.dataAccess.DBAdmin"%><%@page import="java.sql.Connection"%><%@page import="com.st.db.dataAccess.DBConnection"%><%@page import="com.st.db.dataAccess.ConnectionDAO"%><%@page import="com.dogma.dataAccess.DBManagerUtil"%><html><head><title>Repeated Imported Functionality Verifier</title><style type="text/css">
		body {
			font-family: verdana;
			font-size: 10px;
		}
...
protected class Test extends DBAdmin {
	...
	private void addError(StringBuffer buffer, Throwable e) {
		this.addError(buffer, e.getMessage());
		this.addMessage(buffer,StringUtil.toString(e, true));
	}
	...
	private void updateEnvPrfFunctionalityToUse(Connection conn, Integer fncIdToKeep, Collection<Integer> idsWithError) throws SQLException {
		...
		boolean funcionalityIsInProifle = profilesToDeleteFrom != null && profilesToDeleteFrom.size() > 0;
		
		if (! funcionalityIsInProifle) { //Esto quiere decir que la funcionalidad con la que nos quedamos no está asociada a un perfil
			statement = StatementFactory.getStatement(conn, "delete from env_prf_functionality where fnc_id = ?" , false);
			try {
				for (Integer fncId : idsWithError) {
					if (fncIdToKeep.equals(fncId)) continue;
					statement.setInt(1, fncId.intValue());
		...
		} else { //Borramos las demás funcionliadades que están asociadas a los distintos perfiles
...
Options: 
	<%= verify?"<b>":"" %><a href="?action=verify" title="Check process functionalities">[ Verify ]</a><%= verify?"</b>":"" %><%= doSolution?"<b>":"" %><a href="?action=doSolution" title="Check and correct process functionalities">[ Verify &amp; Correct ]</a><%= doSolution?"</b>":"" %><a href="?" title="Init the screen">[ Init ]</a><br><hr><div style="overflow: auto; white-space: nowrap; width: 100%; height: 95%"><%
if (action != null) {
