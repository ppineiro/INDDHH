<%@page import="com.dogma.dao.EnvironmentDAO"%><%@page import="com.dogma.vo.EnvironmentVo"%><%@page import="java.io.FileFilter"%><%@ page contentType="text/html; charset=iso-8859-1" language="java"%><%@ page import="com.st.db.dataAccess.*" %><%@ page import="java.util.*" %><%@ page import="java.sql.*" %><%@page import="java.io.File"%><%@page import="com.dogma.dao.BusClassDAO"%><%@page import="com.dogma.vo.filter.BusClassFilterVo"%><%@page import="com.dogma.vo.BusClassVo"%><%@page import="com.dogma.Parameters"%><%@page import="com.st.util.FileUtil"%><%@page import="java.io.FileNotFoundException"%><%@page import="com.st.util.StringUtil"%><%@page import="com.dogma.dataAccess.DBManagerUtil"%><html><head><title>BusClassVerifier</title><style type="text/css">
		body {
			font-family: verdana;
			font-size: 10px;
		}
...
Options: 
	<%= isVerifiyJava?"<b>":"" %><a href="?action=verifyJava" title="Check if all require .class are available">[ Verify Java ]</a><%= isVerifiyJava?"</b>":"" %><%= isVerifiyJavaE?"<b>":"" %><a href="?action=verifyJavaE" title="Check if all require .class are available (only shows errors/warnings)">[ Verify Java OE ]</a><%= isVerifiyJavaE?"</b>":"" %><%= isVerifiyJS?"<b>":"" %><a href="?action=verifyJS" title="Check if all require .js are available and correct">[ Verifiy JavaScript ]</a><%= isVerifiyJS?"</b>":"" %><%= isVerifiyJSE?"<b>":"" %><a href="?action=verifyJSE" title="Check if all require .js are available and correct (only shows errors/warnings)">[ Verifiy JavaScript OE ]</a><%= isVerifiyJSE?"</b>":"" %><%= isCorrectJS?"<b>":"" %><a href="?action=correctJS" title="Implements 'Verifiy JavaScript' and tries to correct the files (backup of files is recomended)" onclick="return confirm('Database will be updated. Rollback not available.\n\rWant to continue?');" >[ Correct JavaScript ]</a><%= isCorrectJS?"</b>":"" %><%= isMigFiles?"<b>":"" %><a href="?action=migFiles" title="Migrates files from <origine>_mig_<destination> environments">[ Migrate files ]</a><%= isMigFiles?"</b>":"" %><%= isUpdateMigFiles?"<b>":"" %><a href="?action=isUpdateMigFiles" title="Migrates files from <origine>_mig_<destination> environments" onclick="return confirm('Database will be updated. Rollback not available.\n\rWant to continue?');" >[ Update migrated JS files ]</a><%= isUpdateMigFiles?"</b>":"" %><a href="?" title="Init the screen">[ Init ]</a><br><hr><div style="overflow: auto; white-space: nowrap; width: 100%; height: 95%"><%
if (action != null) {
