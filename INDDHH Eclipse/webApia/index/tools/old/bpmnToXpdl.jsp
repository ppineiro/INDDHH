<%@page import="com.st.db.dataAccess.DAOException"%><%@page import="com.dogma.DogmaException"%><%@page import="com.st.db.dataAccess.DBConnection"%><%@page import="com.st.db.dataAccess.StatementFactory"%><%@page import="com.st.util.i18n.I18N"%><%@page import="java.io.*"%><%@page import="java.math.BigInteger"%><%@page import="java.sql.*"%><%@page import="java.util.Date"%><%@page import="java.util.*"%><%@page import="org.apache.xmlbeans.*"%><%@page import="org.wfmc.x2008.xpdl21.*"%><%@page import="com.dogma.*"%><%@page import="com.dogma.vo.*"%><%@page import="com.dogma.dao.*"%><%@page import="com.st.util.*"%><%@page import="com.dogma.vo.custom.ProDefinitionVo"%><%@page import="com.st.db.dataAccess.ConnectionDAO"%><%@page import="java.sql.Connection"%><%@page import="com.dogma.dataAccess.DBManagerUtil"%><%@page import="com.apia.core.CoreFacade"%><%@page import="org.wfmc.x2008.xpdl21.ActivitiesDocument.Activities"%><%@page import="org.wfmc.x2008.xpdl21.ActivityDocument.Activity"%><%@page import="org.wfmc.x2008.xpdl21.ActivitySetDocument.ActivitySet"%><%@page import="org.wfmc.x2008.xpdl21.ActivitySetsDocument.ActivitySets"%><%@page import="org.wfmc.x2008.xpdl21.BlockActivityDocument.BlockActivity"%><%@page import="org.wfmc.x2008.xpdl21.ConditionDocument.Condition"%><%@page import="org.wfmc.x2008.xpdl21.ConnectorGraphicsInfoDocument.ConnectorGraphicsInfo"%><%@page import="org.wfmc.x2008.xpdl21.ConnectorGraphicsInfosDocument.ConnectorGraphicsInfos"%><%@page import="org.wfmc.x2008.xpdl21.CoordinatesDocument.Coordinates"%><%@page import="org.wfmc.x2008.xpdl21.EndEventDocument.EndEvent"%><%@page import="org.wfmc.x2008.xpdl21.EventDocument.Event"%><%@page import="org.wfmc.x2008.xpdl21.ImplementationDocument.Implementation"%><%@page import="org.wfmc.x2008.xpdl21.LoopDocument.Loop"%><%@page import="org.wfmc.x2008.xpdl21.LoopMultiInstanceDocument.LoopMultiInstance"%><%@page import="org.wfmc.x2008.xpdl21.LoopStandardDocument.LoopStandard"%><%@page import="org.wfmc.x2008.xpdl21.NodeGraphicsInfoDocument.NodeGraphicsInfo"%><%@page import="org.wfmc.x2008.xpdl21.NodeGraphicsInfosDocument.NodeGraphicsInfos"%><%@page import="org.wfmc.x2008.xpdl21.PackageHeaderDocument.PackageHeader"%><%@page import="org.wfmc.x2008.xpdl21.ParticipantDocument.Participant"%><%@page import="org.wfmc.x2008.xpdl21.ParticipantsDocument.Participants"%><%@page import="org.wfmc.x2008.xpdl21.PerformerDocument.Performer"%><%@page import="org.wfmc.x2008.xpdl21.PerformersDocument.Performers"%><%@page import="org.wfmc.x2008.xpdl21.PoolDocument.Pool"%><%@page import="org.wfmc.x2008.xpdl21.PoolsDocument.Pools"%><%@page import="org.wfmc.x2008.xpdl21.RouteDocument.Route"%><%@page import="org.wfmc.x2008.xpdl21.StartEventDocument.StartEvent"%><%@page import="org.wfmc.x2008.xpdl21.SubFlowDocument.SubFlow"%><%@page import="org.wfmc.x2008.xpdl21.TaskDocument.Task"%><%@page import="org.wfmc.x2008.xpdl21.TransitionDocument.Transition"%><%@page import="org.wfmc.x2008.xpdl21.TransitionsDocument.Transitions"%><%@page import="org.wfmc.x2008.xpdl21.WorkflowProcessesDocument.WorkflowProcesses"%><%//@page import="com.dogma.xpdl.BaseToXPDL"%><head><title>Base to XPDL</title><style type="text/css">
...
public static String escapeXMLForXPDLExport(String s) {
	if (s != null) {
		String auxStr = StringUtil.replace(s, "&", "&amp;");
		auxStr =StringUtil. replace(auxStr, ">", "&gt;");
		auxStr = StringUtil.replace(auxStr, "<", "&lt;");
		auxStr = StringUtil.replace(auxStr, "\"", "&amp;quot;");
		auxStr = StringUtil.replace(auxStr, "'", "&apos;");
		return auxStr;
	...
	private org.wfmc.x2008.xpdl21.PackageDocument getXPDLWorkflowProcStandard(DBConnection dbConn, Connection conn, Integer envId, ProDefinitionVo proVo) throws DAOException, DogmaException {
		...
		try {
			...
			while(resultSet.next()) {
				...
				docEvts += "busClaDesc=&quot;" + escapeXMLForXPDLExport(busClaDesc) +"&quot; ";
				docEvts += "/&gt;";
				addEvtDoc = true;
				addProcDoc = true;
			}
			...
			while(resultSet.next()) {
				...
				if (resultSet.getObject("pro_ele_start") != null || resultSet.getObject("pro_ele_end") != null) {
					...
					while(resultSetFrms.next()) {
						...
						docFormsEvts += "frmTitle=&quot;" + escapeXMLForXPDLExport(frmTitle) + "&quot; ";
						...
						while(resultSetAtts.next()) {
							...
							docAtts += "attLabel=&quot;" + escapeXMLForXPDLExport(resultSetAtts.getString("att_label")) + "&quot; ";
						...
						if (resultSetEvts != null) {
							while(resultSetEvts.next()) {
								...
								docFrmEvts += "busClaDesc=&quot;" + escapeXMLForXPDLExport(busClaDesc) + "&quot; ";
								docFrmEvts += "/&gt;";
								addFrmEvtDoc = true;
							}
						}
...
if (logged.booleanValue()) {
	...
	try {
		...
		%><div><center><b>User Options</b></center><center><a href="?logout=yes">Logout</a></center><br><center><b>Operation Complete</b></center><br></div><% if (processedXPDLs.size() > 0) { %><div><center><b>The generated XPDLs have been stored in the configured Temporal Folder Path.</b></center><br><center><b>The following processes have been successfully generated: </b></center><br><% }
		for (String procName : processedXPDLs) {
			%><center><%=procName%></center><br><%
		}
		%></div><%
		if (errorsXPDLs.size() > 0) { %><div><center><b>Check the definition of the following processes, errors occurred while trying to generate their corresponding XPDLs.</b></center><br><center><b>The following processes haven't been successfully generated: </b></center><br><% }
		...
		Process Name: <input type="text" name="proName"><br><input type="submit" value="Login"></form><% } %></body></html>
