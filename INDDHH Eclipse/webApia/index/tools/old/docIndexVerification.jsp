...
<%@page import="com.dogma.Parameters"%><%@page import="org.apache.lucene.analysis.Analyzer"%><%@page import="org.apache.lucene.analysis.standard.StandardAnalyzer"%><%@page import="org.apache.lucene.queryParser.QueryParser"%><%@page import="org.apache.lucene.search.Query"%><%@page import="org.apache.lucene.search.Searcher"%><%@page import="org.apache.lucene.search.IndexSearcher"%><%@page import="org.apache.lucene.search.TopDocs"%><%@page import="org.apache.lucene.search.Sort"%><%@page import="org.apache.lucene.search.TopDocCollector"%><%@page import="java.util.ArrayList"%><%@page import="java.util.Collection"%><%@page import="org.apache.lucene.document.Document"%><%@page import="org.apache.lucene.index.IndexReader"%><%@page import="org.apache.lucene.index.IndexWriter"%><%@page import="com.st.db.dataAccess.DBManager"%><%@page import="com.st.db.dataAccess.DBConnection"%><%@page import="com.dogma.dataAccess.DBManagerUtil"%><%@page import="com.st.db.dataAccess.DBAdmin"%><%@page import="java.util.HashMap"%><%@page import="java.sql.Connection"%><%@page import="com.st.db.dataAccess.ConnectionDAO"%><%@page import="com.st.db.dataAccess.StatementFactory"%><%@page import="java.sql.PreparedStatement"%><%@page import="java.sql.ResultSet"%><%@page import="java.util.Iterator"%><html><head><title>Document Index Verification</title><style type="text/css">
...
%><% if (logged.booleanValue()) { 
	if (Parameters.DOC_INDEX_ACTIVE) { %>
		...
		Amount of documents in index: <b><%= indexDocIds.size() %></b>.<br><% if (errorMsg != null) { %><b>Error</b> while getting documents information: <%= errorMsg %><br><% } %><br><%
		if (errorMsg == null) { %>
			...
			Amount of documents in DB: <b><%= dbDocIds.size() %></b>.<br><% if (errorMsg != null) { %><b>Error</b> while getting documents information: <%= errorMsg %><br><% }
	...
	} else { %><b>Note</b>: the document index in Apia is not active. There is no reason to run/execute this.
<% } %><hr><a href="?logout=yes">Logout</a><%
} else { %><form action="" method="post"><b>Login is require to continue</b><br>
		User: <input type="text" name="user"><br>
		Password: <input type="password" name="pwd"><br><input type="submit" value="Login"></form><b>Warning</b>: the execution of this JSP will may consume a lot of RAM memory and CPU. Before getting all files the index will be optimized.</b><% } %></body></html>