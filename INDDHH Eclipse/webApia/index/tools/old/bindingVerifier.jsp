<%@page import="com.dogma.DogmaException"%><%@page import="com.dogma.vo.BusClaParBindingVo"%><%@page import="com.dogma.dao.BusClaParBindingDAO"%><%@page import="com.dogma.dao.gen.GenericDAO"%><%@page import="java.io.PrintWriter"%><%@page import="java.io.ByteArrayOutputStream"%><%@ page contentType="text/html; charset=iso-8859-1" language="java"%><%@ page import="com.st.db.dataAccess.*" %><%@ page import="java.util.*" %><%@ page import="java.sql.*" %><%@page import="java.io.File"%><html><head><title>Process Condition Verifier</title><style type="text/css">
		body {
			font-family: verdana;
			font-size: 10px;
		}
...
Options: 
	<%= isVerifiy?"<b>":"" %><a href="?action=verify" title="Check if all events bindings are OK">[ Verify bindings ]</a><%= isVerifiy?"</b>":"" %><%= isCorrect?"<b>":"" %><a href="?action=correct" title="Correct all bad events bindings" onclick="return confirm('Database will be updated. Rollback not available.\n\rWant to continue?');">[ Correct events bindings ]</a><%= isCorrect?"</b>":"" %><a href="?" title="Init the screen">[ Init ]</a><br><hr><div style="overflow: auto; white-space: nowrap; width: 100%; height: 95%"><%
if (action != null) {
