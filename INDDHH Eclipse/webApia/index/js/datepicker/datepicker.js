...
var DatePicker = new Class({
	...
	attach: function() {
		...
		$$(this.attachTo).each(function(item, index) {
			...
			if ($chk(item.get('value')) && item.get('value') != '____________') {
			...
			if ($chk(this.options.toggleElements)) {
				...
				clone.addEvents({
					'blur': function() {
						var dateParts = clone.get('value').split("/");
						if (new Date(dateParts[1] + "/" + dateParts[0] + "/" + dateParts[2]).format("%d/%m/%Y") != clone.get('value')) {
							clone.set('value', '');
						}
				...
				clone.addEvents({
					'keydown': function(e) {
						...
						/*if (this.options.allowEmpty && (e.key == "delete" || e.key == "backspace")) {
							...
							if (e.key != "backspace") {
								if(this.options.format == 'd-m-Y' || this.options.format == 'd/m/Y') {
								...
								} else if(this.options.format == 'm-d-Y' || this.options.format == 'm/d/Y') {
								...
								} else if(this.options.format == 'Y-m-d' || this.options.format == 'Y/m/d') {
	...
	onFocus: function(original_input, visual_input) {
		if (original_input.disabled) return;
		...
		if(document.body.clientHeight < pos_y + 165)
			pos_y -= 165 + d.height;
		
		this.show({ left: d.left + this.options.positionOffset.x, top: pos_y }, init_visual_date);
