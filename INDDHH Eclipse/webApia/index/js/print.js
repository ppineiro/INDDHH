...
function printForms() {
	...
	if(!titleSpan)
		titleSpan = $('frmData').getElement('div.taskTitle')
	...
	var modal = ModalController.openContentModal({url: CONTEXT + '/page/generic/printBlanck.jsp?' + TAB_ID_REQUEST, content: modalContent}, 750, 600);
...
function printUserQuery(qryTitle,results,headers,values,graphs){
	...
	for (var v = 0; v < values.length; v++){
		for (var h = 0; h < headers.length; h++){
			if (h == 0){
				table = new Element("table.tbl-query",{"colspan":"4","cellspacing":"0"}).inject(modalContent);
			}
			if (h % 2 == 0){
