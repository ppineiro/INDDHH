...
SynchronizeFields.DIRTY = "dirty";
SynchronizeFields.SYNC_FAILED = "syncFailed";
SynchronizeFields.SYNC_OBSERVER = "sync_observer";
...
SynchronizeFields.syncAJAX = function(container, value, retries) {
	...
	/*var myRequest = new Request({	    
		url: 'apia.execution.FormAction.run?action=processFieldSubmit&frmId=' +  frmId + '&frmParent=' + frmParent + '&timestamp=' + timestamp + '&attId=' + attId + val_to_server + '&index=' + (element.index ? element.index : '0') + TAB_ID_REQUEST,
	    
		onSuccess: function(responseText, responseXML) {
	    	//AJAX exitoso
	    	if(responseXML && responseXML.childNodes && responseXML.childNodes.length && responseXML.childNodes[0].tagName == 'result') {
	    		if(responseXML.childNodes[0].getAttribute('success')) {
	...
	var myRequest = new Request({
		url: 'apia.execution.FormAction.run?action=processFieldSubmit&frmId=' +  frmId + '&frmParent=' + frmParent + '&timestamp=' + timestamp + '&attId=' + attId + '&index=' + (element.index ? element.index : '0') + TAB_ID_REQUEST,
...
SynchronizeFields.syncAJAXMulti = function(container, frmId, frmParent, attId, value, id, retries) {
	...
	var myRequest = new Request({	    
		url: 'apia.execution.FormAction.run?action=processFieldSubmit&frmId=' +  frmId + '&frmParent=' + frmParent + '&timestamp=' + (new Date().getTime()) + '&attId=' + attId + /*val_to_server +*/ '&index=' + id + TAB_ID_REQUEST,
