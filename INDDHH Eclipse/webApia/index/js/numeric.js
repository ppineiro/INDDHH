...
Numeric.setNumeric = function (input, event) {
	input.validate = function() {
		Numeric.validateNumber(this);
	}
	
	if(event && typeOf(event) == 'string')
		input.addEvent(event, input.validate);
	else
		input.addEvent('blur', input.validate);
}
	...
	if(!regExp.test(value)) {
		...
		if(try_convert) {
			...
			if(decimalIndex >= 0) {
				...
				var max_decimal_length = Number.min(amountDecimalSeparator, value.length - decimalIndex - 1);
				max_decimal_length = Number.max(max_decimal_length, amountDecimalZeros);
		...
		var attLabel = element.get('attLabel');
