		...
		btnPrint.addEvent('click', function(e){
			...
			if ($('chartContainer')){
				...
				$$('div.chartButton').each(function (chartBtn){
					chartValues.push({'id':chartBtn.get("qryChtId"), 'title': chartBtn.get("title")});
				});
...
function showChart(evt) {
	...
	$('chartContainer').set("qryChtId", null);
	
	tabComponent.changeTo(1);
	loadChart(this.get("qryChtId"));
}
...
function loadChart(qryChtId) {
	...
	if (qryChtId == null) {
		qryChtId = chartContainer.get("qryChtId");
		if (qryChtId==null) qryChtId = chartContainer.getElements("img").get("qryChtId")[0];
		chartContainer.set("qryChtId", null);
	}
...
//	if (! chartImg)	{
	...
	chartImg = new Element('img', {
		...
		chartImg.set('qryChtId', qryChtId);
...
function doZoom(evt) {
	...
	var qryChtId = this.get('qryChtId');
...
function fncExecuteAction() {
	...
	if (doRefresh == "true") {
		...
		if(url.indexOf('?') > 0){
			...
			// ejecuci�n de consultas del usuario
