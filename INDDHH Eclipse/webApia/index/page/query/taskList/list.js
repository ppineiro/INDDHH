function initPage(){
	...
	var btnShowOnlyMyTasks = $('btnShowOnlyMyTasks');
	var btnWork = $('btnWork');
	...
	if (btnShowOnlyMyTasks) {
		btnShowOnlyMyTasks.addEvent('click', function(evt){
			this.toggleClass('pressed');
			var request = new Request({
				method: 'post',
				url: CONTEXT + URL_REQUEST_AJAX + "?action=changeOnlyMyTask" + TAB_ID_REQUEST,
				onRequest: function() { sp.show(true); },
				onComplete: function(resText, resXml) { modalProcessXml(resXml); sp.hide(true); }
			}).send();
		});
