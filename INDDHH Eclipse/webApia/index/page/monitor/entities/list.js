...
function initPage(){
	...
	if (btnChngPrio){
		...
		if (selectionCount($('tableData')) > 1) {
			...
			window.location = CONTEXT + URL_REQUEST_AJAX + '?action=view&id=' + id + TAB_ID_REQUEST+"&dateFrom="+$('modifiedDateFilterStart').value+"&dateTo="+$('modifiedDateFilterEnd').value+"&busEntId="+$('entNumFilter').value+"&pageNumber="+$('navBarCurrentPage').value;
		}
...
function drawFilters(xml){
	...
	for (var i=0;i<listFecha.length;i++){
		...
		datepicker.addEvent("change", function(e) {
			this.setFilter();
		});
	}
...
function openMonDocument(busEntTitle, busEntRegInst){
	...
	var url = CONTEXT + URL_REQUEST_AJAX_MON_DOCUMENT + '?action=init&favFncId=54&preFilBusEntity=true&busEntTitle=' + busEntTitle + '&busEntRegInst=' + busEntRegInst + TAB_ID_REQUEST;
