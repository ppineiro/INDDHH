...
function initPage(){
	...
	$('txtCreateFrom').addEvent("change", function(e) {
		this.setFilter();
	});
	$('txtCreateTo').setFilter = setFilter;
	$('txtCreateTo').addEvent("change", function(e) {
		this.setFilter();
	});
		...
		$('btnDown').addEvent("click", function(e){
			...
			if (selectionCount($('tableData')) > 1) {
				...
				if (trSelected.hasClass("noDown")){
					...
					new Request({
						method: 'post',
						url: CONTEXT + URL_REQUEST_AJAX + '?action=loadDownloadDocument&isAjax=true&id=' + id + TAB_ID_REQUEST,
