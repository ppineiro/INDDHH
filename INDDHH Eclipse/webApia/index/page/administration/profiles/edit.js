...
function initPage(){
	...
	addFnc.addEvent("click", function(e) {
		e.stop();
		/*ADDITIONAL_INFO_IN_TABLE_DATA = false;
		FNCMODAL_SELECTONLYONE = false;
		FNCS_ENVIRONMENT = envSelected;
		FNCS_EXCLUDED = getFncIdToExclude();
		showFncsModal(processFncsModalReturn);*/
		showTreeFncsModal(getFncIdToExclude().split(","),envSelected,processFncsModalReturn);		
	});
	...
	if (ajaxCallXml != null && ajaxCallXml.getElementsByTagName("dashboards")[0].getElementsByTagName("dashboard")) {
		...
		for (var i = 0; i < dashboards.length; i++){
			...
			element.addEvent('click', function(evt) {
				...
				var request = new Request({
					method: 'post',
					url: CONTEXT + URL_REQUEST_AJAX + '?action=removeDashboard&isAjax=true&envId=' + envSelected  + "&dshId=" + this.getAttribute('id') + TAB_ID_REQUEST,
					onRequest: function() { SYS_PANELS.showLoading(); },
...
function createEnv(envId,envName){
	var elemDiv = new Element("div", {'id': envId, 'name': envName, 'class': 'option optionMiddle'});
	...
	if (envId == 0){
		...
		elemDiv.addEvent('click', function(evt) { deleteEvt(this); });
	}
		
	var fncDiv = new Element("div", {'class': 'optionIcon optionModify'});
	fncDiv.tooltip(FNCS, { mode: 'auto', width: 110, hook: false });
...
function showFunc(env){
	...
	var request = new Request({
		...
		onRequest: function() { },
		onComplete: function(resText, resXml) { modalProcessXmlDashboards(resXml); spFncs.hide(true); }
	}).send();	
}
		...
		for (var i = 0; i < functionalities.length; i++){
			...
			if (!$("fnc_"+xmlFnc.getAttribute("id"))){
				...
				if (xmlFnc.getAttribute("type") == "F") {
					...
					ul.inject(elem);
					remove.addEvent('click', function(evt) { folToRemove = this.parentNode.parentNode; this.OBJtooltip.hide(); confirmDelete(); });
					...
					remove.addEvent('click', function(evt) { fncToRemove = this.parentNode.parentNode; this.OBJtooltip.hide(); deleteFnc(); });
				}				
			}			
		}
	}
