...
function initPage(){
	...
	//['addPoolMail','addPoolMsg'].each(setTooltip);
...
function processParametersXml(ajaxCallXml){
	...
	if (ajaxCallXml != null) {
		...
		if (parameters != null && parameters.length > 0 && parameters.item(0) != null) {
			...
			for(var i = 0; i < parameters.length; i++) {
				...
				var p = new Element("div",{'class': 'option optionWidthParameter', 'id': "param_"+xmlParam.getAttribute("id"), html: xmlParam.getAttribute("name")+": "  }).inject(container);
				if (i % 2 == 0){ p.addClass("left"); } else { p.addClass("right"); }
				p.setAttribute("idNum",xmlParam.getAttribute("id"));
				p.setAttribute("paramType",xmlParam.getAttribute("type"));
				var input = new Element("input",{'type': 'text', 'id': "val_"+xmlParam.getAttribute("id"), 'value': xmlParam.getAttribute("value"), 'class': 'optionParameter' }).inject(p);
	...
	if (!hasParams){
		...
		span.tooltip(NO_PARAM_TT, { mode: 'auto', width: 100, hook: false });		
	}
}
...
function createPool(poolId,poolName,poolDesc){
	...
	var pool = new Element("div", {'id': pre+poolId, 'poolIdNum': poolId, html: poolName, 'class': 'option optionMiddle optionRemove pool'});
	pool.addEvent('click', function(evt) { if (this.OBJtooltip){this.OBJtooltip.hide()}; this.destroy(); });
	pool.setAttribute("poolName",poolName);
	if (poolDesc != null && poolDesc != "") { 
		pool.tooltip(poolDesc, { mode: 'auto', width: 100, hook: false });
