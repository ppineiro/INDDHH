...
function initPage(){
	...
	$('addFreeDay').tooltip(TT_ADD_FREE_DAY, { mode: 'auto', width: 100, hook: false });
	
	//Por defecto se deshabilitan los checkboxes de todo el día
...
function addFreeDay(day){
	if (!existDay(day) && isCorrect(day)){ 
		...
		var newDay = new Element("div", {'id': idFreeDay, 'class': 'option optionRemove'});
		...
		yearDiv.tooltip(TT_SHOW_YEAR, { mode: 'auto', width: 130, hook: false });
		yearDiv.addEvent('click', function(evt) { addRemoveYear(this.parentNode.getAttribute("id"),this); evt.stopPropagation(); });
