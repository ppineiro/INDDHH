...
function initPage(){
	...
	//['btnUpdate','btnCloseTab'].each(setTooltip);
	...
	for(var i = 0; i < arrayXml.length; i++){
		...
		if (!deleted){
			...
			if (xmlWS.getAttribute("tooltip") == "PROC") {
				wsSpan.tooltip(TOOLTIP_WS_PROC, { mode: 'auto', width: 100, hook: false }); 
			} else if (xmlWS.getAttribute("tooltip") == "TSK") {
				wsSpan.tooltip(TOOLTIP_WS_TSK, { mode: 'auto', width: 100, hook: false });
			}						
		}
		//ACTION
		...
		if (action == "P"){
			...
			wsAction.tooltip(TOOLTIP_PUB_UNPUB, { mode: 'auto', width: 140, hook: false });
		...
		if (!deleted && hasStatus) {
			...
			wsUsr.tooltip(TOOLTIP_USERS, { mode: 'auto', width: 100, hook: false });
			...
			wssDiv.tooltip("WSS", { mode: 'auto', width: 100, hook: false });
