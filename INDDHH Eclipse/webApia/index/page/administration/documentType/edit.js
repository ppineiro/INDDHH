...
function setValues(){
	...
	container.getElements("div.optionRemoveNoImg").each(function(ext){
...
function loadExtensions(){
	var request = new Request({
		...
		onComplete : function(resText, resXml) { processXmlExtensions(resXml); }
	}).send();
}
...
function addExtension(ext,used){
	...
	used = used || IS_APIA_DOC_TYPE;
	var newExt = new Element("div", {'class': 'option ' + (used ? 'optionRemoveNoImg used' : 'optionRemove'), html: ext}).inject($('add'),"before");;
	newExt.setAttribute("ext",ext);
	if (!used) newExt.addEvent('click', function(evt) { processRemoveExt(this); });	
}
