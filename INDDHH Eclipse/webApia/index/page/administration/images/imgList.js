...
function initPage(){
	...
	//['btnCreate','btnUpdate','btnRemove','btnRefresh','btnDependencies','btnSort'].each(setAdmListTitle);
...
function btnDeleteClickConfirm() {
	...
	if(PRIMARY_SEPARATOR_IN_BODY) {
		...
		new Request({
			method : 'post',
			url : CONTEXT + URL_REQUEST_AJAX + '?action=deleteImages&isAjax=true&id=' + selected + TAB_ID_REQUEST,
			onRequest : function() { sp.show(true); },
			onComplete : function(resText, resXml) { modalProcessXml(resXml); sp.hide(true); }
		}).send();
	}
