var Check = new Class({
	...
	booleanOptions: [Field.PROPERTY_REQUIRED, Field.PROPERTY_DISABLED, Field.PROPERTY_READONLY, Field.PROPERTY_VISIBILITY_HIDDEN, Field.PROPERTY_CHECKED],
...
//		if(this.xml.getAttribute(Field.FUNC_CLICK) && !window.editionMode) {
	...
	parseXMLforGrid: function(td_container, grid_index, isGridReadonly) {
		...
		if(this.options[Field.PROPERTY_TOOLTIP])
			check.tooltip(this.options[Field.PROPERTY_TOOLTIP], { mode: 'auto', width: 200, hook: 0, mouse:true, click:false});
		...
		if(this.options[Field.PROPERTY_REQUIRED]) {
			if(!this.options[Field.PROPERTY_VISIBILITY_HIDDEN]) {
				div.addClass('gridCellRequired');
				check.setStyle('margin-left', Generic.getHiddenWidth(div) / 2 - Generic.getHiddenWidth(check) / 2);
			}
		}
...
//		if(this.xml.getAttribute(Field.FUNC_CLICK) && !window.editionMode) {
	...
	showFormListener: function() {
		var input = this.content.getChildren('input')[0] || (this.content.getChildren('div')[0] ? this.content.getChildren('div')[0].getChildren('input')[0] : null);
		if(input)
