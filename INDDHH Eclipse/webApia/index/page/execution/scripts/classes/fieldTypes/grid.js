	...
	parseXML: function(redrawing, new_row_xml) {
		...
		if(new_row_xml) {
			...
			if(this.editionTable) {
				...
				var edit_button = new Element('div.editable-row').inject(this.editionTable).addEvents({
...
//					rowHeights = this.divBody.getElements('tr').getStyle('height');
		...
		var tableHeader = new Element('table', {
			cellppading: 0,
			cellspacing: 0
		}).inject(divHeader);
		...
		if(this.xml.childNodes[0].nodeName == Grid.tags.gridHeader) {
			Array.from(this.xml.childNodes[0].childNodes).each(function(col_xml, col_index) {
				if(col_xml.nodeName == Grid.tags.gridHeaderCol) {
					...
					var gridColHeader = new GridHeader(th, col_title, this.xml.childNodes[col_index + 1].getAttribute('fieldType'), JSON.decode(this.xml.childNodes[col_index + 1].getAttribute('properties')), fn_col_select);
		...
		if(!this.options[Field.PROPERTY_HIDE_GRID_BTN] && !this.options[Field.PROPERTY_READONLY]) {
			...
			if(!this.options[Field.PROPERTY_GRID_HIDE_BTN_INCLUDE]) {
				if(!this.options[Field.PROPERTY_GRID_HIDE_BTN_DEL]) {
					...
					var btnEliminar = new Element('div.gridButton', {'html': GRID_BTN_DELETE}).inject(nav_buttons_options);
...
//						if(java_submit && !allAjaxRowDelete) {
				...
				if(!this.options[Field.PROPERTY_GRID_HIDE_BTN_ADD]) {
					...
					var btnAgregar = new Element('div.gridButton', {'html': GRID_BTN_ADD}).inject(nav_buttons_options);
...
//				if(this.xml.getAttribute(Field.FUNC_ROW_SORT) && !window.editionMode)
				...
				var btnBajar = new Element('div.gridButton', {'html': GRID_BTN_DOWN}).inject(nav_buttons_options);
...
//							java_submit = fn_row_sort(new ApiaField(this));
				...
				var btnSubir = new Element('div.gridButton', {'html': GRID_BTN_UP}).inject(nav_buttons_options);
...
var GridHeader = new Class({
	...
	initialize: function(th_container, title, type, opts, fn_col_select) {		
