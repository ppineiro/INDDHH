 ...
 * returns: A modal object, capable of fire the events: close, show.
 */
ApiaFunctions.admEntity = function(entName, entNum, parameters, width, height) {
	var url = CONTEXT + '/apia.execution.EntInstanceListAction.run?action=admEntity&busEntName=' + entName + TAB_ID_REQUEST;
	if (entNum)
	...
	return ModalController.openWinModal(url, width, height, null, null, null, true, true);
}

/*
ApiaFunctions.viewAdmEntity = function(entName, entNum) {
...
ApiaFunctions.disableActionButton = function(button) {
	...
	if(btnId) {
		btnId = $(btnId);
		if(btnId) {
			btnId.addClass('apia-disabled-rounded-button').getElement('button').set('disabled', true);
		}
	}
}
...
ApiaFunctions.enableActionButton = function(button) {
	...
	if(btnId) {
		btnId = $(btnId);
		if(btnId)
			btnId.removeClass('apia-disabled-rounded-button').getElement('button').erase('disabled');
	}
}
