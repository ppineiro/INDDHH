...
%><%@include file="../../includes/startInc.jsp" %><html><head><%@include file="../../includes/headInclude.jsp" %><link href="<system:util show="context" />/css/<system:util show="currentStyle" />/generalAdm.css" rel="stylesheet" type="text/css" ><script type="text/javascript" src="<system:util show="context" />/page/execution/entities/selectEntity.js"></script><script type="text/javascript" src="<system:util show="context" />/page/includes/adminActionsEdition.js"></script><script>
	...
	</script></head><body><div class="header"></div><div class="body" id="bodyDiv"><form id="frmData" action="" method="post"><div class="optionsContainer"><system:campaign inLogin="false" inSplash="false" location="verticalUp" /><div class="fncPanel info"><div class="title"><system:label show="tooltip" label="mnuManEntGlo" /></div><div class="content divFncDescription"><div class="fncDescriptionImgUsers"></div><div id="fncDescriptionText"><system:label show="text" label="dscFncAdmGloEnt" /></div><div class="clear"></div></div></div><%@include file="../../includes/adminActionsEdition.jsp" %><div class="fncPanel options"></div><system:campaign inLogin="false" inSplash="false" location="verticalDown" /></div><div class="dataContainer"><system:campaign inLogin="false" inSplash="false" location="horizontalUp" /><div><div class="fieldGroup"><div class="title"><system:label show="tooltip" label="titEjeCreEnt" /></div></div><div class="fieldGroup"><div class="field required"><label title="<system:label show="tooltip" label="lblEjeTipEnt" />"><system:label show="text" label="lblEjeTipEnt" />:</label><%
							if (dBean.isGlobalAdministration()) { %><select name="busEntId" id="cmbBusEntId" class="validate['required']"><option></option><%  
					   		...
					   		} else { %><input type="hidden" name="busEntId" id="cmbBusEntId" value="<%=BeanUtils.fmtInt(dBean.getBusEntityVo().getBusEntId())%>" busEntInstId="<%=dBean.getBusEntInstId()%>" proType="<%=BeanUtils.fmtStr(dBean.getBusEntityVo().getBusEntAdminType())%>"><b><%=BeanUtils.fmtHTML(dBean.getBusEntityVo().getBusEntTitle())%></b><% } %></div><div class="field hidden" id="processes"><label title="<system:label show="tooltip" label="lblEjeNomPro" />"><system:label show="text" label="lblEjeNomPro" />:</label><select id="selPro" name="txtProId"></select></div></div></div><system:campaign inLogin="false" inSplash="false" location="horizontalDown" /></div></form></div><%@include file="../../includes/footer.jsp" %></body></html>
