	...
	$('btnConf').addEvent("click", function(e) {
		...
		if(!$('processes').hasClass('hidden') && selPro.value!="" ){
			...
			var url = (busEntInstId == null) ? 
					'apia.execution.TaskAction.run?action=startCreationProcess&busEntId=' + cmbBusEntId.value + '&proId=' + selPro.value + TAB_ID_REQUEST
				:
					'apia.execution.TaskAction.run?action=startAlterationProcess&busEntInstId=' + busEntInstId + '&busEntId=' + cmbBusEntId.value + '&proId=' + selPro.value + TAB_ID_REQUEST;
...
//			$('selPro').addClass("validate['required']");
//			$('frmData').formChecker.register($('selPro'), 3);
			
		} else {
			$('processes').removeClass('required');
//			$('selPro').removeClass("validate['required']");
//			$('frmData').formChecker.dispose($('selPro'));
		}
