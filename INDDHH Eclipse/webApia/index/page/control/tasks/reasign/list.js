function initPage(){
	...
	$('fecAdqStartFilter').setFilter = setFilter;
	$('fecAdqStartFilter').addEvent("change", function(e) {
		this.setFilter();
	});
	$('fecAdqEndFilter').setFilter = setFilter;
	$('fecAdqEndFilter').addEvent("change", function(e) {
		this.setFilter();
	});
	...
	$('fecFinStartFilter').setFilter = setFilter;
	$('fecFinStartFilter').addEvent("change", function(e) {
		this.setFilter();
	});
	$('fecFinEndFilter').setFilter = setFilter;
	$('fecFinEndFilter').addEvent("change", function(e) {
		this.setFilter();
	});
...
function startReasign(e,param){
	...
	if(selectionCount($('tableData')) == 0) {
		...
		if(PRIMARY_SEPARATOR_IN_BODY) {
			...
			new Request({
				method: 'post',
				url: CONTEXT + URL_REQUEST_AJAX + '?action=startReasig&isAjax=true&id=' + ids + TAB_ID_REQUEST + param,
				onRequest: function() { SYS_PANELS.showLoading(); },
				onComplete: function(resText, resXml) { modalProcessXml(resXml); }
			}).send();
		}
