...
function initFormMdlPage(){
	...
	$('tableDataForm').addEvent("dblclick", function(e) {
		if (mdlFormContainer.onModalConfirm) jsCaller(mdlFormContainer.onModalConfirm,getSelectedRows($('tableDataForm')));
		closeFormModal();
	});
	...
	['orderByNameFormMdl'].each(function(ele){
		$(ele).addEvent("click", function(e) {
			e.stop();
			currentPrefix = 'Form';
			callNavigateOrder(this.getAttribute('sortBy'),this,'/apia.modals.FormsAction.run','Form');
		});
	});
	
	window.sp_Form = new Spinner($('tableDataForm'), {message: WAIT_A_SECOND});
