...
function initDashboardsMdlPage(){
	...
	['orderByNameDshMdl'].each(function(ele){
		ele = $(ele);
		ele.tooltip(GNR_ORDER_BY + ele.getAttribute("name"), { mode: 'auto', width: 100, hook: 0 })
	});
	...
	$('tableDataDsh').addEvent("dblclick", function(e) {
		if (mdlDshContainer.onModalConfirm) jsCaller(mdlDshContainer.onModalConfirm,getSelectedRows($('tableDataDsh')));
		closeDashboardsModal();
	});
	...
	['orderByNameDshMdl'].each(function(ele){
		$(ele).addEvent("click", function(e) {
			e.stop();
			callNavigateOrder(this.getAttribute('sortBy'),this,'/apia.modals.DashboardsAction.run','Dsh');
		});
	});
	
	window.sp_Dsh = new Spinner($('tableDataDsh'), {message: WAIT_A_SECOND});
