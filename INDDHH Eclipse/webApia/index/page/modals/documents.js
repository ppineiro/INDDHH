...
function initDocumentMdlPage(prefix) {
	...
	if (cmbDocType){
		cmbDocType.addEvent("change",function(e){
			...
			docType.free = toBoolean(this.options[this.selectedIndex].getAttribute("free"));
			...
			if(PRIMARY_SEPARATOR_IN_BODY) {
				new Request({
					method: 'post',
					url: CONTEXT + url + '?action=reloadMetadata&isAjax=true&docTypeId=' + docType.id + TAB_ID_REQUEST + ((extra && extra.params) ? '&' + extra.params : '') + "&prefix=" + $('mdlDocumentContainer').getAttribute("prefix"),
				...
				new Request({
					method: 'post',
					url: CONTEXT + url + '?action=reloadMetadata&isAjax=true&docTypeId=' + docType.id + '&metadata=' + strMetadata + TAB_ID_REQUEST + ((extra && extra.params) ? '&' + extra.params : '') + "&prefix=" + $('mdlDocumentContainer').getAttribute("prefix"),
					onRequest: function() { SYS_PANELS.showLoading(); },
					onComplete: function(resText, resXml) { processXmlMetadata(resXml); }
				}).send();
			}
	...
	if (btnConfirmDocumentModal) {
		...
		btnConfirmDocumentModal.callUpdateStatus = function() {
			...
			new Request({
				method: 'post',
				url: CONTEXT + url + "?action=ajaxUploadFileStatus" + extra_params + "&isAjax=true" + TAB_ID_REQUEST + "&prefix=" + $('mdlDocumentContainer').getAttribute("prefix"),
		...
		btnConfirmDocumentModal.addEvent('click', function(evt){
			...
			if(PRIMARY_SEPARATOR_IN_BODY) {
				...
				frmModalDocumentUpload.action = CONTEXT + url + "?action=ajaxUploadFile" + extra_params + "&isAjax=true" + TAB_ID_REQUEST + "&prefix=" + $('mdlDocumentContainer').getAttribute("prefix");
				...
				$('frmModalDocumentUpload').action = CONTEXT + url + "?action=ajaxUploadFile" + extra_params + "&isAjax=true" + TAB_ID_REQUEST + "&prefix=" + $('mdlDocumentContainer').getAttribute("prefix") + "&txtMetadata=" + txtMetadata + "&txtFreeMetadata=" + txtFreeMetadata;
			}
...
function docProcessModalReturn(id, text, canUpdate, pool) {
	var mdlDocumentContainer = $('mdlDocumentContainer');
	...
	var span = new Element("span", {'class': 'option optionRemove'});
	...
	span.idCheckIn = id;
	span.addEvent('click', function(evt) {this.checkIn.erase(this.idCheckIn); this.destroy(); });
	
	var divider = $('mdlDocumentPoolContainterDivider');
	span.inject(divider, 'before');
}


function showDocumentsModal(retFunction, docId, extra, docInfo, hidePermissiosn, hideMetadata){
	table = $('tableMetadata');
	addNewDoc = true;
	...
	if(hideMetadata)
		$('metadata').setStyle('display', 'none').set('hideMetadata', 'true');
	else
		$('metadata').setStyle('display', '').set('hideMetadata', 'false');
	
	$('documentModalDocFile').value = '';
	$('documentModalDocDesc').value = '';
	$('selDocAllPoolPerm').selectedIndex = 0;
	$('mdlDocumentPoolContainter').getElements("span.optionRemove").each(function(ele) { ele.dispose(); });
	...
	if (docId == ""){ //subir archivo nuevo
		...
		$('spanDocType').innerHTML = docInfo.docTypeTitle;
	...
	$('mdlDocumentContainer').setAttribute("prefix",extra.prefix);
...
function showDocumentInfo(docInfo, extra, hideMetadata){
	...
	if (xmlGenerals != null) {
		for (var i = 0; i < xmlGenerals.length; i++) {
			...
			if(locked=='true'){
				...
				$('lck'+docId+prefix).tooltip(locked, {
					mode : 'auto',
					width : 100,
					hook : false
				});
...
function populateDocInfo(){
	...
	if (xmlGenerals != null) {
		for (var i = 0; i < xmlGenerals.length; i++) {
			...
			var prefix = xmlGeneral.getAttribute("prefix");
			var docTypeTitle = xmlGeneral.getAttribute("docTypeTitle");
		 
			$('lblNom').innerHTML = docName;
			$('docDesc').innerHTML = docDesc;
			$('docTypeTitle').innerHTML = docTypeTitle;
		}
	}
	...
	if (xmlVersions != null) {
		...
		for (var i = 0; i < xmlVersions.length; i++) {
			...
			var downloadIcon = new Element('a', {html: verNumber, 'href':CONTEXT + URL_REQUEST_AJAX + "?action=downloadDocument&docId=" +  docId + "&version="+ verNumber +"&prefix=" + prefix + TAB_ID_REQUEST});
...
function fncDocumentLoadInformation() { 
	...
	//var xmlPermisions = ajaxCallXml.getChildren('permisions')[0];
	//var xmlPermisions = ajaxCallXml.childNodes[1]; //IE Friendly
	//var xmlPermisions = Browser.ie ? ajaxCallXml.getElementsByTagName('permisions') : ajaxCallXml.getChildren('permisions'); //IE Friendly
	var xmlPermisions = ajaxCallXml.getElementsByTagName('permisions'); //IE Friendly
	
	if (xmlPermisions != null && xmlPermisions.length) {
		
		xmlPermisions = xmlPermisions[0];
		
		//var xmlUsers = Browser.ie ? xmlPermisions.getElementsByTagName('user') : xmlPermisions.getChildren('user');
		var xmlUsers = xmlPermisions.getElementsByTagName('user');
		//var xmlPools = Browser.ie ? xmlPermisions.getElementsByTagName('pool') : xmlPermisions.getChildren('pool');
		var xmlPools = xmlPermisions.getElementsByTagName('pool');
		
		if (xmlUsers != null) {
			for (var i = 0; i < xmlUsers.length; i++) {
				var xml = xmlUsers[i];
				//var permType = xml.getProperty("permType");
				var permType = xml.getAttribute("permType");
				//var id = xml.getProperty("id");
				var id = xml.getAttribute("id");
				//var name = xml.getProperty("name");
				var name = xml.getAttribute("name");
				
				docProcessModalReturn(id, name, permType, false);
			}
		}
...
function processXmlMetadata(resXml,fromShow){
	...
	if (resXml){
		...
		if(_metadata.get('hideMetadata') == 'true') {
			if(hasRequired) 
...
function processXmlDocTypes(resXml){
	...
	for (var i = 0; i < docTypes.length; i++){
		...
		var option = new Option(xmlDocType.getAttribute("title"),xmlDocType.getAttribute("id"));
		option.setAttribute("free",toBoolean(xmlDocType.getAttribute("free")));
	...
	if (newDoc){
		...
		docType.free = toBoolean(cmbDocType.options[cmbDocType.selectedIndex].getAttribute("free"));
