...
function initEnvMdlPage(){
	...
	['orderByNameEnvMdl','orderByDescEnvMdl'].each(function(ele){
		ele = $(ele);
		ele.tooltip(GNR_ORDER_BY + ele.getAttribute("title"), { mode: 'auto', width: 100, hook: 0 })
	});
	
	['nameFilterEnvMdl','descFilterEnvMdl'].each(function(ele) {
		ele = $(ele);
	...
	$('clearFiltersEnv').addEvent("click", function(e) {
		e.stop();
		['nameFilterEnvMdl','descFilterEnvMdl'].each(clearFilter);		
	...
	['orderByNameEnvMdl','orderByDescEnvMdl'].each(function(ele){
		$(ele).addEvent("click", function(e) {
			e.stop();
			callNavigateOrder(this.getAttribute('sortBy'),this,'/apia.modals.EnvironmentsAction.run','Env');
		});
	});
	
	window.sp_Env = new Spinner($('tableDataEnv'), {message: WAIT_A_SECOND});
