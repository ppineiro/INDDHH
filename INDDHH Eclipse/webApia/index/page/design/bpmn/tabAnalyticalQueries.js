...
function initTabAnalyticalQueries(){	
	...
	//eventos para el tab
    $('tabAnalyticalQueries').addEvent("focus", function(evt){
    	isShowing = true;
...
function executeBeforeConfirmTabAnalyticalQueries(){
	...
	if ($('cubeChanged').value == "true"){ //Hubieron cambios en el cubo
		if (forceConfirm || verifyCubeData()){
			...
			values = "";
			$('profilesCubeContainer').getElements("div.optionRemove").each(function (prof){
				if (values != "") values += ";";
				values += prof.getAttribute("profId");				
			});
			$('txtProfCube').value = values;
			values = "";
			$('profilesCubePermContainer').getElements("div.optionRemove").each(function (prof){
				if (values != "") values += ";";
				values += prof.getAttribute("profId");
...
function confirmDeleteCube(){
	...
	$('profilesCubeContainer').getElements("div.optionRemove").each(function (option){ option.destroy(); })
	$('profilesCubePermContainer').getElements("div.optionRemove").each(function (option){ option.destroy(); })
...
function processXmlDimRestricted(resXml){
	...
	var dimensions = result.getElements("dimension");
...
function loadDimensions(){
	var request = new Request({
		...
		onRequest : function() { },
		onComplete : function(resText, resXml) { processXmlDimensions(resXml,true); }
	}).send();
}
...
function createDimension(dimProDwColId,attId,attType,attName,attFrom,dwName,allMemberName,date,mapEntityId,mapEntityName){
	...
	var div32 = new Element("div",{styles:{'width':'80%','display':'inline','float':'rigth'}}).inject(div3);
...
function processStartAddDimension(resXml){
	...
	if (result != null && result.length > 0 && result.item(0) != null){
		...
		panelAddDim.content.addClass("modalOptionsContainer");
		...
		for(var i = 0; i < result.length; i++){
			...
			var span = new Element("span",{html:name,'title':tooltip}).inject(li);
...
function processXmlAddInfoModal(resXml){
	...
	if (result != null && result.length > 0 && result.item(0) != null){
		...
		for (i = 0; i < lis.length; i++){
			...
			var newLi = new Element("li",{styles:{'line-height':'20px !important'}}).inject(objUl);
			...
			var span = new Element("span",{html:label,'title': hasTooltip ? tooltip : label}).inject(newLi);
...
function loadMeasures(){
	var request = new Request({
		...
		onRequest : function() { },
		onComplete : function(resText, resXml) { processXmlMeasures(resXml,true); }
	}).send();
}
...
function loadProfiles(){
	var request = new Request({
		...
		onRequest : function() { },
		onComplete : function(resText, resXml) { processXmlProfiles(resXml); }
	}).send();
}
...
function createProfile(profId,profName,profDesc,allEnv,fromModal){
	...
	if (!$(prefix+profId)){
		var prof = new Element("div",{'class': 'option optionRemove optionTextOverflow optionWidth40', html: (allEnv?"<br>":"")+profName+(allEnv?"</br>":""), 'id': prefix+profId}).inject(before,"before");
		...
		if (!fromProfCube){
			...
			profDim.tooltip(RESTRIC, { mode: 'auto', width: 140, hook: false });
