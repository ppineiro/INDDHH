var kpiTypeSelected; //Usada para volver para atras el cambio de tipo de kpi cuando el usuario selecciona balanza 
						// y no desea perder las zonas, cancelando la acción de cambio de kpi
...
function initGenDataTab(mode) {
	...
	if (mode==INSERT_MODE) {
		...
		$('titIndicatorSample').set('html', LBL_TITLE);
	...
	if (cmbKpiValueType){
		cmbKpiValueType.addEvent("change", function () {
			if (cmbKpiValueType.get('value') == 0) {
				...
				if (cmbKpiValueType.get('value')==2){ //Si eligió %
	...
	//Seteamos la acción de boton agregar zonas
	...
	//Seteamos la acción de boton eliminar info de la grilla de zonas
	...
	$('widTitle').addEvent('change', function() {
		$('titIndicatorSample').set('html', this.get('value'));
	});
...
function selectGaugeVelocimeterKPIType() {
	...
	showChart(); //Es necesario que este aquí, no sacar para afuera
}
...
function showChart(backColor, pointerColor, valueColor) {	
	...
	var value = getValueToShow(); //Según la zona seleccionada
	...
	if (indType == WIDGET_KPI_TYPE_THERMOMETER_ID && $('gridZones').rows.length != 3){
		...
		try {
			$('ifrIndicatorSample').set('src', 'page/design/widgets/indicatorSample.jsp?indType=' + indType + '&minValue=' + $('txtKpiMin').get('value') + '&maxValue=' + $('txtKpiMax').get('value') + '&valueFontSize='+ $('txtKpiValueFontSize').get('value')+ '&scaleFontSize=' + $('txtKpiScaleFontSize').get('value') + '&backgColor='+ getRGBColor(backColor) +'&pointerColor='+ getRGBColor(pointerColor) + '&valueColor='+ getRGBColor(valueColor)+'&valueType=' + $('cmbKpiValueType').get('value') +'&valueDecimals=' + $('cmbKpiValueDecimals').get('value') + '&withBorder=' + $('chkSeeBorder').get('checked') + '&zones=' + zones + '&actualValue=' + value);
		}catch(e){
...
function loadZones() {
	var request = new Request({
		...
		onComplete: function(resText, resXml) {loadZonesXML(resXml);  }
	}).send();
}
...
function loadWidgetPropertiesXML(ajaxCallXml){
	if (ajaxCallXml != null) {
		...
		//Según si el widget esta customizado o no mostramos las propiedades o las ocultamos
		chkPersonalize();
		
		$('titIndicatorSample').set('html', $('widTitle').get('value'));
...
function fncAddZone (name, desc, min, max, color, transp) {
	...
	if (name==null) fncZoneChanged(); //Si se agregó una zona, marcamos como que hubó un cambio en las zonas
	...
	var input = new Element('input',{type:'text', name:'zneMin'});
	...
	var input = new Element('input',{type:'text', name:'zneMax'});
	var kpiMax = $('txtKpiMax');
