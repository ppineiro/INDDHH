...
function loadProcess(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX +'?action=loadProcess' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { processXMLProcess(resXml); sp.hide(true); }
	}).send();
}
...
function addActionElementProcess(container, text, id,father,helper,addRemove){
	...
	elemDiv.addEvent('click', function(e){
		e.stop();		
		proToDelete = "";
...
function validDiv(item){
	return !item.hasClass("optionIcon") && item.id!="addProcess" && !item.hasClass("optionAdd");
}
...
function existOnDiv(proId){
	$('processContainer').getElements("DIV").each(function(item,index){		
			if (validDiv(item)){
...
function getRealFathers(fathersIds){
	...
	trows=$('processContainer').getElements("DIV");
...
function reLoadComboProcess(){
	var proRows=$('processContainer').getElements("DIV");
...
function refreshPools(){
	...
	var request = new Request({
		method: 'post',		
		url: CONTEXT + URL_REQUEST_AJAX +'?action=refreshPools' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
...
function addAjaxProcess(id){
	var request = new Request({
		method: 'post',
		data:{'id':id},
		url: CONTEXT + URL_REQUEST_AJAX +'?action=addProToBean' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { modalProcessXml(resXml); sp.hide(true); }
	}).send();
}
...
function loadPools(){
	...
	var request = new Request({
		method: 'post',		
		url: CONTEXT + URL_REQUEST_AJAX +'?action=loadPools' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { modalProcessXml(resXml); sp.hide(true); }
	}).send();
}
...
function changeGenType(val){
	...
	if (val == GEN_DISABLED){//dont start
		...
		$("tdTxtPar1").removeClass("required");
...
function verifyOtherReqObjects(){
	...
	if ($("useHistChk").checked){ //Si es clickeado usar hist�rico	
		if (isGreater($("txtHistFrecDatIni").value, $("txtHistFrecDatEnd").value)){		
			...
			return false;
		}else if ($("txtHistFrecDatIni").value == $("txtHistFrecDatEnd").value){
	...
	var trows = $('processContainer').getElements("DIV");
	var found = false;
	for (i=0;(!found && i<trows.length);i++) {
		var item = trows[i];
		if (validDiv(item)){
