	...
	if (objPanel){
		...
		if (parameters != null && parameters.length > 0){
			...
			for (var i = 0; i < parameters.length; i++){
				...
				var div3 = new Element('div', {styles: {width: '260px', overflow: 'hidden', 'white-space': 'pre', 'margin': '4px 0px'}}).inject(td3);
				...
				if (param.viewType == PNL_PARAM_VIEW_TYPE_CHECKBOX){
					...
					if (arrPosVal != null && arrPosVal.length > 0){
						for (var j = 0; j < arrPosVal.length; j++){
							var pv = arrPosVal[j];
							var div = new Element("div.option.optionRemove",{html: pv.text}).inject(div3);
					...
					var divBut = new Element("div.option.optionAdd",{html: LBL_ADD, styles:{'height':'20px'}}).inject(div3)
					divBut.addEvent("click",function(e){
						e.stop();
						...
						} else if (param.viewType == PNL_PARAM_VIEW_TYPE_MDL_TSK){
				...
				tr.getContentStr = function (){ //format: [pnlParamId,value]
					...
					if (viewType == PNL_PARAM_VIEW_TYPE_COMBOBOX){
						inputValue = this.getElements("td")[2].getElements("div")[0].getElements("select")[0];
								...
								viewType == PNL_PARAM_VIEW_TYPE_MDL_TSK || param.viewType == PNL_PARAM_VIEW_TYPE_MDL_DSH){
						...
						mdlValues.each(function(mdlVal){
							...
							strNewValue += mdlVal.getAttribute("strId");
						});
				...
				tr.getPosValuesForModal = function (){ //retorna array
					...
					this.getElements("td")[2].getElements("div")[0].getElements("div.optionRemove").each(function(posVal){
						var obj = {
									value : posVal.getAttribute("strId"),
									text : posVal.getAttribute("strText")
							     }
						ret.push(obj)
					});
					return ret;
	...
	if (ret){
		for (var i = 0; i < ret.length; i++){
			...
			for (var j = 0; j < all.length && !exist; j++){
				exist = all[j].getAttribute("strId") == ret[i].getRowId();
			}
			if (!exist){
				var content = ret[i].getRowContent();
				var div = new Element("div.option.optionRemove",{html: content[0]}).inject(containerForModal.getElement("div.optionAdd"),"before");
...
function createPool(poolId, poolName){
	var obj = new Element("div.option.optionRemove.poolCanViewClass",{html:poolName,'id':'p_'+poolId});
	obj.setAttribute("poolId",poolId);
	obj.setAttribute("poolName",poolName);
