	...
	if (sections != null && sections.length > 0 && sections.item(0) != null) {
		...
		for(var i = 0; i < sections.length; i++){
			...
			if (tr == null){
				...
				/* td Section */
				var tdSection = new Element("td", { html: xmlSection.getAttribute("name"), 'styles': {width: tdWidths[0]} } );
				tdSection.inject(tr);
...
function createPanel(xmlPanel){
	/* Panel */
	var objPanel = new Element("div", {'class': 'option optionRemoveTD optionWidth30 forceAlignOption', 'title': xmlPanel.getAttribute("name") });
	objPanel.setAttribute("pnlId",xmlPanel.getAttribute("id"));
	objPanel.setAttribute("pnlName",xmlPanel.getAttribute("name"));
	objPanel.setAttribute("pnlSection",xmlPanel.getAttribute("section"));
	objPanel.setAttribute("pnlPosition",xmlPanel.getAttribute("position"));
	objPanel.setAttribute("pnlOpRemove",xmlPanel.getAttribute("remove"));
	objPanel.setAttribute("pnlOpVisible",xmlPanel.getAttribute("visible"));
	objPanel.setAttribute("pnlOpEdit",xmlPanel.getAttribute("edit"));
	objPanel.setAttribute("tempId",xmlPanel.getAttribute("tempId"));

	objPanel.addClass("panel_"+xmlPanel.getAttribute("tempId"));

	new Element("div",{'class':'text', html: xmlPanel.getAttribute("name")}).inject(objPanel);

	//Remover
	objPanel.addEvent('click', function(evt) { removePanel(this); evt.stopPropagation(); });	

	//Opciones
	var opts = new Element("div", {'class': 'optionIcon optionModify'}).inject(objPanel);
	opts.addEvent('click', function(evt) { 
		var section = this.getParent().getAttribute('pnlSection');
	...
	if (parameters != null && parameters.length > 0 && parameters.item(0) != null) {
		...
		for (var i = 0; i < parameters.length; i++){
			...
			var param = { 	id : xmlParam.getAttribute("id"),
...
function updatePanel(objPanel){
	if (objPanel){
		...
		var flags = (objPanel.getAttribute("pnlOpEdit")=="true"?"1":"0") + (objPanel.getAttribute("pnlOpVisible")=="true"?"1":"0") + (objPanel.getAttribute("pnlOpRemove")=="true"?"1":"0");
...
function loadDshPnlResponsive(){
	var nowResp = $('flagLand').checked && $('flagResp').checked; 
	var request = new Request({
		method: 'post',
...
function processXMLDshPnlResponsive(resXml){
	...
	if (responsive != null && responsive.length > 0 && responsive.item(0) != null) {
		...
		for (var i = 0; i < responsive.length; i++){
			...
			createDshPnlRespOrd(resp.getAttribute('id'), resp.getAttribute('name'));
		}
...
function createDshPnlRespOrd(id,name){
	if (!$('dshPnlResp_'+id)){
