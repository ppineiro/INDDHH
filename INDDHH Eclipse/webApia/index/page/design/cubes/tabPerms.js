...
function initPermissionsTab(mode) {
	...
	//Configuramos acción del botón de agregar perfiles con acceso al cubo en modo navegador
	if ($('divAddProfileNav')) {
	...
	//Configuramos acción del botón de agregar perfiles en modo visualizador
...
//Agrega un perfil en el container de perfiles con acceso restringido en alguna dimensión en modo visualizador
	...
	if(addRemove){
		...
		divElement.addEvent("click", function(e){
			var request = new Request({
				method: 'post',
				data:{'prfName':text},
				url: CONTEXT + URL_REQUEST_AJAX +'?action=removePrfRestrictions' + TAB_ID_REQUEST,				
				onRequest: function() { sp.show(true); },
				onComplete: function(resText, resXml) { 
					modalProcessXml(resXml);
					sp.hide(true);
...
//Carga los permisos con acceso de edición sobre el cubo
