...
function initMeasuresTab(mode) {
	...
	if (radFactTable1){
		//Al presionar el botón Agregar se recupera un string con nombre de tablas separdos por , y se procesan en el metodo processXMLmodalAddTables()
...
function canChangeFactTable(rad) {
	...
	if (rad==1){ //Ahora se selecciono usar tabla
		...
		$('btnTest').set('disabled','disabled'); //Deshabilitamos el botón de testear la vista
		...
		$('btnTest').erase('disabled'); //Habilitamos el botón de testear la vista 

	}else { //La función no se llamo al hacer click en un radioButton (se llamó al iniciar)
	...
	if (changed) {
		var res = confirm("Si modifica la tabla de hechos seleccionada, se perderán todas las medidas actuales. ¿Desea continuar?");
		if (res){
...
function testSQL(onConfirm, afterAction) {
	...
	var selConn = $('selConn').value; //Conexión seleccionada en este momento
...
function fncAddMeasure(column, measureName, measureNameToShow, typeMeasure, funcion, format, visible){
	...
	var oTd4 = new Element("TD"); //función
	...
	//if (typeMeasure=='standard') {
		...
		if (selType == "S" || selType == "D"){
			...
			if (funcion!=null && funcion == 'DIST. COUNT') optionDOM.set('selected', 'selected');
