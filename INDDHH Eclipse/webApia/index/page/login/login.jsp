...
%>
<%if (!Parameters.LOGIN_USE_DASHBOARD || forceClassicLogin || externalMode) {%> <!-- LOGIN CLASICO -->
	<%@include file="classic/login.jsp" %>
<%} else { %> <!-- LOGIN DASHBOARD -->
	<%
		ThreadData.setMobileAccess(new Boolean(false));
	...
	%>
	<%if (forceMobile){ %>
		<%@include file="portalToMobileVersionRedirect.jsp" %>
	<%} else { %>
		<%if (StringUtil.isEmpty(request.getParameter("tokenId"))){ %>
			<%@include file="portalRedirect.jsp" %>
		<%} else { %>
