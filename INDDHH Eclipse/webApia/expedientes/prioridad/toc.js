...
function showCursorWait() {
	...
	var parent = window.content ? window.content.parent : window.parent; 
	
	if (parent.frames.length >= 4) {
		parent.document.getElementById("iframeMessages").showResultFrame(document.body);
	} else {
		parent.parent.document.getElementById("iframeMessages").showResultFrame(document.body);
	}
}

function hideCursorWait() {
	//document.body.style.cursor = 'default';
	var parent = window.content ? window.content.parent : window.parent; 
	
	if (parent.frames.length >= 4) {
		parent.document.getElementById("iframeMessages").hideResultFrame(document.body);
	} else {
		parent.parent.document.getElementById("iframeMessages").hideResultFrame(document.body);
	}
}


//NUEVO
function desChequearTodos(ele, flag, tipo){	
	...
	for (var i=1;i<iNumElems;i++) {
		if (cElems[i].id != null && cElems[i].id.indexOf('checkbox') != -1 ){			
	...
	//alert(tipo);
	if (tipo == 'OE'){
		document.getElementById("organismoDestino").value=ele.value;
		document.getElementById("textNombOrganismoDestino").value=ele.name;
		document.getElementById("textMesaDestino").value= ""; 
		document.getElementById("textIdMesaDestino").value= ""; 
...
function desChequearMesa(ele){	
	...
	ele.checked=true;
	document.getElementById("textIdMesaDestino").value=ele.name;
	document.getElementById("textMesaDestino").value= ele.value; 
	//alert(document.getElementById("textMesaDestino").value);
	//alert(document.getElementById("textIdMesaDestino").value);
