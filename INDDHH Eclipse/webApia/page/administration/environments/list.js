function initPage(){
	...
	['orderByName','orderByTitle','orderByDesc','orderByLabel','orderByRegUser','orderByRegDate'].each(setAdmOrder);
	['orderByName','orderByTitle','orderByDesc','orderByLabel','orderByRegUser','orderByRegDate'].each(setAdmListTitle);
	['nameFilter','titleFilter','descFilter','orderByLabel','regUsrFilter','regUsrFilter'].each(setAdmFilters);
	['regDateFilter'].each(setDateFilters);
	
	$('clearFilters').addEvent("click", function(e) {
		if(e) e.stop();
		['nameFilter','titleFilter','descFilter','labelFilter','regUsrFilter'].each(clearFilter);
...
function processXMLInitEnvParam(){
	...
	if (ajaxCallXml != null && ajaxCallXml.getElementsByTagName("environment")) {
		...
		var tabTitle = TITLE_TAB + " - " + xmlEnv.getAttribute("title") + " 3.0";
