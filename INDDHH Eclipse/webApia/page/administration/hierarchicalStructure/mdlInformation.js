...
function showMdlInformation(poolId,poolName){
	...
	cleanTable($('tableDataEnvironments'));
	cleanTable($('tableDataUsers'));
...
function processXmlEnvironments(resXml){
	var tableEnvironments = $('tableDataEnvironments');
	...
	if (environments != null && environments.length > 0 && environments.item(0) != null) {
		...
		for (var i = 0; i < environments.length; i++){
			...
			var envTitle = xmlEnv.getAttribute("title");
			...
			var div = new Element('div', {html: envTitle, styles: {width: '95%', overflow: 'hidden', 'white-space': 'pre', 'margin': '4px 0px'}}).inject(td);				
		}
