...
function initPage(){
	...
	$('btnConf').addEvent("click", function(e) {
		...
		var request = new Request({
			...
			url: CONTEXT + '/apia.administration.UsersAction.run?action=confClone&isAjax=true',
	...
	$('btnRea').addEvent("click", function(e) {
		...
		var request = new Request({
			...
			url: CONTEXT + '/apia.administration.UsersAction.run?action=confReactivateClone&isAjax=true',
			onRequest: function() { sp.show(true); },
			onComplete: function(resText, resXml) { processXMLResponse(resXml); sp.hide(true); }
		}).send();
	});
