...
function loadEnvs(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX + '?action=getEnvironments&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { 
			processXMLEnvs(resXml); sp.hide(true);
			
			initAdminModalHandlerOnChangeHighlight($('envContainter'));
		}
	}).send();
}
...
function addEnvironmentToContainer(text, id) {
	...
	element.getElement('div.optionRemove').removeEvent('click', actionElementAdminClickRemove).addEvent('click', function(evt) {
		...
		if ($('envContainter').getElements("DIV").length == 3)
			$('envContainterAll').show();
	});
