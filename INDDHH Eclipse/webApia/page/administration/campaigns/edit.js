function initPage(){
	...
	initAdminFieldOnChangeHighlight(null, null, null, document.getElement('#cmpName').getParent('.aTab'));
	...
	cmpSource.addEvent('change', function(evt){
		if (this.selectedIndex == 0) {
			...
			$('cmpSrcImageLabel').removeClass("hidden");
			...
			$('cmpSrcImageLabel').addClass("hidden");
	...
	var cmpSrcImage = $('cmpSrcImage');
	if (cmpSrcImage){
		cmpSrcImage.addEvent("change",function(){
...
function addNewIpElement(ipFrom, ipTo) {
	var add = $('addIp');
	var newIP = new Element("div", {'class': 'option'});
	new Element('span', {html: ipFrom + " - " + ipTo}).inject(newIP);
	new Element('input', {type: 'hidden', name: 'cmpIpFrom', value: ipFrom}).inject(newIP);
	new Element('input', {type: 'hidden', name: 'cmpIpTo', value: ipTo}).inject(newIP);
	
	new Element('div.optionRemove').addEvent('click', function(evt) { this.getParent().destroy(); }).inject(newIP);
	
	newIP.inject(add,"before");
}
...
function addNewNodeElement(nodeName) {
	...
	if (notAdded) {
		var add = $('addNode');
		var newDay = new Element("div", {'class': 'option'});
		new Element('span', {html: nodeName}).inject(newDay);
		new Element('input', {type: 'hidden', name: 'cmpNode', value: nodeName}).inject(newDay);
		
		new Element('div.optionRemove').addEvent('click', function(evt) { this.getParent().destroy(); }).inject(newDay);
...
function addNewTimeElement(timeFrom, timeTo) {
	var add = $('addTime');
	var newTime = new Element("div", {'class': 'option'});
	...
	new Element('div.optionRemove').addEvent('click', function(evt) { this.getParent().destroy(); }).inject(newTime);
...
function loadXmlValues(){
	...
	if (ajaxCallXml != null) {
		...
		$$('.modalOptionsContainer').each(function(ele){
			if (ele.id != "weekdayContainer") initAdminModalHandlerOnChangeHighlight($(ele), null, true);
		})
		 
		var visTab = document.getElement('#cmpFlag5').getParent('.aTab');
		initAdminFieldOnChangeHighlight(true, null, null, visTab); 
		addElementsOnChangeHighlight(visTab.getElements('input[type=checkbox]'));
	}
}
