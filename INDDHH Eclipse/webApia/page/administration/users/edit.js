function initPage(){
	...
	//-se est� creando un usuario global
	...
	var imgUsr = $('imgUsrImage');
	if (imgUsr){
		imgUsr.src = CONTEXT + "/getImageServlet.run?usrLogin=" + $('usrLogin').value + "&height=24&width=24";
		imgUsr.set('initialValue', imgUsr.src);
		
		imgUsr.addEvent("change",function(){
	...
	$('addEnvUsrStyle').addEvent("click", function(e) {
		...
		if (isGlobal){
			...
			//sólo se intenta agregar ambiente actual
	...
	if(authFullLDAP){
		...
		//$('usrLongLogin').addClass("readonly");
		...
		//$('usrLongLogin').setAttribute('readOnly','readonly');
...
function loadEnvs(){
	var request = new Request({
		...
		onComplete: function(resText, resXml) { 
			processXMLUsrEnvs(resXml); sp.hide(true);
			initAdminModalHandlerOnChangeHighlight($('environmentsContainter'));
		}
	}).send();
}
...
function loadPools(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + '/apia.administration.UsersAction.run?action=getUserPools&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { 
...
function loadProfiles(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + '/apia.administration.UsersAction.run?action=getUserProfiles&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { 
			processXMLUsrProfiles(resXml); sp.hide(true);
			initAdminModalHandlerOnChangeHighlight($('profilesContainter'));
		}
	}).send();
}
...
function loadStyles(){
	var request = new Request({
		...
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { 
			processXMLUsrStyles(resXml); sp.hide(true);
			initAdminModalHandlerOnChangeHighlight($('envStylesContainter'));
		}
	}).send();
}
