<%@page import="com.dogma.controller.ThreadData"%><%@page import="com.dogma.UserData"%><%@page import="biz.statum.apia.utils.AES"%><%@page import="biz.statum.apia.web.bean.monitor.BusinessBean"%><%@page import="java.util.HashMap"%><%@page import="com.dogma.Configuration"%><%@page import="java.util.Iterator"%><%@page import="com.dogma.vo.DocumentVo"%><%@include file="../includes/startInc.jsp" %><%
...
if(basicBean instanceof biz.statum.apia.web.bean.execution.EntInstanceListBean){
	...
	eBean = ((BusinessBean)basicBean).getEntInstanceBean();
	pBean = ((BusinessBean)basicBean).getMonitorProcessesBean();
	if(pBean != null && eBean == null) {
		eBean = pBean.getEntInstanceBean();
		isMonBusEntity = false;
	}
}
...
private String processCollection(Collection docs) {
	if (docs != null) {
		...
		while (iterator.hasNext()) {
			...
			buffer.append("<li class='noCursor'>");
			buffer.append("<A href='#nowhere' onclick=\"downloadDocument('" + AES.encrypt(docVo.getDocId().toString(), ThreadData.getUserData().getTokenId()) + "')\">" + docVo.getDocName() + "</a>");
			buffer.append("</li>");
		}
...
%><html lang="<%=biz.statum.apia.web.bean.BasicBeanStatic.getUserDataStatic(request).getLangCode()%>"><head><%@include file="../includes/headInclude.jsp" %><script type="text/javascript">
	...
	function downloadDocument(docId) {
		...
		<% } else if(basicBean instanceof BusinessBean) {
			if(isMonBusEntity) {%>
			$('docFrame').src = CONTEXT + "/apia.monitor.BusinessAction.run?action=downloadDocument&prefix=E&docId=" +  docId + "<system:util show="tabIdRequest"  />";
			<%} else {%>
			$('docFrame').src = CONTEXT + "/apia.monitor.BusinessAction.run?action=downloadDocument&docId=" +  docId + "<system:util show="tabIdRequest"  />";
			<%}
		} else {%>
			alert("ver que bean esta usando para agregar el link");
		<%}%>
	}
	
	</script></head><body><iframe id="docFrame" style="display:none"></iframe><div class="body" id="bodyDiv"><div class="optionsContainer"></div><div class="dataContainer allowMobileMaxSize dataContainerFullSize"><div class="fieldGroup"><div class="title"><system:label show="text" label="sbtEjeDoc" /></div></div><% if (tBean != null || pBean!=null) {
			if(tBean!=null){
