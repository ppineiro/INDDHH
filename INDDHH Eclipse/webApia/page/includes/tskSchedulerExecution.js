var Scheduler = new Class({
	...
	generateScheduler: function(xml) { 
		...
		//Corregir alto en caso de que sea menor de 400
		if(tbody.getHeight() < 400) {
			var cant_tr = tbody.getElements('tr').getHeight().length;
			tbody.setStyle('height', cant_tr * 24);
		}
	...
	selectTD: function(event) {
		...
		if(cal.cell_selected) {
			...
			prev_tds = cal.cell_selected.getAllPrevious();
			prev_tds.removeClass('tr_selected');
			cal.cell_selected.getParent('tr').getAllPrevious().each(function(tr) {
				tr.getElements('td')[prev_tds.length].removeClass('tr_selected');
			})
		}
		
		
		event.target.addClass('td_selected');
		prev_tds = event.target.getAllPrevious();
		prev_tds.addClass('tr_selected');
		event.target.getParent('tr').getAllPrevious().each(function(tr) {
			tr.getElements('td')[prev_tds.length].addClass('tr_selected');
		})
...
TskScheduler.loadSchedExec = function(schedDiv, schId, proId, proVerId, tskId, showDisponibility) {
	...
	TskScheduler.schedDiv = $(schedDiv).setStyle('display', 'none').set('html', 
		'<div id="toolsDiv">' +
			'<div align="center">' +
				'<div class="schedNavButton" id="btnGoToToday">' + LBL_TODAY + '</div>' +
				'<div class="schedNavButton" id="btnGoToPrev">&lt;</div>' +
				'<input id="navDate" type="text" style="border: 0px; width: 0px; height: 0px; visibility: hidden;"/>' +			
				'<div type="button" class="schedNavButton" id="navBtn"></div>' +
				'<div class="schedNavButton" id="btnGoToNext">&gt;</div>' +
			'</div>' +
		'</div>' +
		'<div id="schedulerDiv">' +
			'<div id="schedTableContainer">' +
			'</div>' +
		'</div>');
	...
	//DatePicker para navegaci�n de la agenda
...
TskScheduler.getSched = function(weekDay) {
	...
	var request = new Request({
		method: 'post',
		url: CONTEXT + '/apia.design.TaskSchedulerAction.run?action=getExecutionSchedulerWeek&isAjax=true&weekDay=' + weekDay + '&schId=' + TskScheduler.schId + '&proId=' + TskScheduler.proId +'&proVerId=' + TskScheduler.proVerId +'&tskId=' + TskScheduler.tskId + TAB_ID_REQUEST,
		onRequest: function() {
			calSpinner.show();
		},
