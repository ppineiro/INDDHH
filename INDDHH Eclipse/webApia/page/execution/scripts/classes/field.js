...
var Field = new Class({
	...
	parseXMLposition: function(auxContainer) {
		...
		if(this.content == null) {
			...
			var maxWidth = this.cols == this.form.cols;
			if(this.rows > 0 && !maxWidth) 
	...
	parsePrintXMLposition: function(auxContainer) {
		...
		if(!td) throw new Error('No se encontró celda para el campo');
	...
	/**
	 * Elimina los td's y tr's innecesarios del formulario.
	 */
			...
			for(var r = 0; r < this.rows; r++) {
				if(this.cols > 0) {
					for(var c = 0; c < this.cols; c++) {
						...
						tr = td_aux.parentNode; tr.removeChild(td_aux);
						if (tr.childElementCount==0) { tr.dispose(); }
	...
	addTranslationIcon: function(input, icon_container) {
		...
		if(this.options.disabled == true || this.options.readonly == true || this.options.inputAsText == true) return; 
...
Field.PROPERTY_ALLOW_EDITION			= "allowEdition";
...
Field.PROPERTY_SIGNATURE_REQUIRED		= "reqSign";
Field.PROPERTY_SHOW_TOOLTIP_AS_HELP		= "tooltipHelp";
