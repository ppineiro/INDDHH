...
//				});
			...
			if(this.editionTable) {
				...
				var edit_button = new Element('div.editable-row', {tabindex:''}).inject(this.editionTable).addEvents({
					...
					click: function(e) {
						var modal = ModalController.openWinModal(CONTEXT + '/apia.execution.FormAction.run?action=editGrid' + '&frmName=' + this_grid.options[Field.PROPERTY_GRID_FORM] + '&frmParent=' + this_grid.frmId.split('_')[0] + '&editionMode=true' + (this_grid.options[Field.PROPERTY_READONLY] ? '&ro=true' : '') + '&index=' + edition_index + TAB_ID_REQUEST, 640, 600, null, null, true, false, true);
...
//					rowHeights = this.divBody.getElements('tr').getStyle('height');
		...
		var tableHeader = new Element('table', {title: this.options[Field.PROPERTY_GRID_TITLE]}).inject(divHeader);
		...
		var tableBody = new Element('table', {title: this.options[Field.PROPERTY_GRID_TITLE]}).inject(divBody);
		...
		if(this.xml.childNodes[0].nodeName == Grid.tags.gridHeader) {
			Array.from(this.xml.childNodes[0].childNodes).each(function(col_xml, col_index) {
				if(col_xml.nodeName == Grid.tags.gridHeaderCol) {
					...
					var fieldType, properties;
					if (this.xml.childNodes[col_index + 1]){
						fieldType = this.xml.childNodes[col_index + 1].getAttribute('fieldType')
						properties = this.xml.childNodes[col_index + 1].getAttribute('properties')
					}
					
					var gridColHeader = new GridHeader(th, col_title, col_tooltip,fieldType, JSON.decode(properties), fn_col_select, this.exec_ajax_call);
...
//			if(i == 0)
		...
		if(this.options[Field.PROPERTY_PAGED_GRID]) {
			...
			var nav_buttons = new Element('div.navButtons').set('html',
					...
					'<div class="pGroup" id="navigator"><input class="navBarCurrentPage" type="text" value="' + this.currentPage + '" size="2"><span class="navBarPageCount">' + LBL_FROM + ' ' + this.pages + '</span></div>' + 
					'<div class="pGroup"><div class="btnNext pButton"></div><div class="btnLast pButton"></div></div>');
			
			nav_buttons.inject(divFooter);
					
			updateNavigatorCount(this, this.xml.getAttribute('rowCount'));
						...
						if(this.options[Field.PROPERTY_MAX_REG_GRID]) {
							if(this.options[Field.PROPERTY_PAGED_GRID]) {
								if((this.pages - 1) * this.options[Field.PROPERTY_PAGED_GRID_SIZE] + this.max_index >= this.options[Field.PROPERTY_MAX_REG_GRID]) {
									showMessage(GRID_MAX_ROW_REACHED);
									return;
								}
							} else {
...
//		this.container.set('html', '<div class="gridMinWidth">' + title + '</div>');
		...
		if(this.options[Field.PROPERTY_FONT_COLOR])
			this.container.getElements('div')[0].setStyle('color', this.options[Field.PROPERTY_FONT_COLOR]);
...
function updateNavigatorCount(ele, rowCount){
	var navigator = ele.divFooter.getElementById('navigator');
	if (!navigator) return;
	
	var totalRecords=ele.max_index;
	if (totalRecords>0)
		navigator.set('title', GNR_TOT_RECORDS + ": " + rowCount);
	else
		navigator.set('title', '');	
}