var Editor = new Class({
	...
	apijs_setFieldValue: function(value) {
		...
		if(!this.options[Field.PROPERTY_READONLY]){
			...
			$("div_" + this.content.id).set('html',value);
		}
	},
	...
	parseXML: function() {
		...
		//LABEL		
		var label = new Element('span.asLabel.asLabelEditor');
		...
		*/
		...
		if(this.options[Field.PROPERTY_READONLY] || this.form.readOnly) {
			...
			editor.set('data-attLabel', this.xml.getAttribute("attLabel"));
	...
	getPrintHTML: function(formContainer) {
		...
		if(!this.options[Field.PROPERTY_VISIBILITY_HIDDEN]) {
			...
			new Element('span.asLabel.asLabelEditor').appendText(this.xml.getAttribute("attLabel") + ':')
				.inject(div);
			...
			onSuccess: function(responseText, responseXML) {
		    	...
		    	if(responseXML && responseXML.childNodes && responseXML.childNodes.length) {
		    		...
		    		if(response.getAttribute('success')) {
		    			...
		    			var leftField = new Element('div.field')
		    				.setStyles({'margin-bottom':'10px','margin-right':'30px','width':'40%'})
		    				.inject(data.content);
		    			
	    				
	    				var div = new Element('div.editor_container');
	    				
	    				new Element('span.asLabel').appendText(ele.xml.getAttribute("attLabel") + ':').inject(div);
		    			...
		    			var rightField = new Element('div.field')
		    				.setStyles({'margin-right':'20px','width':'55%'})	
		    				.inject(data.content);
		    			
		    			new Element('span.asLabel', {text: ele.form.langs[lang_id] + ':'}).inject(rightField);
