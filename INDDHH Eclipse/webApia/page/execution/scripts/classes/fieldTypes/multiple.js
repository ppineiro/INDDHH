var Multiple = new Class({
		...
		if(values) {
			...
			if(this.options[Field.PROPERTY_READONLY]) {
				this.content.getElements('span').each(function(option) {
					if(values.contains(option.get('data-optvalue')))
	...
	apijs_setProperty: function(prp_name, prp_value) {
		...
		if(prp_name == Field.PROPERTY_NAME) {
			...
			if((prp_boolean_value == true || "true" == prp_value ) && this.options[Field.PROPERTY_READONLY] == false) {
				...
				this.HTMLselect.getElements('option').each(function (option) {
					if(option.get('selected')) {
						option_html += '<span style="font-weight: bold;" data-optvalue="' + option.get('value') + '">' + option.get('html') + '</span><br/>'; 
					} else {
						option_html += '<span data-optvalue="' + option.get('value') + '">' + option.get('html') + '</span><br/>';
					}
				});
	...
	apijs_clearOptions: function() {
		...
		if(this.options[Field.PROPERTY_READONLY]) {			
			var spans = this.content.getElements('span');
			for(var i = 0; i < spans.length; i++) {
				if(spans[i].hasClass('asLabel'))
					continue;
				spans[i].destroy();
			}
		...
		if(allowRepeatedValue) {
			...
			if(this.options[Field.PROPERTY_READONLY]) {
				this.content.getElement('div').getChildren('span').each(function(spn) {
					if(opt.get('data-optvalue') == store_value) {
						opt.set('html', show_value);
						found = true;
					}
				})
	...
	apijs_removeOption: function(value) {
		...
		if(this.options[Field.PROPERTY_READONLY]) {
			...
			if(opts) {
				for(var i = 0; i < opts.length; i++) {
					if(opts[i].get('data-optvalue') == value) {
			...
			if(!this.options[Field.PROPERTY_VISIBILITY_HIDDEN]) {
				...
				Array.from(this.xml.childNodes).each(function (option_xml) {
					...
					if(option_xml.getAttribute('selected')) {
						option_html += '<span data-optvalue="' + option_xml.getAttribute(Field.PROPERTY_VALUE) + '">' + inner_node + '</span><br/>'; 
					}
				});
...
//		label.appendText(this.xml.getAttribute("attLabel") + ':');
		...
		multiple.set('data-attLabel', this.xml.getAttribute("attLabel"));
		...
		if(this.options[Field.PROPERTY_READONLY]) {
			...
			Array.from(this.xml.childNodes).each(function (option_xml) {
				...
				if(option_xml.getAttribute('selected')) {
					option_html += '<span style="font-weight: bold;" data-optvalue="' + option_xml.getAttribute(Field.PROPERTY_VALUE) + '">' + inner_node + '</span><br/>'; 
				} else {
					option_html += '<span data-optvalue="' + option_xml.getAttribute(Field.PROPERTY_VALUE) + '">' + inner_node + '</span><br/>';
				}
			});
			...
			multiple_ro.inject(label);
		} else {
			multiple.inject(label);
		}
	...
	getPrintHTML: function(formContainer) {
		...
		var valueColor = this.options[Field.PROPERTY_VALUE_COLOR];
		...
		if(this.form.readOnly) {
			Array.from(this.xml.childNodes).each(function (option_xml) {
				...
				if(option_xml.getAttribute('selected')) {
					var input = new Element('span', {html: '<span class="multiple-value">' + option_xml.getAttribute(Field.PROPERTY_VALUE) + '</span><span class="multiple-separator"> - </span>' + inner_node});
					if(valueColor)
						input.setStyle('color', valueColor);
			...
			for(var value in opts) {
				var input = new Element('span', {html: '<span class="multiple-value">' + value + '</span><span class="multiple-separator"> - </span>' + opts[value]});
				if(this.options[Field.PROPERTY_VALUE_COLOR])
