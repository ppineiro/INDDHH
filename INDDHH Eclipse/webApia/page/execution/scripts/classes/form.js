...
var Form = new Class({
	...
	initialize: function(contentDIV) {
		...
		if (window.DOMParser) {
			...
			xmlDoc = parser.parseFromString(contentDIV.getAttribute("data-xml"),"text/xml");
		} else {
			// Internet Explorer
			xmlDoc = new ActiveXObject("Microsoft.XMLDOM");
			xmlDoc.async = false;
			xmlDoc.loadXML(contentDIV.getAttribute("data-xml")); 
		}
	...
	parseXML: function(form_tab_parent) {
		...
		if(this.options[Form.PROPERTY_FORM_TAB] == "true") {
			...
			// if(this.DOMform == null) {
				...
				this.DOMform.addClass('fieldGroup').addClass(this.frmName);
					
				var subTit = new Element('div', {id: this.id+'_title'});
				...
				var DOMtable = new Element("table", {id: "tbl" + this.id}).setStyle('table-layout', 'fixed');
				if (this.frmTitle && this.frmTitle.trim()!=''){
					DOMtable.title = this.frmTitle;
				} else {
					DOMtable.setAttribute('aria-describedby', this.id+'_title');
				}
				DOMtable.setAttribute('aria-labelledby',this.id);
				// var DOMtable = new Element("table", {id: "tbl" + this.id});
				
//-----
				var DOMtHead = new Element("thead");
				DOMtHead.inject(DOMtable);
				var DOMTrThs = new Element("tr");
				DOMTrThs.inject(DOMtHead);
				for(j=0;j<cols;j++){
					var domTh = new Element("th",{id:this.id +"_th_" + j});
					if (j==0){
						domTh.addClass("LBL_TITULO");
					}
					domTh.inject(DOMTrThs);
...
//-----
				...
				if(this.xml.getAttribute(Form.SIGNABLE_FORM) == "true") {
					 ...
					 */
					/*
					 * this.buttonSign = Generic.setButton(new
					 * Element('div.mark-to-sign', { html: '<table ' +
					 * margin_top + '><tbody><tr><td class="no-padding"><span
					 * style="line-height:15px;">' + BTN_MARK_FRM_TO_SIGN + '</span></td><td class="no-padding"><div
					 * style="line-height:17px;" class="' + sign_class + '"></div></td></tr></tbody></table>'
					 * })).inject(signatureDiv);
					 */
					
					if(!isMonitor && !window.kb && !IS_READONLY && !this.readOnly) {
						this.buttonSign = new Element('div.mark-to-sign', {
							// html: '<table ' + margin_top + '><tbody><tr><td
							// class="no-padding"><span
							// style="line-height:15px;">' +
							// BTN_MARK_FRM_TO_SIGN + '</span></td><td
							// class="no-padding"><div style="line-height:17px;"
							// class="' + sign_class +
							// '"></div></td></tr></tbody></table>'
							html: '<table><tbody><tr><td class="no-padding"><span style="line-height:15px;">' + BTN_MARK_FRM_TO_SIGN + '</span></td><td class="no-padding" style="padding-left: 3px;"><div style="line-height:17px;" class="' + sign_class + '"></div></td></tr></tbody></table>',
							tabIndex: ''
						}).inject(signatureDiv);
						
						if(this.xml.getAttribute(Form.REQUIRED_SIGN) == "true") {
							this.signRequired = true;
							// El formulario tiene firma requerida, se marca
							// para firmar y se lo deshabilita
							this.signed = true;
						} else {
							// Se agrega la funcion para marcar para firmar
							this.buttonSign.addEvent('click', this.signForm.bind(this));
							
							this.buttonSign.addEvent('keypress', Generic.enterKeyToClickListener);
						}
					}
					
					/*
					 * var buttonVerify = Generic.setButton(new
					 * Element('div.verify-signature', { html:
					 * BTN_VERIFY_FRM_SIGN
					 * })).inject(signatureDiv).addEvent('click', function() {
					 * ModalController.openWinModal(CONTEXT +
					 * '/apia.execution.FormAction.run?action=viewSigns&frmId=' +
					 * frmId + '&frmParent=' + frmType + TAB_ID_REQUEST, 700,
					 * 460, null, null, true,true,false); });
					 */
...
// }
//					
// new Element('div.langIco', {title: 'Mostrar traduccion de
// contenido'}).inject(signatureDiv).addEvent('click', function(evt) {
...
// }
			...
			for(var i = 0; i < this.xml.childNodes.length; i++) {
				...
				// TODO: pasar el this.gridEditionIndex al field si
				// this.isGridEditionForm
...
// }
	...
	parseField: function(xml) {
		...
		switch (xml.getAttribute("fieldType")) {
			...
			case Field.TYPE_CAPTCHA:
				field = new Captcha(this, this.id, xml);
				break;
		}
	...
	collapseForm: function(noFx) {
		...
		current_form.max_height = Generic.getHiddenHeight(current_form.DOMform) ;// current_form.DOMform.getHeight();
	...
	getPrintHTML: function() {
		...
		// this.id = frmType + "_" + this.xml.getAttribute("id");
		/*
		 * if(this.options[Form.PROPERTY_FORM_INVISIBLE]) { this.hideForm(); }
		 */
		if(this.options[Form.PROPERTY_FORM_TAB] != "true") {
			
			// this.DOMform.addClass('fieldGroup');
				
			var subTit = new Element('div.title', {text: this.frmTitle});
			...
			if(this.xml.getAttribute(Form.SIGNABLE_FORM) == "true") {
				...
				Generic.setButton(new Element('div.mark-to-sign', {
					html: '<table ' +  margin_top + '><tbody><tr><td class="no-padding"><span style="line-height:15px;">' + BTN_MARK_FRM_TO_SIGN + '</span></td><td class="no-padding"><div style="line-height:17px;" class="' + sign_class + '"></div></td></tr><tbody></table>'
				})).inject(signatureDiv);
...
Form.PROPERTY_FORM_READONLY		= "frmReadonly"; // ESTA PROPIEDAD SOLO PUEDE
													// SER LEIDA
