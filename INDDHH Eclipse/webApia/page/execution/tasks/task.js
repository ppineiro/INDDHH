...
function initPage() {
	...
	if(!isMonitor && TSK_TITLE && getTabContainerController())
	...
	try{
		...
		if(currentClassE!=""){
			showMessage(Generic.formatMsg("<system:label show='text' label='errExecBusClass' forHtml='true'/>", currentClassE));
		}
	}
	try{
		frmOnloadP();
		...
		if(currentClassP!=""){
			showMessage(Generic.formatMsg("<system:label show='text' label='errExecBusClass' forHtml='true'/>", currentClassP));
		}
	}
			...
			if(btnConf.getProperty('btnDisabled') != 'true'){
				...
				if(forceConfirm) {
					...
					SynchronizeFields.syncJAVAexec(function() {
						 ...
						 */
						var submitUrl = 'apia.execution.TaskAction.run?action=checkWebDavDocumentsLocks' + TAB_ID_REQUEST;
	...
	if (btnCloseTab) {
		btnCloseTab.addEvent('click', function() {
			...
			showConfirm(GNR_PER_DAT_ING, '', function(res) {
				if(res)
					try{
...
function checkActionRedirect(){
	...
	if(EXTERNAL_ACCESS=="true"){
		...
		if(onFinish=="1"){
			...
			*/
			window.top.location.href =  CONTEXT + "/portal/redirectSLO.jsp?url=" + URL_RETORNO + TAB_ID_REQUEST;
		} else if(onFinish=="101") {
			//XD.postMessage("custom_close_tab", "*", parent.parent)	
			window.top.location.href =  CONTEXT + "/portal/redirectSLO.jsp?url=" + URL_RETORNO + TAB_ID_REQUEST;
		} else if(onFinish=="102") {
			//XD.postMessage("custom_close_tab", "*", parent.parent)
			//top.window.location = URL_RETORNO;						
			window.top.location.href =  CONTEXT + "/portal/redirectSLO.jsp?url=" + URL_RETORNO + TAB_ID_REQUEST;			
		} else {
			//keepBlocking
