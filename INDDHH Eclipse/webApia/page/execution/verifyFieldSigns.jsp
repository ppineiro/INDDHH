<%@page import="biz.statum.apia.web.bean.monitor.TasksBean"%><%@page import="biz.statum.apia.web.bean.monitor.BusinessBean"%><%@page import="biz.statum.apia.web.bean.BeanUtils"%><%@page import="com.dogma.vo.UsrSignsVo"%><%@include file="../includes/startInc.jsp" %><%
...
if(basicBean instanceof biz.statum.apia.web.bean.execution.EntInstanceListBean){
	...
	if("E".equals(request.getParameter("frmParent")))
		aBean = ((biz.statum.apia.web.bean.monitor.ProcessesBean)basicBean).getEntInstanceBean();
	else if("P".equals(request.getParameter("frmParent")))
		aBean = ((biz.statum.apia.web.bean.monitor.ProcessesBean)basicBean).getProInstanceBean();
...
Collection<UsrSignsVo> formSignatures = fBean.getFieldSignatures(Integer.valueOf(request.getParameter("attId")), Integer.valueOf(request.getParameter("index")), langId, fBean.getUserData(request));
System.out.println(formSignatures);
%><html lang="<%=biz.statum.apia.web.bean.BasicBeanStatic.getUserDataStatic(request).getLangCode()%>"><head><%@include file="../includes/headInclude.jsp" %><script type="text/javascript">
			...
			function checkSign(signId){
				var submitUrl = 'apia.execution.FormAction.run?action=verifyFieldSignature&signId=' + signId + "&frmId=<%=request.getParameter("frmId")%>&frmParent=<%=fBean.getParentBeanDesc()%>&fldId=<%=request.getParameter("fldId")%>&index=<%=request.getParameter("index")%><%=langId == null ? "" : "&langId=" + langId%>" + TAB_ID_REQUEST;
