...
function processXmlParameters(resXml){
	...
	if (parameters != null && parameters.length > 0 && parameters.item(0) != null) {
		...
		for(var i = 0; i < parameters.length; i++){
			...
			if(isRequired)
				div1.set('text', '*');
			var spanName = new Element('span',{html: xmlParameter.getAttribute("name")}).inject(div1);
...
function setStrParamsToSend(){
	...
	$('tableDataParams').getElements("tr").each(function (tr){
		...
		var parName = trContent[0];
		if(trContent[0].indexOf('*')>-1){
			parName = trContent[0].substr(trContent[0].indexOf('*')+2,trContent[0].length)	
		}

		strParams += parName + PRIMARY_SEPARATOR + trContent[1];
		
	});
	$('txtParams').value = strParams;
}
