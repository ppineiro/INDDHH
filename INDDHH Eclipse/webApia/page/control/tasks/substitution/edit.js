...
function initPage(){
	...
	$$("input.datePickerCustom").each(function(datepicker) {
		...
		var img = new Element("img.datepickerSelector", {src: CONTEXT+"/css/base/img/calendar.png"});
	...
	getData();
 	...
 	var extraSQL = "SubstitutionsExtra," + ENV_ID;
			...
			if (ajaxCallXml.getElementsByTagName("groups")[0].getElementsByTagName("group")){
				...
				for(var i = 0; i < groups.length; i++){					
					...
					for (var j = 0; j < users.length; j++) {
						var xmlUser = users[j];
						var aUser = new Element("div", {'class': 'option', 'id': xmlUser.getAttribute("id"), html: xmlUser.getAttribute("login") });
						if (!IS_FINISHED) { new Element('div.optionRemove').addEvent('click', function(evt) { this.getParent().destroy(); }).inject(aUser); }
						aUser.inject(tdUsers);
					}
					if (!IS_FINISHED){
						var addUser = new Element("div", {'class': 'option', html: BTN_ADD }).setStyle('cursor', 'pointer');
					...
					for (var j = 0; j < profiles.length; j++) {
						...
						var aProfile = new Element("div", {'class': 'option', 'id': xmlProfile.getAttribute("id"), 'name': xmlProfile.getAttribute("name"), html: xmlProfile.getAttribute("name") });
						if (!IS_FINISHED) { new Element('div.optionRemove').addEvent('click', function(evt) { this.getParent().destroy(); }).inject(aProfile); }
						aProfile.inject(tdProfiles);
					}
					if (!IS_FINISHED){
						var addProf = new Element("div", {'class': 'option', html: BTN_ADD }).setStyle('cursor', 'pointer');
...
function processUsersModalReturnGrid(ret){
	...
	ret.each(function(e){
		if (e.getRowId() != $('usrLogin').value && !alreadyExists(container,e.getRowId())){
			var nUser = new Element("div", {'class': 'option', 'id': e.getRowId(), html: e.getRowId() });
			//nUser.addEvent('click', function(evt) { this.destroy(); });
			new Element('div.optionRemove').addEvent('click', function(evt) { this.getParent().destroy(); }).inject(nUser);
...
function processProfilesModalReturnGrid(ret){
	...
	ret.each(function(e){
		if (!alreadyExists(container,e.getRowId())){
			...
			var nProfile = new Element("div", {'class': 'option', 'id': e.getRowId(), 'name': content[0], html: content[0] });
//			nProfile.addEvent('click', function(evt) { this.destroy(); });
			new Element('div.optionRemove').addEvent('click', function(evt) { this.getParent().destroy(); }).inject(nProfile);
...
function getValuesToSend(){
	...
	for (var i = 0; i < totalGroups; i++){
		...
		//Usuarios
		$("usrContainer_"+i).getChildren("DIV").each(function(item){
		...
		values = "";
		$("profContainer_"+i).getChildren("DIV").each(function(item){
			var id = item.getAttribute("id");
			if (id != "addPro_"+i){
