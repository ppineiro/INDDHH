function initPage(){
	...
	['createDateFilterStart', 'createDateFilterEnd', 'txtReaSta', 'txtReaEnd', 'alertDateFilterStart', 'alertDateFilterEnd', 'overdueDateFilterStart', 'overdueDateFilterEnd'].each(setDateFilters);
...
function loadTasksByProcess(cmbProcTitFilter){
	...
	if (cmbValue != null && cmbValue != ""){
		var proInfo = cmbProcTitFilter.options[cmbProcTitFilter.selectedIndex]; 
		var proId = proInfo.getAttribute("proId");
		var proTitle = proInfo.value;
		var request = new Request({
			method : 'post',
			url : CONTEXT + URL_REQUEST_AJAX + '?action=loadTasksByProcess&isAjax=true&proId=' + proId + '&txtProTitle=' + proTitle + TAB_ID_REQUEST,
			onRequest : function() { },
