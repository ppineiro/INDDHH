...
function openTableData(element, tableName){
	var selConn = $('selConn').value; //Conexion seleccionada en este momento
	extra = {'table':tableName, 'selConn': selConn };
...
function addColumnsToContainer(addCols, delCols) {
	...
	if (addCols!=""){
		...
		for (var i=0;i<addColsArr.length;i++){
			...
			new Element('input',{ type: 'hidden', 'name': 'chkColumn', 'value': tabCol}).inject(divElement);
			new Element('input',{ type: 'hidden', 'name': 'colName', 'value': tabCol}).inject(divElement);
		 	
			divElement.container = container;
//			divElement.addClass("optionRemove");
			
			new Element('div.optionRemove').addEvent('click', actionClickRemoveColumn).inject(divElement);
...
function actionClickRemoveColumn (evt) {
	...
	var tableCol = evt.target.getParent().get('id');
	...
	var f_binded = actionElementAdminClickRemove.bind(this);
	f_binded(evt);
