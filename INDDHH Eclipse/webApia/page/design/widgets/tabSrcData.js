...
function initSrcDataTab(mode) {
	...
	if ($('cmbSrcType')) {
		...
		showDivSrc();
		// Accion al modificar combo de origen de datos
	...
	if (widType == WIDGET_TYPE_QUERY_ID) {
		loadCmbSrcQryType(); //Mostramos las consultas segun si deben tener grafico o no
	}
	...
	// Accion al modificar combo de cubos
	if ($('cmbSrcCube')) {
		loadCubeViews();
	...
	// Accion al modificar combo de cubos para el boton de otros cubos
	if ($('cmbSrcOthCube')) {
		loadOthCubeViews();
		$('cmbSrcOthCube').addEvent("change", function(e) {
			loadOthCubeViews();
		});
	}
...
function testSQL() {
	if ($('cmbType').get('value') == WIDGET_TYPE_KPI_ID) {
		...
		var request = new Request({
			...
			onRequest: SYS_PANELS.showLoading(),
			onComplete : function(resText, resXml) {SYS_PANELS.closeAll(); processSQLTestResult(resXml);},
		}).send();
	} else { // CUSTOM
...
//Carga las vistas del cubo seleccionado para el boton de otros cubos
function loadOthCubeViews() {
	cleanCombo('cmbSrcOthCubeView'); // vaciamos el combo de vistas
	var cbeId = $('cmbSrcOthCube').get('value');
	if (cbeId > 0) {
		var request = new Request({
			method : 'post',
			data : {
...
//Lee el xml resultado de solicitar las vistas de un cubo (para el boton otros cubos)
function loadOthViewsXML(ajaxCallXml) {
	if (ajaxCallXml != null) {
		var views = ajaxCallXml.getElementsByTagName("views");
		if (views != null && views.length > 0 && views.item(0) != null) {
			views = views.item(0).getElementsByTagName("view");
...
//Agrega una vista en el combo de vistas (del boton otros cubos)
function fncAddOthView(vwId, vwName) {
	var views = $('cmbSrcOthCubeView').getElements('option');
	...
	var option = new Element('option', {
		html : vwName,
		value : vwId
	}).inject($('cmbSrcOthCubeView'));

	if (vwId == widOthCbeVwId)
		option.selected = true;
}
...
function showDivSrc() {
	...
	if (!$('chkOpenOthCbe').checked) {
		$('divSrcOthCubeView').setStyle('display', 'none');
	}
	
	if (!$('chkOpenOthQry').checked) {
...
function loadCmbSrcQryType(onlyWithCharts) {
	if (onlyWithCharts==null){
		...
		if (cmbQryViewType.get('value') == WIDGET_QRY_VIEW_CHART) {
...
function loadCmbOthQuerys() {
	cleanCombo('cmbSrcOthQry');
	var request = new Request({
		method: 'post',		
		data:{'onlyWithCharts':false},
		url: CONTEXT + URL_REQUEST_AJAX+'?action=getQuerys&isAjax=true' + TAB_ID_REQUEST,
		onComplete: function(resText, resXml) {loadOthQuerysXML(resXml);  }
	}).send();
}

function loadOthQuerysXML(ajaxCallXml) {
	/*
	 * <result>
	 * 	<querys>
	 *     <query qryId="1011" qryName="AAAA"/>
	...
	var selectedQryId = $('hidOthQryId').get('value');
	...
	if ($('chkOpenOthQry').checked) {
		$('cmbSrcOthQry').set('value', $('hidOthQryId').get('value'));
	}
