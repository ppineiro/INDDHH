...
function initPage() {
	...
	//Se actualizan eventos de cierre, para detectar cambios en datos
	closeBtn = window.parent.document.body.getElement('div.modalBottomBar').getElement('div.modalBottomBarClose');
	...
	var topCloseBtn = window.parent.modal.modalWin.getElement('div.modalCloseBtn');
	topCloseBtn.removeEvents('click');
	topCloseBtn.addEvent('click', function(e){
		if (e) e.stop();
		closeBtn.click();
	})
}
...
function loadQueryShowColsDataXML(ajaxCallXml) {
	if (ajaxCallXml != null) {
		...
		if (params != null && params.length > 0 && params.item(0) != null) {
			...
			showMessage(MSG_WID_QRY_COL_ERR, GNR_TIT_WARNING, 'modalWarning');
			setTimeout(closeModal,4000);			
		}
	}
}
...
function checkParamsUpdate(){
	var modElements = $('frmParamsDesign').getElements('*.highlighted');
	if (modElements.length>0){		
		SYS_PANELS.newPanel();
		var panel = SYS_PANELS.getActive();
		panel.addClass("modalWarning");
		panel.content.innerHTML = GNR_PER_DAT_ING;
		panel.footer.innerHTML = "<div class='button' onClick=\"SYS_PANELS.closeAll(); closeModal();\">" + BTN_CONFIRM + "</div>";
...
//			if (! tdWidths[i]) tdWidths[i] = headers[i].get('width');
	...
	var oTd0 = new Element("TD",{styles:{'width':tdWidths[0]}}); //nombre
	var oTd1 = new Element("TD",{styles:{'width':tdWidths[1]}}); //valor
	var oTd2 = new Element("TD",{styles:{'width':tdWidths[2]}}); //tipo
	var oTd3 = new Element("TD",{styles:{'width':tdWidths[3]}}); //in/Out
	var oTd4 = new Element("TD",{styles:{'width':tdWidths[4]}}); //usar en widget
...
//	oTr.addClass("selectableTR");
	...
	var div = oTd0.getElement('div');
	if (div.scrollWidth > div.offsetWidth) {
		oTd0.title = div.getElement('label').get('text');			
		oTd0.addClass("titiled");
	}
...
//			if (! tdWidths[i]) tdWidths[i] = headers[i].get('width');
	...
	var oTd0 = new Element("TD",{styles:{'width':tdWidths[0]}}); //col
	var oTd1 = new Element("TD",{styles:{'width':tdWidths[1]}}); //type
	var oTd2 = new Element("TD",{styles:{'width':tdWidths[2]}}); //value
	var oTd3 = new Element("TD",{styles:{'width':tdWidths[3]}}); //req
...
//			if (! tdWidths[i]) tdWidths[i] = headers[i].get('width');
	...
	var oTd0 = new Element("TD", {styles:{'width':tdWidths[0]}}); //name
	var oTd1 = new Element("TD", {styles:{'width':tdWidths[1]}}); //type
	var oTd2 = new Element("TD", {styles:{'width':tdWidths[2]}}); //hidden
	var oTd3 = new Element("TD", {styles:{'width':tdWidths[3]}}); //forWidget
...
function myToggle(oTr){
	//CAM_12713 - Se quita estilo al seleccionar fila
...
function unselOthers(el){
	...
	if (oRows != null) {
		...
		for (i = 0; i < oRows.length; i++) {
			...
			if(input!=el){
				input.checked=false;
				input.getParent('tr').removeClass("selectedTR");
				if (forZone){
					oTd = oRow.cells[1];
...
function onLoadModalEvent(){
	resetModalHighlight();
	window.parent.initAdminFieldOnChangeHighlight(false, false, false, $('frmParamsDesign'));
}

function resetModalHighlight(){
    //Se limpia información de modal
    $('frmParamsDesign').getElements('*.highlighted').each(function(ele) { 
