...
function initPage(){//Funcion necesaria en todo jsp de la 3.0
	//Este m�todo es invocado al princpio
} 

function initChart(){ //Este m�todo es invocado luego de cargada la p�gina
	loadSeriesEvents();
...
function loadChartDesignFromXML(ajaxCallXml) {
	if (ajaxCallXml != null) {
		...
		if (!("true" == chart.item(0).getAttribute("viewYLabels"))) $('chkYLabels').set('checked', false);
		if (!("true" == chart.item(0).getAttribute("viewXLabels"))) $('chkXLabels').set('checked', false);
...
function showChart() {
	$('ifrChartSample').set('src', 'chartSample.jsp?chartType=' + $('chtType').get('value') + '&chtSubType='+ $('chtSubType').get('value') +
			...
			'&viewGrid=' + $('chkGrid').get('checked') + '&viewLegend=' + $('chkLegend').get('checked') + 
			'&viewXLabels=' + $('chkXLabels').get('checked') + '&viewYLabels=' + $('chkYLabels').get('checked') +
			'&viewValues=' + $('chkValues').get('checked') + '&h=220&w=300');
...
function fncAddSeries(id, name, color, show, attId, busClaParId) {
	...
	inputColor.setStyle('width',  20);
	inputColor.setStyle('margin-left',  10);
...
function lockSerie(){
	...
	for(var i = 0; i < rows.length; i++) {
		...
		if(id == rowId) {
			tds[1].getElement('input').set('disabled', true);
			...
			tds[1].getElement('input').set('disabled', false);
...
function getFormParameters() {
	...
	params += "&viewYLabels=" + $('chkYLabels').get('checked');
	params += "&viewXLabels=" + $('chkXLabels').get('checked');
