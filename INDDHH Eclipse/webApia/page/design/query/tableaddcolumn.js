function addColumn(bodyShow,prefix,extra,width){
				...
				if (aux!=null){
					...
					for (var k=0;k<aux.arr.length;k++){
						...
						if (auxRow.type=="text"){
							...
							if ('txtShowHeadWidth' == auxRow.id)
								domElement.maxLength = 3;
						...
						if(!QRY_FREE_SQL_MODE) {
							...
							} else if (auxRow.name=='cmbShoAttFrom'){
								if (auxRow.isAttribute) domElement.setAttribute('data-isAttribute', true);
								domElement.addEvent('change', function() {
									if (this.isVisible()){
	...
	addElementsOnChangeHighlight(elements);
	
	//var hideTDs = bodyShow.getElementsByClassName('hideTD');
	var hideTDs = bodyShow.getElements('.hideTD')
	...
	if(tableBodyUserRows) {
		for(l = 0; l < tableBodyUserRows.length; l++) {
			if(tableBodyUserRows[l].cells[0].getElement('span').get('html') == col_name) {
				showConfirm(msgConfColChng, lblEncFilter, function(res) {
					if(res)
