//En caso de cambiar de 'Tipo', se da la opci�n de 
//actualizar posici�n en �rbol de funcionalidades
var uptFuncHierarchy = false;

var STEP_1_MODIFIED, STEP_2_MODIFIED;
...
function initPage(){
	...
	if (btnSig){
		btnSig.addEvent("click",function(e){
			...
			params += '&uptFuncHierarchy=' + uptFuncHierarchy;
			
			sp.show(true);
			
			if (isAnyElementModified()) params += '&stepDbConnMod=true';
		...
		if (this.value!=""){
			var request = new Request({
				...
				onComplete: function(resText, resXml) { 
					//Limpio valores del combobox tipo de consulta
					var selQryTyp = $('selQryTyp');
					if (selQryTyp){
						selQryTyp.empty();	
					}	
...
function processQueryTypesXml(){
	...
	select.addEvent("change",function(e){		
		if (QRY_ORIGINAL_TYPE != this.value && !MODE_CREATE && !ONLY_VIEW){
			showConfirm(LBL_QRY_UPT_FUNC_HIER, GNR_TIT_WARNING, 
				function(optSelected){	uptFuncHierarchy = optSelected; },
				'modalWarning');			
		}
	});
	
	addElementsOnChangeHighlight( $$('#'+PARAM_NAME_QUERY_TYPE) );	
...
function processViewsXml(showAdvancedTab){
	...
	addElementsOnChangeHighlight( $$('#'+PARAM_NAME_VIEWS) );
