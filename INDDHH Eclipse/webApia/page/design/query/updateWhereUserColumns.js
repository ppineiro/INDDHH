...
//Variable que almancena nombre de columnas a mostrar
var currentColumnsNames = [];
...
function addFreeSqlEvents(tr) {
	...
	var inputShowAsAddF	= tds[18].getElementsByTagName("INPUT")[0];//INPUT
	...
	inputType.inputShowAsAddF	= inputShowAsAddF;	
...
function executeFreeSqlvent(caller) {
	...
	if(currentValue == COLUMN_DATA_DATE){
		caller.inputShowAsAddF.checked  = true;
		caller.inputShowAsAddF.disabled = true;
	}
}

function processColumnsModalReturnUser(ret){
	currentColumnsNames = [];
	$('tblShows').getElements("[name='hidShoColName']").each(function(ele){
		currentColumnsNames.push(ele.value);
	})
...
function generateUser(val0,val1,val2,val3,val4,val5) {
	...
	if (val3 == COLUMN_DATA_DATE) {
		arrayOptions.push({'value':1,'text':lblBoth,'selected':true});	
		arrayOptions.push({'value':0,'text':lblNone,'selected':false});
		arrayOptions.push({'value':2,'text':lblDteFrom,'selected':false});	
		arrayOptions.push({'value':3,'text':lblDteTo,'selected':false});
	} else {
	...
	arrayOptions.push({'value':1,'text':lblYes,'selected':false});		
	arrayOptions.push({'value':2,'text':lblListbox,'selected':false});
	...
	//Fila
	arrayCell = new Array();
	aux = {'type':'checkbox',name:'chkShowAsAddFilter',id:'chkShowAsAddFilter','required':false,'checked':false,value:1};
	if (!QRY_FREE_SQL_MODE){
		if (val3 == COLUMN_DATA_DATE || !currentColumnsNames.contains(val1)){
			aux.checked=true;
			aux.disabled=true;
		}		
	}
...
function onChangeCmbType(ele){
	...
	var filter = row.getElementById('cmbUsuFilter');
	if (sort){
		if (ele.value==0){
			sort.onclick = function() {return false};
			sort.checked = false;
			filter.value = 0;
			filter.disabled = '';
		}
...
function addRowWhereUser(table,arrTable,displayRow){
	...
	initAdminFieldOnChangeHighlight(false, false, false, rowDOM);
...
function addRowWhereUserTd(temp,div) {
	...
	for (var i=0;i<td.length;i++){
		var aux = td[i];
		if (aux.type=="text"){
			domElement = new Element('input',{type:'text',name:aux.name,id:aux.id, maxlength:aux.maxLength});			
			//domElement.setAttribute("value",aux.value);
			domElement.set("value", aux.value);
			if ('txtUserHeadName' == aux.id || 'txtUserTool' == aux.id)
				domElement.maxLength = 255;
			if ('txtUserVal' == aux.id) {
				if (aux.validation && aux.validation.includes('numeric'))
					domElement.maxLength = 10;
				else if (!(aux.className && aux.className.includes('date')))
					domElement.maxLength = 100;
			}
...
function processLoadWhereUser(){
	...
	if (tableDOM!=null){
		...
		var table = $('tableBodyUser');
		if (table && table.getParent('.fieldGroup')){
