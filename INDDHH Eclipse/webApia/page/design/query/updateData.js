...
function initPage(){
	...
	if (btnBackToList){
		btnBackToList.addEvent("click",function(e){
			e.stop();	
			if (ONLY_VIEW || !(isAnyElementModified() || STEP_DB_CONN_MODIFIED)){
				goBackList();
			} else {
				SYS_PANELS.newPanel();
	...
	if (btnAnt){
		btnAnt.addEvent("click",function(e){
			...
			if (ONLY_VIEW){
				sp.show(true);
				window.location = CONTEXT + URL_REQUEST_AJAX + '?action=update&back=true&onlyRead=true&' + TAB_ID_REQUEST;
			} else {
				if (!isAnyElementModified()){
					...
					panel.addClass("modalWarning");
					panel.content.innerHTML = GNR_PER_DAT_ING;
					panel.footer.innerHTML = "<div class='button' onClick=\"SYS_PANELS.closeAll(); goPrevious();\">" + BTN_CONFIRM + "</div>";
					SYS_PANELS.addClose(panel);
					SYS_PANELS.refresh();	
				}
			}
			...
			if (c==0){
				...
				for (var a=0;a<cols.length;a++){
					if (cols[a]!=""){
						...
						//Se actualiza, si corresponde, forma de mostrar filtro
						var columnName = trows[0].cells[col].getElement("[name='hidShoColName']");
						updateUserFilter(columnName.value, false);
...
//        });
	...
	if (IS_QUERY_TYPE_OFFLINE){
		...
		var fchIni = $('txtFchIni');
		setAdmDatePicker(fchIni);
		$('frmData').formChecker.dispose(fchIni);
		$('frmData').formChecker.register(fchIni.getNext());
	}
...
function downRow(pos,tblName){
	...
	if (pos==table.rows.length){
		...
		table.rows[p].toggleClass("trOdd");
		table.rows[p+1].toggleClass("trOdd");
...
function processTd(td){
	...
	while (i<cells.length){
		...
		if (ccType =="input") {
			var maxLength = firstChild.getAttribute("maxlength");
			var aux = {'type':'text',name:ccName,id:ccId,'required':isRequired,value:firstChild.getAttribute("value"),'display':display,'validation':validation,hasDatePicker:cc.getAttribute("hasDatePicker"),disabled:isDisabled,bgColor:cc.getAttribute("bgColor"),isReadOnly:isReadOnly,'onkeypress':firstChild.getAttribute("onkeypress"), 'maxLength':maxLength ? maxLength : 255};
			...
			isDisabled = isDisabled || toBoolean(firstChild.getAttribute('disabled'));
	...
	if (form.childNodes.length > 0) {
		for (var i = 0; i < form.elements.length; i++) {
			...
			if (formElement.type == "select-multiple") {
				...
				if (formEleName=="chkShowAsAddFilter" || !formElement.disabled || formElement.type == "select-one"){
					if (params != "") params += "&";
...
function processLoadShow(){
	...
	if (tableDOM!=null){
			...
			while (k < cells.length){		
				...
				if (type=="row"){
					...
					while (l < cellsCol.length){	
						...
						var isAttribute = cc.getAttribute("isAttribute")=='true';
						...
						if (ccType =="input"){
							...
							if (ccName=='cmbShoAttFrom' && isAttribute) aux.isAttribute=true;
		...
		initAdminModalHandlerOnChangeHighlight(table.getElement('thead'), true, false, notification);
...
function processAttributesModalReturn(ret){
	...
	ret.each(function(e){
		...
		if (addRet) {
			...
			//ret[2] = headName
			//ret[3] = type
			generateShow(e.getRowId(),colName,headName,type,QRT_DB_TYPE_NONE,"", true);
...
function generateShow(val0, val1, val2, val3,val4,val5,addAttribute) {
	//val0;hidShoAttId
	//val1;hidShoColName
	//val3;hidShoDatType
	//val4;hidShoDbType
	...
	if (addAttribute) aux.isAttribute=true;	
	arrayRows.push(aux);	
	...
	if (CAN_HAVE_PANEL && !IS_QUERY_TYPE_OFFLINE){
		//Fila
		arrayRows = new Array();
		var aux = {'type':'checkbox', name:'chkDontShowInPanel', id:'chkDontShowInPanel', 'required':false, 'checked':false, value:0 };
		arrayRows.push(aux);
		arrayColumn.push({'display':'', arr:arrayRows});	
	}
	...
	if (!IS_QUERY_TYPE_OFFLINE){
		//Fila
		arrayRows = new Array();
		var aux = {'type':'checkbox', name:'chkAutoFitWidthTitle', id:'chkAutoFitWidthTitle', 'required':false, 'checked':false, value:0 };
		arrayRows.push(aux);
		arrayColumn.push({'display':'', arr:arrayRows});	
	}
	...
	//Fila
	arrayRows = new Array();
	var aux = {'type':'checkbox', name:'chkIsEntState', id:'chkIsEntState', 'required':false, 'checked':false, value:1, 'visibility': COLUMN_DATA_DATE == val3 ? "hidden" : "", display:val0 != null && val0 != ""?'none':''};
	arrayRows.push(aux);
	arrayColumn.push({'display':'', arr:arrayRows});
...
function loadSchedulers(){
	var request = new Request({
		...
		onComplete: function(resText, resXml) { 
			modalProcessXml(resXml); 
			//Se setea validación inicial del nodo
			showOtherNode($('schNode'), null, true);
			SYS_PANELS.closeAll(); }
	}).send();
}

function showOtherNode(cmb, show, avoidClearInput){
	$("radSelected").value = cmb.value;
	
	var input = $('txtExeNode');
	var container = $('divSchNodeName');
	if (!avoidClearInput) input.value = "";
	if (cmb.value == "1"){
...
function loadQueryButtons(){
	var request = new Request({
		...
		onComplete: function(resText, resXml) { 
			...
			initAdminFieldOnChangeHighlight(false, true, true, $('divQueryButtons').getParent('.fieldGroup'));
		}
	}).send();
}
...
function checkActions() {
	if (isQuery) {
		if ($("chkActVieEnt")){
			var hasActCols = hasActColumns(requiereActVieEntCol);
			if (!hasActCols) $("chkActVieEnt").checked = false; 
			$("chkActVieEnt").disabled = ! hasActCols;
		}
		if ($("chkActViePro")){
			var hasActCols = hasActColumns(requiereActVieProCol);
			if (!hasActCols) $("chkActViePro").checked = false; 
			$("chkActViePro").disabled = ! hasActCols;
		}
		if ($("chkActViwTas")){
			var hasActCols = hasActColumns(requiereActVieTasCol);
			if (!hasActCols) $("chkActViwTas").checked = false; 
			$("chkActViwTas").disabled = ! hasActCols;
		}
		if ($("chkActWorEnt")){
			var hasActCols = hasActColumns(requiereActWorEntCol);
			if (!hasActCols) $("chkActWorEnt").checked = false; 
			$("chkActWorEnt").disabled = ! hasActCols;
		}
		if ($("chkActWorTas")){
			var hasActCols = hasActColumns(requiereActWorTasCol);
			if (!hasActCols) $("chkActWorTas").checked = false; 
			$("chkActWorTas").disabled = ! hasActCols;
		}
		if ($("chkActAcqTas")){
			var hasActCols = hasActColumns(requiereActAcqTasCol);
			if (!hasActCols) $("chkActAcqTas").checked = false; 
			$("chkActAcqTas").disabled = ! hasActCols;
		}
		if ($("chkActComTas")){
			var hasActCols = hasActColumns(requiereActComTasCol);
			if (!hasActCols) $("chkActComTas").checked = false; 
			$("chkActComTas").disabled = ! hasActCols;
		}
	}
}
...
function loadAttRemaping(){
	var request = new Request({
		...
		onRequest: function() { SYS_PANELS.showLoading(); },
		onComplete: function(resText, resXml) { 
			modalProcessXml(resXml); addScrollTable($('bodyAttRemping'));
			
			var table = $('bodyAttRemping');
			var footer = table.getParent('.fieldGroup').getElements('.listAddDel')[0];
			var notification = new Element('div', {id : 'editionNot'}); 
			footer.grab(notification, "top");
...
//	$('chartIsInDB').value=isInDB;
	...
	var modal = ModalController.openWinModal(CONTEXT
			+ "/page/design/query/chartDesignModal.jsp?id="+id+"&isInDB=" + isInDB + TAB_ID_REQUEST, 800, 600); //ancho,alto
...
//	elemDiv.addEvent('click', function(evt) { deleteGraph(this,id,isInDB); });
	...
	elemDiv.container = container;
	new Element('div.optionRemove').addEvent('click', function(evt) { deleteGraph(elemDiv, id, isInDB); }).inject(elemDiv);
...
//Actualizar index de valor a guardar/mostrar
function updateValuesForModal() {
	var tds = $('bodyShow').getElement('tbody').getElement('tr').getChildren('td');
	
	//Corregir valor a guardar
	var valueInput = $('selQryColIdModValue');
	if(valueInput.get('tag') == 'input') {
		var valueName = $('selQryColIdModValueName').get('value');
		for(var j = 1; j < tds.length; j++) {
			if(tds[j].getElement('span').get('html') == valueName) {
				valueInput.set('value', j - 1);
				break;
			}
		}
	} else {
		var opts = valueInput.getElements('option');
		for(var i = 0; i < opts.length; i++) {
			var colName = opts[i].get('html');
			
			for(var j = 1; j < tds.length; j++) {
				if(tds[j].getElement('span').get('html') == colName) {
					opts[i].set('value', j - 1);
					break;
				}
			}
	...
	//Corregir valor a mostrar
	var textSelect = $('selQryColIdModText');
	if(textSelect) {
		var opts = textSelect.getElements('option');
		for(var i = 0; i < opts.length; i++) {
			var colName = opts[i].get('html');
...
function updateUserFilter(columnName, addShowColumn){
	
	if(!$('tableBodyUser')) return;
	
	var rows = $('tableBodyUser').getElements('tr');
	if (!rows) return;
	
	rows.each(function(r){
		var showName = r.getElement("[name='hidUserColName']");
		if (showName && showName.value==columnName){
			var chk = r.getElementById('chkShowAsAddFilter');
			chk.checked=!addShowColumn;
			chk.disabled=!addShowColumn;
			return;
		}
	})
}
