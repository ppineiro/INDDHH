...
function initPage() {
	...
	if (btnConf){
		btnConf.addEvent("click",function(e){
			...
			var events = getEventsLayout('BC');
			$('txtMap').set('value', events);
			
			var states = getEventsLayout('STA');
			$('txtEntityStateXML').set('value', states);
	...
	if (btnBackToList){
		btnBackToList.addEvent("click",function(e){
			e.stop();
			if (ONLY_VIEW || !isAnyElementModified()){
				clickGoBack();
			} else {
				SYS_PANELS.newPanel();
    ...
    $('envEvtTab').addEvent("focus", function(evt){
    	["BC", "STA"].each(function(prefix){ 
    		addScrollTable($('tableData'+prefix));
    	});
    });
...
//			chkAttId1.setStyle('visibility', 'hidden');
	...
	var elemSelect = $('cmbType');
	if (elemSelect) {
		if (elemSelect.value != 'F') {
			$('moreEntOptions').style.display = 'none';
		}
	}
	...
	$("envEvtTab").addEvent("click", function() {
		//Ocultar campa�a
...
//		chkAttId1.setStyle('visibility', 'hidden');
	...
	$("cmbUseCache").addEvent("change", function(evt) {
		checkCache();
    });
	
	['txtHorIni'].each(setHourField);
	
	initAdminFieldOnChangeHighlight(false, false, false, null, true);
	initAdminRadioButtonOnChangeHighlight($$('.defBlock')[0], 'radIdePre');
	initAdminRadioButtonOnChangeHighlight($$('.defBlock')[1], 'radIdeNum');
	initAdminRadioButtonOnChangeHighlight($$('.defBlock')[2], 'radIdePos');
}

function checkCache(){
	var txt = $("txtBusEntCache");
	
	if($("cmbUseCache").value=="0"){
		txt.readOnly=true;
		txt.addClass("readonly");
		txt.value="";
		$('frmData').formChecker.dispose(txt);
		if (txt.hasClass("validate['required','digit','~checkLen99999']")){
			txt.removeClass("validate['required','digit','~checkLen99999']");
			txt.addClass("validate['digit','~checkLen99999']")
			
		}
	} else {
		txt.readOnly=false;
		txt.removeClass("readonly");
		$('frmData').formChecker.register(txt);
		if (txt.hasClass("validate['digit','~checkLen99999']")){
			txt.removeClass("validate['digit','~checkLen99999']");
			txt.addClass("validate['required','digit','~checkLen99999']");
		}
	}
}
...
function addNewLineTableAttBusEntRelation(attId, attName, entId, entName) {
	...
	[htmlAttName , htmlEntName].each(function(ele){
		ele.addEvent('optionSelected', function(visible,fromClick,fromEnter){
	...
	initAdminFieldOnChangeHighlight(false, false, false, tr, true);
}
...
function addAttributeBehaviors(txts, ids, chks, opts, adtFilters,redundant) {
	for (var i = 0; i < txts.length; i++) {
		...
		opt.addEvent('click', function(evt){
			...
			this.htmlTxt.removeClass("highlighted");
			this.htmlId.value = '';
...
function checkExistCubeName(cubeName,cubeId){
	var request = new Request({
		...
		url: CONTEXT + URL_REQUEST_AJAX +'?action=checkExistCbeName&isAjax=true' + TAB_ID_REQUEST,		
...
function continueCube(resXml){
	...
	var i = 0;
	var prfs = "";
	var divs = $("divNoAccProfiles").getElements("DIV.option");
	var i=0
	while (i<divs.length){
		var item = divs[i].getElement("input");
		if (item && "true" == item.getAttribute("flagNew")){
			if (prfs==""){
				prfs = item.getAttribute("profile");
			}else {
				prfs += ";" + item.getAttribute("profile");
			}			
		}
...
function loadStatus(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX +'?action=getStatuses&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { 
			processXMLStatus(resXml); sp.hide(true); 
			
			updateStatesEventCombo();
			initAdminModalHandlerOnChangeHighlight($('statusContainer'), true);
		}
	}).send();
}
...
function addActionElementStatus(container, text, id, helper, addRemove) {
	...
	new Element('input', { type: 'hidden', name: 'chkStatus', value: id}).inject(divElement);
	new Element('input', { type: 'hidden', name: 'staName', value: text}).inject(divElement);
...
function initFormTable(addName,removeName,upName,downName,tableName){
	...
	$(upName).addEvent("click", function(e) {
		...
		if(selectionCount($(tableName)) == 0) {
			...
			fixTableEnt($(tableName));
			if (row){
				if (Scroller1.v && tableName == 'tableDataFormEnt'){
					Scroller1.v.showElement(row);
				} else if (Scroller2.v && tableName == 'tableDataFormMonEnt'){
...
function loadFormMon(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX +'?action=getFormsMon&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { 
			processXMLFormsMon(resXml); sp.hide(true);
			
			var table = $('tableDataFormMonEnt');
			var footer = table.getParent('.fieldGroup').getElements('.listAddDel')[1];
...
function loadFormPanel(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX +'?action=getFormsPanel&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { 
			processXMLFormsPanel(resXml); sp.hide(true);
			
			var table = $('tableDataFormPanelEnt');
			var footer = table.getParent('.fieldGroup').getElements('.listAddDel')[2];
			var notification = new Element('div', {id : 'editionNot'}); 
			footer.grab(notification, "top");
...
function loadAttBusEntRelation(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX +'?action=getAttBusEntRelation&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
...
function loadForm(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX +'?action=getForms&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { 
			processXMLForms(resXml); sp.hide(true); 
			
			var table = $('tableDataFormEnt');
			var footer = table.getParent('.fieldGroup').getElement('.listAddDel');
			var notification = new Element('div', {id : 'editionNot'}); 
			footer.grab(notification, "top");
...
function loadProcess(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX +'?action=getProcesses&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { 
			processXMLProcesses(resXml); sp.hide(true); 
			
			initAdminModalHandlerOnChangeHighlight($('processContainer'), true);
		}
	}).send();
}
...
function processXMLProcesses(ajaxCallXml){
	if (ajaxCallXml != null) {		
		...
		if (envs != null && envs.length > 0 && envs.item(0) != null) {
			...
			//En caso que tengan procesos asociados, se deshabilita el tipo de 
			//administraci�n Funcionalidad
			enabledAdminTypeFunct(false);					
		}
	}
}

function enabledAdminTypeFunct(enable){
	var cmbType = $('cmbType');
	if (cmbType && cmbType.getChildren().length > 0){
		cmbType.getChildren().each(function(opt){
			if (opt.value==ADMIN_FUNCT){
				//Control por si no se pasa par�metro
				if (enable == undefined){
					var processes = $('processContainer').getChildren('.option');
					enable = !(processes && processes.length);				
				}
				
				opt.disabled=!enable;
				if (enable)
					opt.removeClass('locked');
				else
					opt.addClass('locked');
			}
		});		
...
function processProcessModalReturn(ret){
	ret.each(function(e){
		...
		//Al asociar un procesos, se deshabilita el tipo de administraci�n Funcionalidad
		enabledAdminTypeFunct(false);
	});
}
...
function addActionElementProcess(container, text, id,hidProVerId,helper,addRemove){
	...
	new Element('input',{ type: 'hidden', name: 'chkProc', value: id}).inject(divElement);
	new Element('input',{ type: 'hidden', name: 'hidProVerId', value: hidProVerId}).inject(divElement);
	 	
	if(addRemove){
		divElement.container = container;
		new Element('div.optionRemove').addEvent('click', warningDelete).inject(divElement);
	} else {
...
function warningDelete(evt) {
	...
	currentProc = evt.target;
	currentProc.addEvent("confirm", actionElementAdminClickRemove);
	
	panel.footer.innerHTML = "<div class='button' onClick=\"SYS_PANELS.closeAll(); currentProc.fireEvent('confirm'); enabledAdminTypeFunct();\">" + BTN_CONFIRM + "</div>";	
	SYS_PANELS.addClose(panel);
	SYS_PANELS.refresh();
}
...
function loadRelAnc(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX +'?action=getRelAnc&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
...
function loadRelDesc(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX +'?action=getRelDesc&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
...
function getWidgetDependency(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX +'?action=getWidgetDeps&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
...
function getCubesDependency(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX +'?action=getCubeDeps&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
...
function borrarAllInBean(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX +'?action=removeAllEntDwCol&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
...
function borrarAllProfiles(){
	...
	var divs = container.getElements("DIV.option");
		...
		if (xmlRoot.length == 0) {
			...
			for(i=0;i<rowset.childNodes.length;i++){
				...
				if (type=="entityRedAtts" || type == "entityFormAtts" || type=="entityForm" || type == "form"){
					objName = prefix + xRow.childNodes[1].innerHTML + " (" + xRow.childNodes[3].innerHTML;
					if (xRow.childNodes[2].firstChild != null){ //Si tiene descripcion la mostramos
						objName = objName + " - " + xRow.childNodes[2].innerHTML + ")";
					}else{
						objName = objName + ")";
					}
				}else if (type=="processJer") { 
					if (xRow.childNodes[4]!=null && 'true' == xRow.childNodes[4].firstChild.nodeValue){ // es subproceso
						...
						objName = prefix2 + xRow.childNodes[1].innerHTML + " (" + xRow.childNodes[3].innerHTML;
						if (xRow.childNodes[2].firstChild != null){ //Si tiene descripcion la mostramos
							objName = objName + " - " + xRow.childNodes[2].innerHTML + ")";
						}else{
							objName = objName + ")";
					...
					objName = prefix + xRow.childNodes[1].firstChild.nodeValue + " (" + xRow.childNodes[3].innerHTML;
					if (xRow.childNodes[2].firstChild != null){ //Si tiene descripcion la mostramos
						objName = objName + " - " + xRow.childNodes[2].innerHTML + ")";
					}else{
						objName = objName + ")";
...
function btnEstTime_click(){
	if (verifyCubeDataToEstimateTime()){
		...
		if (str == ""){
			...
			var request = new Request({
				method: 'post',
				url: CONTEXT + URL_REQUEST_AJAX +'?action=estimateTime&isAjax=true' + TAB_ID_REQUEST,
				onRequest: function() { sp.show(true); },
...
function changeRadFact(val){
	if (val == 1){		
    	...
    	$('btnEstTime').style.display='';
		$("radSelected").value = 1;		
		$("txtFchIni").getNext().disabled=true;
		$("txtHorIni").disabled=true;
		disposeValidation($("txtFchIni").getNext());
		...
		$("txtFchIni").getNext().disabled=false;
		disposeValidation($("txtFchIni"));
		registerValidation($("txtFchIni").getNext());
		registerValidation($("txtHorIni"), "validate['required','~validateHours']");
	}
} 
...
function loadNoAccProfiles(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX +'?action=loadCubeRestrictedProfiles&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { 
			processXMLCubeRestrictedProfiles(resXml); sp.hide(true);
			
			initAdminModalHandlerOnChangeHighlight($('divNoAccProfiles'));
		}
	}).send();
}
...
function addActionElementNoAccProfile(container, text, id, helper, addRemove, inputName,isNew){
	...
	new Element('div.profileNoAccData',{html:text}).inject(divElement);
	...
	editIcon.addEvent("click", function(e){
		...
		if ($("txtCbeName") == '' || $("txtCbeName").get('value') == ""){
			showMessage(MSG_CBE_NAME_MISS);
			return;
		}
...
//		divElement.addEvent("mouseleave", actionElementAdminMouseOverToggleClasss);
		...
		new Element('div.optionRemove').addEvent("click", function(e){
			var request = new Request({
				method: 'post',
				data:{'prfName':text},
				url: CONTEXT + URL_REQUEST_AJAX +'?action=removeNoAccProfile&isAjax=true' + TAB_ID_REQUEST,				
				onRequest: function() { sp.show(true); },
				onComplete: function(resText, resXml) { modalProcessXml(resXml);sp.hide(true);}
			}).send();
			
			var parent = this.getParent();
			if (parent.container && parent.container.onRemove) parent.container.onRemove();
			parent.destroy();
...
function loadProfiles(){
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX +'?action=loadCubeProfiles&isAjax=true' + TAB_ID_REQUEST,
		onRequest: function() { sp.show(true); },
		onComplete: function(resText, resXml) { 
			processXMLCubeProfiles(resXml); sp.hide(true); 

			initAdminModalHandlerOnChangeHighlight($('divProfiles'));
		}
	}).send();
}
...
function fixTableEnt(table){
	if (table){
		...
		if (table == $('tableDataFormEnt')){
			...
			Scroller3 = addScrollTable(table);	
		}		
	}	
}
...
function showMoreOptions(value) {
	if (value == 'F') {
		$('moreEntOptions').style.display = '';
	} else {
		$('moreEntOptions').style.display = 'none';
	}
}
