...
function loadFilters(){
	var request = new Request({
		...
		onComplete: function(resText, resXml) { 
			processXmlFilters(resXml);
			
			['containerDocType','containerUser'].each(function(id){ 
				initAdminModalHandlerOnChangeHighlight($(id)); 
			});
			
			['tsk','pro','ent','att','frm'].each(function(id){ 
				initAdminModalHandlerOnChangeHighlight($('containerSrcTit_'+id)); 
			})
			
			initAdminModalHandlerOnChangeHighlight($('containerSize'));
			initAdminModalHandlerOnChangeHighlight($('containerDate'));
			initAdminModalHandlerOnChangeHighlight($('containerRegInst'));
		}
	}).send();
}
...
function createFilDocType(id,name){
	if (!$('filDocType_'+id)){
		var filDocType = new Element("div.option",{'id': "filDocType_"+id, html: name, 'docTypeId': id}).inject($('addFilDocType'),"before");
...
function getFiltersDocType(){
	var ret = "";
	$('containerDocType').getElements("div.option").each(function(docType){
		if (ret != "") ret += ";";
		
		if(docType.getAttribute("docTypeId"))
...
function createFilSize(from,to){
	var div = new Element("div.option",{'data-helper':'true'}).inject($('containerSize'));
...
function createFilUser(usrLogin){
	if (!$('filUser_'+usrLogin)){
		var filSize = new Element("div.option",{'id': "filUser_"+usrLogin, html: usrLogin, usrLogin: usrLogin}).inject($('addFilUser'),"before");
...
function getFiltersUser(){
	var ret = "";
	$('containerUser').getElements("div.option").each(function(user){
		if (ret != "") ret += ";";
		if(user.getAttribute("usrLogin"))
...
function createFilDate(from,to){
	var div = new Element("div.option",{'data-helper':'true',styles:{'height':'29px'}}).inject($('containerDate'));
...
function createFilRegInst(regInst){
	var div = new Element("div.option",{'data-helper':'true',styles:{'height':'18px','cursor':'auto'}}).inject($('containerRegInst'));
...
function createFilSrcTit(id,name,type){
	...
	if (prefix != null){
		...
		if (!$(itemId)){ //no existia, se crea			
			...
			var filSrcTit = new Element("div.option",{'id': itemId, html: name}).inject(objAdd,"before");
...
function getFilSrcTit(){
	...
	containers.each(function(container){
		container.getElements("div.option").each(function(item){
			if(item.getAttribute("srcTitId")) {
				if (ret != "") ret += ";";
