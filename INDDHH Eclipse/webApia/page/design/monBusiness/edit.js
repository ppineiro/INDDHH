...
function loadExtraInformation() {
	var request = new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX + '?action=loadExtraInformation&isAjax=true' + TAB_ID_REQUEST,
		onComplete: function(resText, resXml) { 
			loadExtraInformationXml(resXml);
			
			['projectsContainter','entitiesContainter','processesContainter',
			 	'busEntInstancesContainter','proInstancesContainter'].each(function(id){ 
				initAdminModalHandlerOnChangeHighlight($(id)); 
			})
			
			
			var table = $('gridBodyQuery'); var footer = $('btnDeleteQuery');
			...
			var table = $('gridBodyInitFilter'); var footer = $('btnDeleteInitFilter');
			var notification = new Element('div', {id : 'editionNot'}); 
			footer.grab(notification, "top");
			initAdminModalHandlerOnChangeHighlight(table, true, false, notification);
...
function createNewInitFilter(id, name, title, type, defaultValue, required, hidden) {
	...
	initAdminFieldOnChangeHighlight(false, false, false, lastInitFilterAdded);
}
...
function createNewQuery(type, id, name, q1, q2, q3, q4, q5, q6) {
	...
	if (type == "E") {
		tds.push(new Element('td', {'html': "<span>"+TYPE_BUS_ENTITY_DESC+"</span>"}));
	} else if (type == "P") {
		tds.push(new Element('td', {'html': "<span>"+TYPE_PROCESS_DESC+"</span>"}));
	}
	...
	initAdminFieldOnChangeHighlight(false, false, false, lastQueryAdded);
}
