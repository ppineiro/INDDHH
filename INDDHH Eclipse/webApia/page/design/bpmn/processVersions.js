...
function initVersionsMdlPage() {
	...
	SYS_PANELS.fixFocus(mdlVersionsContainer);
}
	...
	if (ajaxCallXml != null) {
		...
		if (columns != null && columns.length > 0 && columns.item(0) != null) {
			...
			for(var i = 0; i < columns.length; i++) {
				...
				var tr = createRow(i,column.getAttribute("id"),column.getAttribute("date"),column.getAttribute("user"),column.getAttribute("comment"),i==(columns.length-1));
				...
				tr.addEvent('keydown', function(e) {
					if(e.key == 'down') {
						...
						if(prev) {
							...
							if(aux_prev && aux_prev.get('hasMoreInfo'))
								prev = aux_prev;
