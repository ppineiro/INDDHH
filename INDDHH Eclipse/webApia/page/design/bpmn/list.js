...
function initPage(){
	...
	$('optionInit').addEvent("click", function(e) {
		...
		if (isCountSelectedOk(false)){
			showModalConfirmWithOption(GNR_INIT_RECORD,0,MSG_INIT_DEL_BUS_ENT_INST);
		}			
	});
	//Versiones
...
function showModalConfirmWithOption(msg,action,optionalMsg){
	SYS_PANELS.newPanel();
	var panel = SYS_PANELS.getActive();
	panel.addClass("modalWarning");	
	var table='<table><tbody>'+
	'<tr><td class="content" colspan="2">'+msg+'</td></tr>'+
	'<tr><td colspan="2"><input type="checkbox" class="default" id="optional" name="optional">'+optionalMsg+'</td></tr>'+
	'</tbody></table>';
	panel.content.innerHTML = table;
	panel.footer.innerHTML = "<div class='button' onClick=\"processOptionalConfirm("+action+",this.getParent('.modalContent'));\">" + BTN_CONFIRM + "</div>";
	SYS_PANELS.addClose(panel);
	SYS_PANELS.refresh();
}

function processOptionalConfirm(action, panel){	
	var opt=panel.getElementById('optional');
	SYS_PANELS.closeAll();
	doActionConfirm(action,opt==null? false : opt.checked);
}

function doActionConfirm(action,optional){
	...
	for(i=0; i<selected.length; i++){
		...
		if(action == 'lockUnlockProc') {
			...
			if(img && img.get('src').contains('lock')) {
				uneditableTR = false;
			} else if (img != null && img[img.length - 1].get('src').contains('noEdit')) {
				showMessage(MSG_ELE_CANT_BLOCK, GNR_TIT_WARNING, 'modalWarning');
			}  else if (uneditableTR){
				showMessage(MSG_ELE_CANT_BLOCK, GNR_TIT_WARNING, 'modalWarning');
			}
		} 
	...
	if(selection != "" || action == 'importXPDL') {
		var request = new Request({
			method: 'post',
			url: CONTEXT + URL_REQUEST_AJAX + '?action=' + action + (optional?'&optional=true':'') + '&isAjax=true&id=' + selection + TAB_ID_REQUEST,
