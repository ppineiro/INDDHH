...
function loadMonitorForms(){
	var request = new Request({
		...
		onComplete : function(resText, resXml) { 
			processMonitorFormsXml(resXml); 
			
			initAdminModalHandlerOnChangeHighlight($('monitorFormsContainer'));	
		}
	}).send();
}
...
function createMonitorForm(id,name){
	if (!($("form_"+id))){		
		...
		var form = new Element("div",{'id': "form_"+id, html: name, 'class': 'option optionWidthAll'}).inject(container);
...
//		form.addEvent("click",function(evt){ this.destroy(); evt.stopPropagation(); });
		...
		new Element('div.optionRemove').addEvent("click", function(evt){ this.getParent().destroy(); }).inject(form);
