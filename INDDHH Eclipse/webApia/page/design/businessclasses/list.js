function initPage(){
	...
	var btnUpload  = $('btnUpCre');
	if (btnUpload) {
		btnUpload.addEvent('click', function(evt) {
			evt.stop();
			var request = new Request({
				method: 'post',
				url: CONTEXT + URL_REQUEST_AJAX+'?action=ajaxUploadStart&isAjax=true&uploadAndCreate=true&' + TAB_ID_REQUEST,
				onRequest: function() { SYS_PANELS.showLoading(); },
	...
	if (btnUpdate) {
		...
		btnUpdate.addEvent("click", function(e) {
			...
			if (selectionCount($('tableData')) > 1) {
				...
				var uneditableTR_WS = getSelectedRows($('tableData'))[0].get('uneditableTR_WS')=='true'?true:false;
				...
				} else if (uneditableTR_WS) {
					showConfirm(MSG_NO_UPDATE_WS_PUB,GNR_TIT_WARNING, function(ret) {  
						if (ret) {
							ADMIN_SPINNER.show(true);
							var id = getSelectedRows($('tableData'))[0].getRowId();
							window.location = CONTEXT + URL_REQUEST_AJAX + '?action=update&onlyRead=true&id=' + id + TAB_ID_REQUEST;
