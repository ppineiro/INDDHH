...
function showType(t){
	...
	}else if (t==TYPE_WS_REST){
		$("divH").style.display='';	
		loadPar=true;
		registerValidation($('txtUri'));
		$("btnTestRest").style.display='';
		$("panelOptions").style.display='';
	...
	if (t!=TYPE_QRY_FIL){
		if (!isC){
			var headerId = isRest? "trHR" : "trHNC";
			$(headerId).style.display='';
			$(headerId).addClass('header');
			$("trHC").removeClass('header');
			$(isRest? "trHNC" : "trHR").removeClass('header');
			$("trBNC").style.display='';
...
divs.push("btnTest");
divs.push("btnTestBR");
divs.push("btnTestRest");
divs.push("btnTestQuery");
divs.push("btnShowExInfo");
divs.push("divH");
divs.push("trHR");
...
function loadParams(){
	...
	if (v!=null || (TYPE_SELECTED!=TYPE_DB && TYPE_SELECTED!=TYPE_DB_PROC)){
		...
		var type = TYPE_SELECTED!=''?TYPE_SELECTED:$('busType').options[$('busType').selectedIndex].value;
		var request = new Request({
			...
			onComplete: function(resText, resXml) { 
				loadTableParams(resXml); 

				if (firstLoad){
					firstLoad=false;
					if (type == TYPE_DB){
						addElementsOnChangeHighlight($$('#connectionViews'));
					} else if (type == TYPE_DB_PROC){
						addElementsOnChangeHighlight($$('#connectionProc'));
					}	
				}
			}
		}).send();
...
function initPage(){
	...
	if(JS_EXECUTABLE != "")
		$('txtExeScript').set('value', JS_EXECUTABLE.substring(currentEnvironmentName.length + 1));
	...
	$('btnBackToList').addEvent("click", function(e) {
		...
		var btnConf = $('btnConf');
		if (btnConf && isAnyElementModified()){
			SYS_PANELS.newPanel();
	...
	*/
	...
	$('btnTestRest').addEvent('click', function(){
		...
		height = (height*75)/100; //80%
		
		var url = REST_TEST_MODAL + '?' + TAB_ID_REQUEST;
		
		ModalController
			.openWinModal(url, width, height, undefined, undefined, false, true, false)
			.setCloseLabel(LBL_CLOSE);
	})
	
	var btnInfo = $('btnShowExInfo');
	if (btnInfo) {
		btnInfo.addEvent('click', function(evt) {
			if (evt) evt.stop();
			var executable = $('txtExeJava').value;
			var request = new Request({
				method: 'post',
				url: CONTEXT + URL_REQUEST_AJAX + '?action=getClassInfo&isAjax=true' + TAB_ID_REQUEST,
				onRequest: function() { SYS_PANELS.showLoading(); },
				onComplete: function(resText, resXml) { modalProcessXml(resXml); }
			}).send('exe=' + executable);
	...
	*/ 
	...
	addElementsOnChangeHighlight($$('#txtDesc'));
	...
	//Se setea según corresponda columna de parámetros para servicios Rest
	initRestParamPathColumn();
}
...
function btnActWsdl_click(){	
	var request = new Request({
		...
		onRequest: function() { SYS_PANELS.showLoading(); },
		onComplete: function(resText, resXml) { loadTableParams(resXml); addScrollTableParams(); }
	}).send();	
}
...
function loadTableParams(ajaxXml){
		...
		for(var i = 0; i < rows.length; i++) {
			...
			for(var j = 0; j < cells.length; j++) {
				...
				if (isCombo){
					...
					if (elements.getAttribute("id") == "cmbParType"){
					...
					} else if (elements.getAttribute("id") == "cmbParInOut" && TYPE_SELECTED==TYPE_WS_REST){						
						var inputDesc = tr.getElementById('txtParDesc');
						var divDesc = inputDesc.getParent();
						registerElement(selector.getProperty('value')!='I', inputDesc, divDesc);
						...
						selector.addEvent('change',function(e){
							...
							var inputDesc = tr.getElementById('txtParDesc');
							var divDesc = inputDesc.getParent();
							registerElement(this.getProperty('value')!='I', inputDesc, divDesc);
						})
					}
					...
					var forceReadOnly = toBoolean(cell.getAttribute("forceReadOnly"));
					var maxLength = data.getAttribute('maxlength');
					...
					if (tdClassToAdd  || isRequired){
						if (tdClassToAdd=="validate['required','~validName']" || isRequired){
					...
					if (tdWidths) {
						...
						var div = new Element('div.min_text', {styles: {width: tdWidths[j]}});
						if (additionalInfo) additionalInfo.inject(div);
						new Element('span', {html: content, title: content}).inject(div);
						div.inject(td);						
	...
	initAdminFieldOnChangeHighlight(false,false,false,object);
}
...
function addRow() {
	...
	inputName.setProperty("maxlength","50");
	...
	inputDesc.setProperty('style','width:93%');
	inputDesc.inject(divDesc);
	if (TYPE_SELECTED==TYPE_WS_REST) inputDesc.setAttribute('disabled','');
	...
	if (TYPE_SELECTED!=TYPE_SCR){
		...
		if (TYPE_SELECTED==TYPE_WS_REST){
			selectInOut.addEvent('change',function(e){
				e.stop();
				registerElement(this.getProperty('value')!='I', inputDesc, divDesc);
			})
		}
	...
	initAdminFieldOnChangeHighlight(false,false,false,row);
}
...
function registerElement(register, element, container){
	var aster = container.getElementById('aster');
	
	if (register){
		element.addClass("validate['required']");
		registerValidation(element);
		if (!aster) {
			new Element("span",{id:'aster', html:"&nbsp;*", styles:{'float':'left'}}).inject(container);
		}
		element.removeAttribute('disabled','');
	} else {
		element.removeClass("validate['required']");
		disposeValidation(element);
		if (aster) {
			container.getElementById('aster').dispose();
		}
...
function initRestParamPathColumn(){
	var comboRestType = $('cmbRestTypeTag');
	if (comboRestType) {
		var pathParamColumn=$('trHR').getElementById('path');
		var typeLabel = comboRestType.value=='J'? LBL_JSON_PATH : LBL_XPATH;  
		pathParamColumn.title=typeLabel;
		pathParamColumn.getElement('div').textContent=typeLabel;
		
		comboRestType.addEvent('change', function(e){
			e.stop();
			
			var typeLabel = this.value=='J'? LBL_JSON_PATH : LBL_XPATH;  
			pathParamColumn.title=typeLabel;
			pathParamColumn.getElement('div').textContent=typeLabel;
		})
	}
