...
function initFncMdlPage(){
	...
	['orderByFncFolderMdl','orderByFncNameMdl'].each(function(ele){
		$(ele).addEvent("click", function(e) {
			e.stop();
			callNavigateOrder(this.getAttribute('data-sortBy'),this, '/apia.modals.FunctionalitiesAction.run','Fnc');
		});
	});
	
	window.sp_Fnc = new Spinner($('tableDataFnc').getParent('table'), {message: WAIT_A_SECOND});
...
function removeOrderByClass(obj){
	$('functionalitiesTrOrderBy').getElements(".orderedBy").each(function(item, index){
	    item.removeClass("orderedBy");
	});
	
	$('functionalitiesTrOrderBy').getElements(".sortUp").each(function(item, index){
		if(obj!=item){
	...
	$('functionalitiesTrOrderBy').getElements(".sortDown").each(function(item, index){
		if(obj!=item){
