...
function initCalendarViewMdlPage(){
	...
	spModalCal = new Spinner($('mdlBodyCalendar'),{message:WAIT_A_SECOND});
...
function cleanModal(){
	$("calName").getElements("span.monitor-input").each(function(span){ span.destroy(); });
	$("calDesc").getElements("span.monitor-input").each(function(span){ span.destroy(); });
...
function modalProcessDataXml(resXml){
	...
	if (calendar != null && calendar.length > 0 && calendar.item(0) != null) {
		...
		if (calName != null && calName != ""){
			var spanName = new Element("span.monitor-input", { html: calName }).inject($("calName"));
//			spanName.setStyle("font-weight","bold");
		}
		var calDesc = generalInfo.getAttribute("description");
		if (calDesc != null && calDesc != ""){
			var spanDesc = new Element("span.monitor-input", { html: calDesc }).inject($("calDesc"));
//			spanDesc.setStyle("font-weight","bold");
		...
		if (labDays != null){
			for(var i = 0; i < labDays.length; i++) {
				...
				var allDay = day.getAttribute("allDay")=="true";
				...
				td1.setStyle("width","40%");
				if (allDay){
					var td2 = new Element("td",{html: LBL_ALL_DAY}).inject(tr);
				} else {
