...
function initBusClaParMdlPage(){
	...
	window.sp_BusClaPar = new Spinner($('tableDataBusClaPar').getParent('table'), {message: WAIT_A_SECOND});
...
function removeOrderByClass(obj){
	$('busClassParamsTrOrderBy').getElements(".orderedBy").each(function(item, index){
	    item.removeClass("orderedBy");
	});
	
	$('busClassParamsTrOrderBy').getElements(".sortUp").each(function(item, index){
		if(obj!=item){
	...
	$('busClassParamsTrOrderBy').getElements(".sortDown").each(function(item, index){
		if(obj!=item){
