...
function initEntInstMdlPage(){
	...
	if (!mdlEntInstContainer || mdlEntInstContainer.initDone) return;
...
function removeOrderByClass(obj){
	$('entityInstanceTrOrderBy').getElements(".orderedBy").each(function(item, index){
	    item.removeClass("orderedBy");
	});
	
	$('entityInstanceTrOrderBy').getElements(".sortUp").each(function(item, index){
		if(obj!=item){
	...
	$('entityInstanceTrOrderBy').getElements(".sortDown").each(function(item, index){
		if(obj!=item){
...
function showEntInstModal(retFunction, closeFunction){
	...
	mdlEntInstContainer.blockerModal.element.setStyle('zIndex', SYS_PANELS.getNewZIndex());
