...
function initQueryMdlPage(){
	...
	window.sp_Qry = new Spinner($('tableDataQry').getParent('table'), {message: WAIT_A_SECOND});
...
function removeOrderByClass(obj){
	$('queryTrOrderBy').getElements(".orderedBy").each(function(item, index){
	    item.removeClass("orderedBy");
	});
	
	$('queryTrOrderBy').getElements(".sortUp").each(function(item, index){
		if(obj!=item){
	...
	$('queryTrOrderBy').getElements(".sortDown").each(function(item, index){
		if(obj!=item){
