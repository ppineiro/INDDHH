<%@include file="../includes/startInc.jsp" %><html lang="<%=biz.statum.apia.web.bean.BasicBeanStatic.getUserDataStatic(request).getLangCode()%>"><head><%@include file="../includes/headInclude.jsp" %><script type="text/javascript" src="<system:util show="context" />/page/modals/jpivotViewFilters.js"></script></head><script type="text/javascript">
	/*
	...
	*/
				...
				if(resText != "") {
					...
					if(doc && doc.childNodes) {
						...
						if(doc.childNodes[2] && doc.childNodes[2].textContent && doc.childNodes[2].textContent != "null") {
							var par2 = doc.childNodes[2].textContent;
							par2 = par2.replace(/_pct_/g, '%');

							//Cargar parametro 2
							$('inpPar2').set('value', par2);
						}
					}
				}
			}
	...
	function getModalReturnValue() {
		...
		if(claName) {
			...
			if(res == "OK") {
 				var par2 = document.getElementById("inpPar2").value;
 				par2 = par2.replace(/%/g, '_pct_');
 				return [document.getElementById("inpCla").value, document.getElementById("inpPar1").value, par2];
			} else alert(res);
		} else {
