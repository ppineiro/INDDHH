...
function initEntMdlPage(){
	...
	['orderByNameEntMdl'].each(function(ele){
		$(ele).addEvent("click", function(e) {
			e.stop();
			currentPrefix = 'Ent';
			callNavigateOrder(this.getAttribute('data-sortBy'),this,'/apia.modals.EntitiesAction.run','Ent');
		});
	});
	
	window.sp_Ent = new Spinner($('tableDataEnt').getParent('table'), {message: WAIT_A_SECOND});
...
function removeOrderByClass(obj){
	$('entitiesTrOrderBy').getElements(".orderedBy").each(function(item, index){
	    item.removeClass("orderedBy");
	});
	
	$('entitiesTrOrderBy').getElements(".sortUp").each(function(item, index){
		if(obj!=item){
	...
	$('entitiesTrOrderBy').getElements(".sortDown").each(function(item, index){
		if(obj!=item){
