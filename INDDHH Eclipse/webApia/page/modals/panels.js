...
function initPanelMdlPage(){
	...
	['orderByNamePnlMdl','orderByTitlePnlMdl','orderByTypePnlMdl'].each(function(ele){
		$(ele).addEvent("click", function(e) {
			e.stop();
			callNavigateOrder(this.getAttribute('data-sortBy'),this,'/apia.modals.PanelsAction.run','Panel');
		});
	});
	
	window.sp_Panel = new Spinner($('tableDataPanel').getParent('table'), {message: WAIT_A_SECOND});
...
function removeOrderByClass(obj){
	$('panelsTrOrderBy').getElements(".orderedBy").each(function(item, index){
	    item.removeClass("orderedBy");
	});
	
	$('panelsTrOrderBy').getElements(".sortUp").each(function(item, index){
		if(obj!=item){
	...
	$('panelsTrOrderBy').getElements(".sortDown").each(function(item, index){
		if(obj!=item){
