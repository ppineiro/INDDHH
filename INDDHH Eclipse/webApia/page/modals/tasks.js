...
function initTaskMdlPage(){
	...
	['orderByNameTaskMdl', 'orderByTitleTaskMdl'].each(function(ele){
		$(ele).addEvent("click", function(e) {
			e.stop();
			currentPrefix = 'Task';
			callNavigateOrder(this.getAttribute('data-sortBy'),this,'/apia.modals.TasksAction.run','Task');
		});
	});
	
	window.sp_Task = new Spinner($('tableDataTask').getParent('table'), {message: WAIT_A_SECOND});
...
function removeOrderByClass(obj){
	$('tasksTrOrderBy').getElements(".orderedBy").each(function(item, index){
	    item.removeClass("orderedBy");
	});
	
	$('tasksTrOrderBy').getElements(".sortUp").each(function(item, index){
		if(obj!=item){
	...
	$('tasksTrOrderBy').getElements(".sortDown").each(function(item, index){
		if(obj!=item){
