...
function initStaMdlPage(){
	...
	['orderByNameStaMdl'].each(function(ele){
		$(ele).addEvent("click", function(e) {
			e.stop();
			currentPrefix = 'Sta';
			callNavigateOrder(this.getAttribute('data-sortBy'),this,'/apia.modals.StatusAction.run','Sta');
		});
	});
	
	window.sp_Sta = new Spinner($('tableDataSta').getParent('table'), {message: WAIT_A_SECOND});
...
function removeOrderByClass(obj){
	$('statusTrOrderBy').getElements(".orderedBy").each(function(item, index){
	    item.removeClass("orderedBy");
	});
	
	$('statusTrOrderBy').getElements(".sortUp").each(function(item, index){
		if(obj!=item){
	...
	$('statusTrOrderBy').getElements(".sortDown").each(function(item, index){
		if(obj!=item){
