...
function initCatMdlPage(){
	...
	$('tableDataCat').getParent().addEvent("dblclick", function(e) {
		if (mdlCatContainer.onModalConfirm) jsCaller(mdlCatContainer.onModalConfirm,getSelectedRows($('tableDataCat')));
		closeCategoriesModal();
	});
	...
	['orderByNameCatMdl'].each(function(ele){
		$(ele).addEvent("click", function(e) {
			e.stop();
			currentPrefix = 'Cat';
			callNavigateOrder(this.getAttribute('data-sortBy'),this,'/apia.modals.CategoriesAction.run','Cat');
		});
	});
	
	window.sp_Cat = new Spinner($('tableDataCat').getParent('table'), {message: WAIT_A_SECOND});
...
function removeOrderByClass(obj){
	$('categoriesTrOrderBy').getElements(".orderedBy").each(function(item, index){
	    item.removeClass("orderedBy");
	});
	
	$('categoriesTrOrderBy').getElements(".sortUp").each(function(item, index){
		if(obj!=item){
	...
	$('categoriesTrOrderBy').getElements(".sortDown").each(function(item, index){
		if(obj!=item){
