...
function initBusClaMdlPage(){
	...
	['orderByNameBusClaMdl','orderByDescBusClaMdl'].each(function(ele){
		$(ele).addEvent("click", function(e) {
			...
			callNavigateOrder(this.getAttribute('data-sortBy'),this,'/apia.modals.BusClassAction.run','BusCla');
		});
	});
	
	window.sp_BusCla = new Spinner($('tableDataBusCla').getParent('table'), {message: WAIT_A_SECOND});
...
function removeOrderByClass(obj){
	$('busClassesTrOrderBy').getElements(".orderedBy").each(function(item, index){
	    item.removeClass("orderedBy");
	});
	
	$('busClassesTrOrderBy').getElements(".sortUp").each(function(item, index){
		if(obj!=item){
	...
	$('busClassesTrOrderBy').getElements(".sortDown").each(function(item, index){
		if(obj!=item){
