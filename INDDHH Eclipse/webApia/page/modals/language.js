...
function initLangMdlPage(){
	...
	['orderByNameLangMdl'].each(function(ele) {
		$(ele).addEvent("click", function(e) {
			e.stop();
			currentPrefix = 'Lang';
			callNavigateOrder(this.get('data-sortBy'), this, '/apia.modals.LanguageAction.run', 'Lang');
		});
	});
	
	window.sp_Lang = new Spinner($('tableDataLang').getParent('table'), {message: WAIT_A_SECOND});
...
function removeOrderByClass(obj){
	$('languagesTrOrderBy').getElements(".orderedBy").each(function(item, index){
	    item.removeClass("orderedBy");
	});
	
	$('languagesTrOrderBy').getElements(".sortUp").each(function(item, index){
		if(obj!=item){
	...
	$('languagesTrOrderBy').getElements(".sortDown").each(function(item, index) {
		if(obj!=item){
