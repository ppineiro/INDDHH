...
function initSocialShareMdlPage(sourceBtn){
	...
	mdlSocialShareContainer.sourceBtn = sourceBtn;
	mdlSocialShareContainer.blockerModal = new Mask();
	
	spModalSocShare = new Spinner($('mdlBodyChn'),{message:WAIT_A_SECOND});
		...
		if (txtToPub != ''){
			...
			if (strChannels != null){ //se verifica que se marque al menos un canal
				...
				if(PRIMARY_SEPARATOR_IN_BODY) {
					...
					new Request({
						method : 'post',
						url : CONTEXT + URL_REQUEST_AJAX + '?action=publishSocialMessage&isAjax=true&channels=' + encodeURIComponent(strChannels)  + TAB_ID_REQUEST,
						onRequest : function() { SYS_PANELS.showLoading(); },
						onComplete : function(resText, resXml) { modalProcessXml(resXml); }
					}).send(txtToPub);
				}
	...
	SYS_PANELS.fixFocus(mdlSocialShareContainer);
}
...
function initModalData(channels){
	...
	if (channels != null){
		for (var i = 0; i < channels.length; i++){
			...
			new Element("label",{'class':'label',text:chnName,'title':chnName}).inject(fieldHalfMdl);
		}
	}
...
function closeSocialShareModal(){
    ...
    if(mdlSocialShareContainer.sourceBtn)
    	mdlSocialShareContainer.sourceBtn.focus();
}
