<%@page import="com.dogma.vo.TaskVo"%><%@page import="com.dogma.vo.TskLanguagesVo"%><%@page import="com.dogma.vo.LanguageVo"%><%@page import="java.util.ArrayList"%><%@page import="java.util.Collection"%><%@page import="biz.statum.apia.web.bean.monitor.ProcessesBean"%><%@page import="biz.statum.sdk.util.HttpServletRequestResponse"%><%@page import="biz.statum.apia.web.action.monitor.ProcessesAction"%><div class="tabContent"><%
...
ProcessesBean tBean = ProcessesAction.staticRetrieveBean(http);
...
// request.getAttribute("tskTradLang", tskTradLang);
// System.out.println("Escribiendo los tskTradLang en taskDetailForms.jsp");
...
while(!stop){
	...
	String id = request.getParameter("frmParent") + "_" + fBean.getFormDefinition().getFrmId();
	...
	%><div id="<%=id%>" class="formContainer" data-xml="<%out.print(fBean.getFullXML(request, response, tskTradLang));%>"></div><%
