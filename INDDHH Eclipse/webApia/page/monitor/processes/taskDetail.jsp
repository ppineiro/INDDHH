...
<%@page import="biz.statum.apia.web.bean.monitor.ProcessesBean"%>
<%@page import="biz.statum.apia.web.action.monitor.ProcessesAction"%>
...
if (dBean.getTaskVo()!=null && dBean.getTaskVo().getTskExeTemplate() != null && !dBean.getTaskVo().getTskExeTemplate().equals("")) {
...
dBean.processForm(request, response, EntInstanceBean.BEAN_TYPE_ENTITY);
dBean.processForm(request, response, ProInstanceBean.BEAN_TYPE_PROCESS);
...
<region:render template='<%=template%>'>
		<region:put section='title'>
			...
			<%}else {%>
				<img title="<system:label show="text" label="lblEjeTitPriPro2"/>" alt="<system:label show="text" label="lblEjeTitPriPro2"/>" src="<system:util show="context" />/css/base/img/priority2.gif"  >
			<%}%>
			<span id="titleSpan"><%= fromProcess ? com.st.util.StringUtil.escapeHTML(dBean.getProNameDetails()) : BeanUtils.fmtHTML(TranslationManager.getTaskTitle(dBean.getCurrentElement(), uData.getEnvironmentId(), uData.getLangId())) %></span>
		</region:put>
		
		<region:put section="tskDescription">
			<%=fromProcess ? com.st.util.StringUtil.escapeHTML(dBean.getProInstanceBean().getProcess().getProDesc()) : BeanUtils.fmtHTML(TranslationManager.getTaskDescription(dBean.getTaskVo().getTskName(), dBean.getTaskVo().getTskDesc(), uData.getEnvironmentId(), uData.getLangId())) %>
		&nbsp;</region:put>
		
		<region:put section="taskImage">
			<%if(fromProcess) {%>
				<%if(dBean.getProInstanceBean().getProcess().getImgPath() != null){%>
					<img src="<system:util show="context" />/images/uploaded/<%=dBean.getProInstanceBean().getProcess().getImgPath()%>" style="height: 64px; width: 64px;">
				<%} else {%>
					<img src="<system:util show="context" />/css/base/img/functionalities/Procesos.gif" style="height: 64px; width: 64px;">
				<%} %>
			<%} else { %>
				...
				<%} else {%>
					<img src="<system:util show="context" />/css/base/img/functionalities/Tareas.gif" style="height: 64px; width: 64px;">
				<%} %>
			<%} %>
		</region:put>
	
		<region:put section='entityMain'><%@include file="/page/execution/includes/entityMain.jsp" %></region:put>
		<region:put section='entityDocuments' content="/page/execution/includes/documents.jsp?frmParent=E"/>
		<region:put section='processMain'><%@include file="/page/execution/includes/processMain.jsp" %></region:put>
		...
		<region:put section='taskComments'><%@include file="/page/execution/includes/taskComments.jsp" %></region:put>
		...
		<region:put section='scripts_include'>			
			...
			<script type="text/javascript" src="<system:util show="context" />/page/monitor/processes/taskDetail.js"></script>
		...
		<region:put section='buttons'>
			<div id="divAdminActEdit" class="fncPanel buttons">
				...
				<div class="content">
					<div id="btnPrint" class="button extendedSize" title="<system:label show="tooltip" label="btnPrint"/>"><system:label show="tooltip" label="btnPrint"/></div>
					<div id="btnBack" class="button extendedSize" title="<system:label show="tooltip" label="btnVol"/>"><system:label show="tooltip" label="btnVol"/></div>
				</div>
			</div>&nbsp;
		</region:put>
		...
		<region:put section="helpDocuments">
			...
			<div class="content">
				<div id="btnViewDocs" class="button extendedSize" title="<system:label show="tooltip" label="sbtEjeDoc" />"><system:label show="text" label="sbtEjeDoc" /></div>
			</div>
