...
//				tabContainer.addNewTab(LABEL_MONITOR_PROCESS,url,null);
	...
	['orderByProPriority','orderByTskTit','orderByProTit','orderByMonInstProNroReg','orderByMonInstProSta'].each(setAdmListTitle);
	...
	['createDateFilterStart', 'createDateFilterEnd', 'endDateFilterStart', 'endDateFilterEnd', 'txtReaSta', 'txtReaEnd', 'txtEndSta',
	 'txtEndEnd', 'alertDateFilterStart', 'alertDateFilterEnd', 'overdueDateFilterStart', 'overdueDateFilterEnd'].each(setDateFilters);	
...
function loadTasksByProcess(cmbProcTitFilter){
	...
	if (cmbValue != null && cmbValue != ""){
		...
		}).send("txtProTitle="+$('cmbProcTitFilter').value);
	}
}
