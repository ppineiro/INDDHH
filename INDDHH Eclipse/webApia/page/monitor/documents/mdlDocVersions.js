...
function showDocVersionsModal(docId, downloadDocId){   
	var mdlDocVersionsContainer = $('mdlDocVersionsContainer');
	...
	mdlDocVersionsContainer.downloadDocId = downloadDocId;
	...
	if (information != null && information.length > 0 && information.item(0) != null) {
		...
		if (versions != null && versions.length > 0 && versions.item(0) != null) {
			...
			for (var i = 0; i < versions.length; i++){
				...
				tr.setAttribute("rowId",xmlVer.getAttribute("value"));
				...
				var div1 = new Element('div', {styles: {width: '80px', overflow: 'hidden', 'white-space': 'pre', 'margin': '4px 0px'}}).inject(td1);
...
function downloadDocVersion(docVerId){
	var mdlDocVersionsContainer = $('mdlDocVersionsContainer');
	var docId = mdlDocVersionsContainer.downloadDocId;
		
	//CAM_12630
	new Request({
		method: 'post',
		url: CONTEXT + URL_REQUEST_AJAX + '?action=loadDownloadDocument&isAjax=true&id='+encodeURIComponent(docId)+'&version='+docVerId+ TAB_ID_REQUEST,
