...
%><!DOCTYPE html><%@page import="java.util.Enumeration"%><%@page import="com.st.util.StringUtil"%><html lang="<%=biz.statum.apia.web.bean.BasicBeanStatic.getUserDataStatic(request).getLangCode()%>"><head></head><body onload="initPage()"><%  out.print("<TEXTAREA id=errorText style='display:none'>"+ errorMsg + "</TEXTAREA>");
			...
			function init(){
				...
				document.getElementById("workArea").src="<%=Parameters.ROOT_PATH%>/apia.security.LoginAction.run?action=redirectToQuery&external=true&txtUser=<%=user%>&txtPwd=<%=pass%>&logFromSession=<%=logFromSession%>&logFromFile=<%=logFromFile%>&cmbEnv=<%=env%>&hidLangId=<%=langId%>&query=<%=qryId%>&onlyGrid=<%=onlyGrid%>&onlyChart=<%=onlyChart%>&onFinish=<%=onFinish%><%=filters%>&sessionAtts=<%=sessionAtts%>&logged=<%=logged%>&tokenId=<%=tokenId%>";
