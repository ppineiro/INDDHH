...
function FORMAS_DOCUMENTALES_CONFIRMAR_MODIF_CONF(evtSource, par_desdeCaratula, par_mensaje) { 
	...
	if (ApiaFunctions.getForm("ACTUACIONES_HISTORIAL") || task != "REALIZAR ACTUACION"){
			...
			if(par_desdeCaratula == "H"){
				...
				if(strTipoAct.search(/^ag\b/i) == 0 || strTipoAct.search(/^inicio de tramite$/i) == 0){
					if(vIndex > 0){
						...
						alert(obtenerMensajeMultilenguajeSegunCodigo('MSG_IMPOSIBLE_CAMBIO_CONF_CARATULA_JS',currentLanguage)); //alert("No se puede modificar la confidencialidad a la car�tula");
					}
					doRollback = true;
					showMessage = false;
				}else{
		...
		alert("La versi\u00f3n de este expediente es vieja, no podr\u00e1 cambiar el acceso restringido de la actuaci\u00f3n hasta que no act\u00fae al menos una vez. Se sugiere hacer un autopase indicando esta situaci\u00f3n.")
		if (evtSource.getValue() == "1"){
			evtSource.setValue("2");
		}else{
			if (evtSource.getValue() == "2"){
