...
function CIUDADANO_JS_RESTRINGIR_VALOR_A_NUMERICO(evtSource, par_form, par_attrib, par_cantNumeros) { 
...
if (par_cantNumeros != null) {
	tope = par_cantNumeros;
  	if (!sonNumeros(strValue)) {  		
		...
		showMsgError(par_form, par_attrib, "El campo debe ser num&eacute;rio.");
     	return;
	} else if (strValue.length > tope) {
		ApiaFunctions.getForm(par_form).getField(par_attrib).clearValue();
