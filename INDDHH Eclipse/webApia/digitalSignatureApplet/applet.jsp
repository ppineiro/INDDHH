<%@page import="com.apia.core.CoreFacade"%><%@page import="biz.statum.apia.web.bean.BasicBeanStatic"%><%@ taglib prefix="system"	uri="/WEB-INF/system-tags.tld" %><%@page import="com.st.util.labels.LabelManager"%><%@page import="com.dogma.UserData"%><%@page import="java.util.Iterator"%><%@page import="sun.misc.BASE64Encoder"%><%@page import="com.dogma.Parameters"%><%@page import="java.util.HashMap"%><%@page import="biz.statum.sdk.util.StringUtil"%><%@page import="com.dogma.DogmaException"%><%@page import="com.st.util.log.Log"%><html lang="<%=BasicBeanStatic.getUserDataStatic(request).getLangCode()%>" style="overflow: hidden;" ><head><%
...
// 					"<button type='button' id='btnNext' disabled='true' style='display:none;' onclick='btnNext_click()' title='" + LabelManager.getToolTip(labelSet,"btnNextSign") + "'>" + LabelManager.getName(uData,"btnNextSign") + "</button>" +
		...
		while(it.hasNext()){
			...
			if(key.endsWith("_EMBEDDED")) {
				strApplet.append("<param name='is_pdf_" + i + "' value='true'>");
				strApplet.append("<param name='out_url_" + i + "' value='/send.AppletReceiveFile'>");
			} else {
				try{
		...
		strApplet.append("<param name='algoritmo' value='"+Parameters.DIGITAL_SIGN_ALG+"'>");
		strApplet.append("<param name='key_factor' value='"+Parameters.DIGITAL_SIGN_KEY_FACTOR+"'>");
		strApplet.append("<param name='digest' value='"+Parameters.DIGITAL_SIGN_DIGEST+"'>");
		strApplet.append("<PARAM name='java_arguments' value='-Xmx512m'>");
		...
		strApplet.append("<div id='div-applet' class='div-applet modalContent'>" +
				"<div class='header'><span>" + LabelManager.getName(uData,"lblAppletTitl") + "</span></div>" +
				"<div id='divContent' class='content'><div id='divContentMsg'><p><strong>"+LabelManager.getName(uData,"lblAppletCantSign") + "</strong></p></div></div>" +
				"<div class='footer'>" +
					"<div tabIndex='4' onclick='window.parent.appletCloser()' class='close' id='btnExit' title='" + LabelManager.getToolTip(labelSet,"btnExitSign") + "'>" + LabelManager.getName(uData,"btnExitSign") + "</div>" +
				"</div>" +
		"</div>");
	}
