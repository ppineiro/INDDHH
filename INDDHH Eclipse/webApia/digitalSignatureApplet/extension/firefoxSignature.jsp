...
var params = {
	...
	lblSignWrongVersion: "<%
		...
		toks.add("<a href='apiaDigitalSignatureFirefoxInstaller.jar' download>");
		toks.add("</a>");
	...
	lblNativeAppNotFound: "<%	
		out.write(LabelManager.getName(uData,"msgNativeAppNotFound") + "TOKBR");
		
		out.write(LabelManager.getName(uData,"msgDownloadSignInst").replaceAll("[<>]",""));
	%>",
...
setInterval(function() {
	...
	if(div && !div.get('processed')) {
		div.addEvent('keydown', function(e) {
 		...
 		toks = new ArrayList<String>();
 		toks.add("<strong>about:addons</strong>");
 		 		
 		out.write(StringUtil.parseMessage(LabelManager.getName(uData,"msgVerifyEnabFFExt"), toks) + "<br/>");
 		
 		toks = new ArrayList<String>();
 		toks.add("<a href=\"https://addons.mozilla.org/es/firefox/addon/apia-digital-signature\" target=\"_blank\">");
 		toks.add("</a>");
 			
 		out.write(StringUtil.parseMessage(LabelManager.getName(uData,"msgVerifyInstFFExt"), toks) + "<br/>");
