...
<%
	...
	String url4 = helper.obtenerFuncionalidad(login, "Consulta expedientes por ubicacion actual", envId);
	String url5 = helper.obtenerFuncionalidad(login, "Consulta expedientes por fecha", envId);
	String url6 = helper.obtenerFuncionalidad(login, "Consulta global por titulares", envId);
	String url7 = helper.obtenerFuncionalidad(login, "Dashboard Tareas", envId);
	String url8 = helper.obtenerFuncionalidad(login, "Bandeja de procesamiento interno", envId);
	String url9 = helper.obtenerFuncionalidad(login, "Consulta por número de expediente", envId);
%>
...
#img6 span {
	background: transparent url('paneles/img/ConsultaPorTitulares.png')	center 0;
}
...
#img8  span {
	background: transparent url('paneles/img/BandejaProcInterno.png') center 0;
}
#img9 span {
	background: transparent url('paneles/img/ConsultaPorNumero.png') center 0;
}
...
<script type="text/javascript">
	var urls = {
		...
		4 : {
			title : "Consulta por ubicación actual",
			url: "<%=url4%>"
		},
		5 : {
		...
		8 : {
			title : "Bandeja de procesamiento interno",
			url : "<%=url8%>"
		},
		9 : {
			title : "Consulta por número de expediente",
			url: "<%=url9%>"
		}
	};
...
<body>
	...
	<div class="box categorias"
		...
		<div class="layout" style="height: 105px;">
			<ul>
				...
				<%if (!url3.isEmpty()){%><li id='img3' onclick='loadURL(3)'><center><span></span> Consulta<br>por número</center></li>	<%}%>
				<%if (!url4.isEmpty()){%><li id='img4' onclick='loadURL(4)'><center><span></span> Consulta por<br>ubicación actual</center></li>	<%}%>
				<%if (!url5.isEmpty()){%><li id='img5' onclick='loadURL(5)'><center><span></span> Consulta<br>por fecha</center></li>	<%}%>
				<%if (!url6.isEmpty()){%><li id='img6' onclick='loadURL(6)'><center><span></span> Consulta<br>por titulares</center></li>	<%}%>
				<%if (!url7.isEmpty()){%><li id='img7' onclick='loadURL(7)'><center><span></span> Tablero de<br>control</center></li>	<%}%>
				<%if (!url8.isEmpty()){%><li id='img8' onclick='loadURL(8)'><center><span></span> Bandeja de<br>procesamiento<br>interno</center></li><%}%>					
				<%if (!url9.isEmpty()){%><li id='img9' onclick='loadURL(8)'><center><span></span> Consulta<br>por número</center></li><%}%>		
			</ul>
		</div>
	</div>
