...
var DatePicker = new Class({
	...
	attach: function() {
		...
		$$(this.attachTo).each(function(item, index) {
			...
			var lblClone = new Element('label');
			lblClone.setStyle('display','none');
			lblClone.set('for',item.get('data-fld_id') + '_d');
			lblClone.appendText('&nbsp;');
			lblClone.inject(clone, 'before');
				...
				clone.addEvents({
					'blur': function() {
						...
						if (dateValue != clone.get('value')) {
							if (clone.get('value')!=""){
						        if (!validateDate(clone.get('value'))){
						        	clone.set('value', '');
						        	return;
						        }        
							}
