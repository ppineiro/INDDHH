...
var FormCheck = new Class({
	...
	*/
	validate : function(el) {
		...
		if (this.form.disabledCheck) return true;
		if(el.get('disabledCheck') == 'true') return true;
	...
	*/
	simpleValidate : function(el) {
		if(el.get('tag') == 'select'){
			if(!el.multiple) {
				if(el.selectedIndex <= 0 && (el.options.length==0 || el.options[el.selectedIndex].value=="")) {
...
debugger;
		if (hayQueRemoveErrors){				
			limpiarErroresFnc();
			//removeErrorsFondo();
			removeErrors();
			hayQueRemoveErrors = false;
		}
		...
		//ver si existe el titulo
		try{
			if (document.getElementById("errorlist")==null){
				this.form.element = new Element('div', {'id' : 'errorlist', 'class' : this.options.errorClass}).injectTop(this.form);
			}
			if (document.getElementById("errorlistTitle")==null){
				this.form.element.title = new Element('div', {'id' : 'errorlistTitle'}).set('html',this.options.alerts.errorsTitle).injectTop(this.form.element);
			}	
			if (document.getElementById("errorlist").innerHTML==""){
				this.form.element.title = new Element('div', {'id' : 'errorlistTitle'}).set('html',this.options.alerts.errorsTitle).injectTop(this.form.element);			
			}
		}catch (e) {
		}	
			...
			if ((obj.validation.contains('required') && obj.errors.length > 0) || (obj.errors.length > 0 && obj.value && obj.validation.contains('required') == false)) {
				...
				obj.errors.each(function(error) {
					if(obj.get("data-attLabel")!=null){
						//new Element('p').set('html',"<div class='leftErrorSpan'>" + indexError + ". " + obj.get("data-attLabel")  + ": </div> <div class='rightErrorSpan'>" + error + " </div>").injectInside(this.form.element).scrollIntoView( false );
						new Element('div').set('html',"<div class='leftErrorSpan'>" + indexError + ". " + obj.get("data-attLabel")  + ": </div> <div class='rightErrorSpan'>" + error + " </div>").injectInside(this.form.element).scrollIntoView( false );
					} else {
						//new Element('p').set('html',"<span>" + indexError + ". "  + obj.name + " : </span>" + error).injectInside(this.form.element);
						new Element('div').set('html',"<span>" + indexError + ". "  + obj.name + " : </span>" + error).injectInside(this.form.element);
					}
					
					//alert(obj.getScrollSize().y);
					var classNameNumError = "err-num-" + indexError;										
					//alert(classNameNumError);
					obj.addClass(classNameNumError);
					obj.addClass("campo-error-perso");
		...
		try{
			...
			try{
				...
				if (el.parentNode.tagName=="FORM"){
			        ...
			        var tblBody = document.getElementsByClassName('TRM_TITULO')[0].childNodes[2].childNodes[1]; 
			        tblBody.appendChild(hilera);			        
					//				
					msgErrorColocado = true;
...
*/				
					/*
					if (document.getElementsByTagName("form")[0].childNodes[4].childNodes[2].childNodes[9].childNodes[2].childNodes[1].childNodes[1].childNodes[2].childNodes[0].childNodes[6].childNodes[0].childNodes[0].childNodes[0].childNodes[0]!=null){
						nel = document.getElementsByTagName("form")[0].childNodes[4].childNodes[2].childNodes[9].childNodes[2].childNodes[1].childNodes[1].childNodes[2].childNodes[0].childNodes[6].childNodes[0].childNodes[0].childNodes[0].childNodes[0];
					}
					*/
/*					nel = document.getElementsByClassName('LBL_TITULO')[1].childNodes[0].childNodes[0].childNodes[0].childNodes[1];				
				}catch (e) {
				}	
...
*/					
						/*
						if (document.getElementsByTagName("form")[0].childNodes[4].childNodes[2].childNodes[9].childNodes[2].childNodes[1].childNodes[1].childNodes[2].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0]!=null){
							nel = document.getElementsByTagName("form")[0].childNodes[4].childNodes[2].childNodes[9].childNodes[2].childNodes[1].childNodes[1].childNodes[2].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0];
						}
						*/
/*						nel = top.document.getElementsByClassName('LBL_TITULO')[1].childNodes[0].childNodes[0].childNodes[0].childNodes[1];
					}catch (e) {				       
					}	
				}
...
*/
			...
			if (EXTERNAL_ACCESS == "true") {
				objError.children[0].style.display = "none";
			}else{
				objError.children[0].innerHTML = "Hay error/es en el formulario";
			} 
...
debugger;
		...
		if (this.options.display.errorsLocation >= 2 && obj.element && this.options.displreay) {
			new Fx.Tween(obj.element, {
				'duration': this.options.displreay.fadeDuration
			}).start('height', 0);
		}
