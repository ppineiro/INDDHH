...
MaskedInput.setMask = function (input, mask) {
	...
	input.set('data-empty_mask', MaskedInput.parseMask(input, mask));
	input.set('data-unmasked_value', input.get('value'));
...
MaskedInput.unsetMask = function (input) {
	if(input.mask) {
		...
		input.mask = null;
		input.endMask = null;
		input.erase('data-empty_mask');
		input.erase('data-unmasked_value');
...
MaskedInput.mask = function(input) {
	...
	var empty_mask = input.get('data-empty_mask');
	...
	if(shown_length == 0) {
		...
		input.set('data-unmasked_value', unmasked_value);
	}
...
MaskedInput.endMask = function(input) {
	//Mostrar la mascara en los lugares que falte del input
	var unmasked_value = input.get('data-unmasked_value');
...
MaskedInput.maskedInputEventHandler = {
	keydown: function(event) {
		...
		if(event.key == "backspace") {
			...
			var unmasked_value = event.target.get('data-unmasked_value');
			...
			if(unmasked_value.length == 0) {
				position = 0;
				var empty_mask = event.target.get('data-empty_mask');
			...
			event.target.set('data-unmasked_value', unmasked_value);
	...
	click: function(event) {
		event.target.setCaretPosition(event.target.get('data-unmasked_value').length);
		event.preventDefault();
	},
	...
	setCharacter: function(target, character) {
		var unmasked_value = target.get('data-unmasked_value');
		...
		target.set('data-unmasked_value', unmasked_value);
