		...
		for(var i = 0; i < rows.length; i++) {
			...
			for(var j = 0; j < cells.length; j++) {
				...
				if (tdWidths) {
					...
					obj[eleType] = content;
					if(boldType){
						new Element('b', obj).inject(new Element('span').inject(div));
					} else {
						new Element('span', obj).inject(div);	
					}
...
function setAdmDatePicker (datepicker) {
	...
	try{
		tskName = ApiaFunctions.getCurrentTaskName();
		if (tskName == "CARGA_DATOS_TRAMITE"  || tskName == "RETOMA_CARGA_DATOS" ) {			
			return;
		}
	} catch (e) {
	}
	*/
	...
	if(datepicker.get('data-fld_id')) {
		...
		datepicker.getNext().getNext().set('id', datepicker.get('data-fld_id') + '_d');
	}
