<DIV class="subTit"><%=LabelManager.getName(labelSet,"sbtPerAccPro")%></DIV><div type="grid" id="permGrid" style="height:200px"><table width="500px" cellpadding="0" cellspacing="0"><thead><tr><th style="width:0px;display:none;" title="<%=LabelManager.getToolTip(labelSet,"lblSel")%>"></th><th style="width:50px">&nbsp;</th><th style="width:300px" title="<%=LabelManager.getToolTip(labelSet,"lblNom")%>"><%=LabelManager.getName(labelSet,"lblNom")%></th><th style="width:100%" title="<%=LabelManager.getToolTip(labelSet,"lblPer")%>"><%=LabelManager.getName(labelSet,"lblPer")%></th></tr></thead><tbody class="scrollContent"><% Collection colPermissions = dBean.getColPermissions();
	   		   if (colPermissions==null || colPermissions.size()==0){ //Si aun no se definieron permisos%><tr cantDelete="true"><td style="width:0px;display:none;" align="center"><input type='hidden' id="idSel" name="chkSel"><input type='hidden' name="hidPoolId" value="-1"><input type='hidden' name="hidRead" value="1"><input type='hidden' name="hidMod" value="1"></td><td align="center"><div style="width:18px; height:18px;"></div></td><td title="<%=LabelManager.getToolTip(labelSet,"lblTod")%>"><%=LabelManager.getName(labelSet,"lblTod")%></td><td><input name="chkAllowRead" type=checkbox checked onClick="chgChkHidRead(event)"><%=LabelManager.getName(labelSet,"lblPerVer")%><input name="chkAllowModify" type=checkbox checked onClick="chgChkHidMod(event)"><%=LabelManager.getName(labelSet,"lblPerMod")%></td></tr><%}else{//Ya se definieron permisos
				 ...
				 while (itPermissions.hasNext()) {
					...
					String poolName;
					boolean readAccess = (1 == objAccessVo.getReadOnly().intValue());
					boolean writeAccess = (1 == objAccessVo.getReadWrite().intValue());
					...
					%><tr cantDelete="<%=cantDelete%>"><td style="width:0px;display:none;" align="center"><input type='hidden' id="idSel" name="chkSel"><input type='hidden' name="hidPoolId" value="<%=poolId.intValue()%>"><input type='hidden' name="hidRead" value="<%=(readAccess)?"1":"0"%>"><input type='hidden' name="hidMod" value="<%=(writeAccess)?"1":"0"%>"></td><td align="center"><%=strImage%></td><td title="<%= dBean.fmtHTML(poolName)%>"><%=dBean.fmtHTML(poolName)%></td><% if (poolId==-1){ %><td><input name="chkAllowRead" type=checkbox <%=(readAccess)?"checked":""%> onClick="chgChkHidRead(event)"><%=LabelManager.getName(labelSet,"lblPerVer")%><input name="chkAllowModify" type=checkbox <%=(writeAccess)?"checked":""%> onClick="chgChkHidMod(event)"><%=LabelManager.getName(labelSet,"lblPerMod")%></td><%}else{ %><td><input type="radio" id="rad<%=intPermId %>" onClick="chgRadHidRead(event)" name="<%=rType + intPermId%>" <%if (readAccess && !writeAccess){%> checked <%}%>><%=dBean.fmtScriptStr(LabelManager.getName(labelSet,"lblPerVer"))%><input type="radio" id="rad<%=intPermId %>" onClick="chgRadHidMod(event)" name="<%=rType + intPermId%>" <%if (writeAccess){%> checked <%}%>><%=dBean.fmtScriptStr(LabelManager.getName(labelSet,"lblPerMod"))%></td><%}%></tr><%}
			}%></tbody></table></div><table class="navBar" id="navBar"><tr><td align="right"><button type="button" type="button" onclick="btnAddPoolUsrPerm_click()" accesskey="<%=LabelManager.getAccessKey(labelSet,"btnAgrGruUsr")%>" title="<%=LabelManager.getToolTip(labelSet,"btnAgrGruUsr")%>"><%=LabelManager.getNameWAccess(labelSet,"btnAgrGruUsr")%></button><button type="button" type="button" onclick="btnDelPoolPerm_click()" accesskey="<%=LabelManager.getAccessKey(labelSet,"btnEli")%>" title="<%=LabelManager.getToolTip(labelSet,"btnEli")%>"><%=LabelManager.getNameWAccess(labelSet,"btnEli")%></button></td></tr></table><script src="<%=Parameters.ROOT_PATH%>/programs/administration/process/permissions.js"></script><SCRIPT>
...
function addObject(rets) {
	if (rets != null) {
		for (j = 0; j < rets.length; j++) {
			...
			if (addRet) {
				//var tr=trows[0].cloneNode(true); //clonamos la primer fila
				//tr.setAttribute("cantDelete","false"); //seteamos el atributo cantDelete en "false"
				//var tds=tr.getElementsByTagName("TD"); //obtenemos los td's de la fila clonada
				//var oTd0 = tds[0]; 
				//var oTd1 = tds[1]; 
				//var oTd2 = tds[2]; 
				//var oTd3 = tds[3];
