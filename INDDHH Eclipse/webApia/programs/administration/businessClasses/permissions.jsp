...
boolean withProject = (busClassVo.getPrjId()!=null);
...
%><DIV class="subTit"><%=LabelManager.getName(labelSet,"sbtPerAccBusCla")%></DIV><table class="tblFormLayout"><tr><td><input name="usePrjPerms" id="usePrjPerms" type=checkbox <%=(chkBoxClicked)?"checked":""%><%=(!withProject)?"disabled":""%> onClick="chkUsePrjPerms()"><%=LabelManager.getName(labelSet,"lblUsePrjPerm")%></td></tr></table><div type="grid" id="permGrid" style="height:200px" <%=(chkBoxClicked)?"disabled":""%>><table width="500px" cellpadding="0" cellspacing="0"><thead><tr><th style="width:0px;display:none;" title="<%=LabelManager.getToolTip(labelSet,"lblSel")%>"></th><th min_width="50px" style="width:50px">&nbsp;</th><th min_width="300px" style="min-width:300px;width:70%" title="<%=LabelManager.getToolTip(labelSet,"lblNom")%>"><%=LabelManager.getName(labelSet,"lblNom")%></th><th min_width="150px" style="min-width:150px;width:30%" title="<%=LabelManager.getToolTip(labelSet,"lblPer")%>"><%=LabelManager.getName(labelSet,"lblPer")%></th></tr></thead><tbody class="scrollContent"><% Collection colPermissions = dBean.getColPermissions();
				 ...
				 while (itPermissions.hasNext()) {
					...
					boolean writeAccess = (objAccessVo.getReadOnly()!=null && 1 == objAccessVo.getReadWrite().intValue());
			...
			}%></tbody></table></div><table class="navBar" id="navBar"><tr><td align="right"><button type="button" id="addPoolUsrPerm" type="button" onclick="btnAddPoolUsrPerm_click()" accesskey="<%=LabelManager.getAccessKey(labelSet,"btnAgrGruUsr")%>" title="<%=LabelManager.getToolTip(labelSet,"btnAgrGruUsr")%>"><%=LabelManager.getNameWAccess(labelSet,"btnAgrGruUsr")%></button><button type="button" id="delPoolUsrPerm" type="button" onclick="btnDelPoolPerm_click()" accesskey="<%=LabelManager.getAccessKey(labelSet,"btnEli")%>" title="<%=LabelManager.getToolTip(labelSet,"btnEli")%>"><%=LabelManager.getNameWAccess(labelSet,"btnEli")%></button></td></tr></table><script src="<%=Parameters.ROOT_PATH%>/programs/administration/businessClasses/permissions.js"></script><SCRIPT>
