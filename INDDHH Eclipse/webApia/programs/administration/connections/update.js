function btnConf_click(){
	if(isValidName(document.getElementById("txtName").value)){
		if (verifyRequiredObjects() && checkValues()) {
			if (verifyPermissions()){
				document.getElementById("frmMain").action = "administration.ConnectionsAction.do?action=confirm";
...
function btnTest_click(){
	//document.focus();
	setTimeout("btnTest_time()",1);
}

function btnTest_time(){
	if (verifyRequiredObjects() && checkValues()) {
		document.getElementById("frmMain").action = "administration.ConnectionsAction.do?action=test";
		submitForm(document.getElementById("frmMain"));
	}
}
...
function checkValues(){
	var connMin = document.getElementById("txtConnMin").value;
	var connMax = document.getElementById("txtConnMax").value;
	var connMaxIdle = document.getElementById("txtConnMaxIdle").value;
	var connMaxWait = document.getElementById("txtConnMaxWait").value;

	if (connMin < 0 || connMax < 0 || connMaxIdle < 0 || connMaxWait < 0){
		alert(MSG_WRNG_VAL);
		return false;
	}else if (connMin > connMax){
		alert(MSG_WRNG_MAX_CON);
		return false;
	}
	return true;
}
...
function chaCmbType() {
	...
	var connString = document.getElementById("txtConnString");
	if (cmbType.value == TYPE_ORACLE) {
		connString.value = "jdbc:oracle:thin:@<server>:<port>:<db name>";
	} else if (cmbType.value == TYPE_SQLSERVER) {
		connString.value = "jdbc:sqlserver://<server>:<port>;DatabaseName=<db name>";
	} else if (cmbType.value == TYPE_POSTGRE) {
		connString.value = "jdbc:postgresql://<server>:<port>/<db name>";
	} else if (cmbType.value == TYPE_MYSQL) {
		connString.value = "jdbc:mysql://<server>:<port>/<db name>";
	} else {
		connString.value = "";
	}
}
...
function loadProyectPerms(){
	...
	var sXMLSourceUrl = "administration.ConnectionsAction.do?action=getProjPermssions&prjId=" + prjId;
