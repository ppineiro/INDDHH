...
function checkExistCubeName(cubeName,cubeId){
	...
	http_request.open('POST', "administration.BPMNAction.do?action=checkExistCbeName"+windowId, false);
...
function btnAddDimension_click() {
	...
	var rets = null;
	var rets = openModal("/administration.BPMNAction.do?action=addAttDimension&attIds="+attIds + "&attEntIds="+attEntIds+"&attProIds="+attProIds + windowId,(getStageWidth()*.8),(getStageHeight()*.7));//ancho,largo
					...
					for (j = 0; j < rets.length; j++) {
						...
						if (ret[0] != "skip"){
							...
							if (addDim) {
								...
								oTd4.innerHTML = "<input type='text' name='txtDimName' onchange='chkDimName(this)' value='" + dimName +"'>"; //--> Agregamos el disp name	
...
function btnAddMeasure_click() {
	...
	//1- Mostramos el tree con formularios y atributos
	var rets = null;
	//var rets = openModal("/administration.BPMNAction.do?action=addAttDimension&attIds="+attIds +"&attEntIds="+attEntIds+"&attProIds="+attProIds + windowId,800,700);//ancho,largo
	var rets = openModal("/administration.BPMNAction.do?action=addAttMeasure&attMsrIds="+attMsrIds + "&attEntMsrIds=" + attEntMsrIds + "&attProMsrIds=" + attProMsrIds + windowId,(getStageWidth()*.8),(getStageHeight()*.7));//ancho,largo
					...
					if (ret[0] != "skip"){
						...
						if (addMeas) {
							...
							*/
							...
							//Nombre a mostrar
							oTd2.innerHTML = "<input type='text' name='dispName' onchange='chkMeasName(this)' value='" + measureName +"'>"; //--> Agregamos el disp name
...
function btnDupMeasure_click() {
	...
	if (trows.length>0 && document.getElementById("gridMeasures").selectedItems.length == 1){
			...
			oTd2.innerHTML = "<input type='text' name='dispName' onchange='chkMeasName(this)' value='" + newMeasureName +"'>"; //--> Agregamos el disp name
...
function getWidgetDependency(){
	var	http_request = getXMLHttpRequest();
	http_request.open('POST', "administration.BPMNAction.do?action=getWidgetDeps"+windowId, false);
	http_request.setRequestHeader("content-type","application/x-www-form-urlencoded");
	var str = "";
	http_request.send(str);
...
function getCubesDependency(){
	var	http_request = getXMLHttpRequest();
	http_request.open('POST', "administration.BPMNAction.do?action=getCubeDeps"+windowId, false);
	http_request.setRequestHeader("content-type","application/x-www-form-urlencoded");
	var str = "";
	http_request.send(str);
...
function getAttIds(){
	...
	http_request.open('POST', "administration.BPMNAction.do?action=sqlTest"+windowId, false);
	http_request.setRequestHeader("content-type","application/x-www-form-urlencoded");
...
function clearAttributes(){
	var	http_request = getXMLHttpRequest();
	http_request.open('POST', "administration.BPMNAction.do?action=clearAtts"+windowId, false);
	http_request.setRequestHeader("content-type","application/x-www-form-urlencoded");
	var str = "";
	http_request.send(str);
...
function getFrmAttributes(frmId){
	var	http_request = getXMLHttpRequest();
	http_request.open('POST', "administration.BPMNAction.do?action=getAtts"+windowId, false);
	http_request.setRequestHeader("content-type","application/x-www-form-urlencoded");
	var str = "frmId=" + frmId;
	http_request.send(str);
...
function btnEstTime_click(){
	if (verifyCubeDataToEstimateTime()){
		var	http_request = getXMLHttpRequest();
		http_request.open('POST', "administration.BPMNAction.do?action=estimateTime"+windowId, false);
		http_request.setRequestHeader("content-type","application/x-www-form-urlencoded");
...
function btnDelNoAccProfile_click() {
	...
	if(cant == 1) {
		...
		frm.action="administration.BPMNAction.do?action=removeNoAccProfile"+windowId+"&prfName="+prfName;
...
function openNoAccDims(obj) {
	...
	var action=frm.action;
	var target=frm.target;
	frm.action="administration.BPMNAction.do?action=loadDims"+windowId+"&after=openNoAccDimsModal&prfName="+prfName;
...
function openNoAccDimsModal(dims){
	...
	rets.onload=function(){
		...
		window.parent.parent.parent.frames[ifr.name].setXml(dims,prfName,cbeName,"bpmnCube");
	}
}
