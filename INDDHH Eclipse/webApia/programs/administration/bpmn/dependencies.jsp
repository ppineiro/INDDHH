<%@page import="com.dogma.vo.*"%><%@page import="com.dogma.vo.custom.*"%><%@page import="java.util.*"%><%@include file="../../../components/scripts/server/startInc.jsp" %><HTML><head><%@include file="../../../components/scripts/server/headInclude.jsp" %></head><jsp:useBean id="dBean" scope="session" class="com.dogma.bean.administration.BPMNBean"></jsp:useBean><body><TABLE class="pageTop"><COL class="col1"><COL class="col2"><TR><TD><%=LabelManager.getName(labelSet,"titBPMN")%></TD><TD></TD></TR></TABLE><div id="divContent"><form id="frmMain" name="frmMain" method="POST"><%
			Collection col = dBean.getProcDependencies(request);
			if (col!=null && col.size()>0) {
				Iterator it = col.iterator();
				boolean blnFirst = true; boolean blnSec = true; boolean blnThr = true; boolean blnFor=true; boolean blnWid=true; boolean blnCbe=true; boolean blnMonBus=true; boolean blnPrf = true; boolean blnFun = true;
				boolean finShowProcess = false; boolean finShowProInstance = false;
				boolean finShowBusEntPro = false; boolean finShowScenario = false; boolean finShowWidget=false; boolean finShowCbe=false; boolean finShowMonBus = false; boolean finShowPrf = false; boolean finShowFun = false;
				int countShownProcess = 1; int countShownProInstance = 1; int countShownBusEntPro = 1; int countShownScenario = 1;
				int countShownWidget = 1; int countShownCbe = 1; int countShownMonBus = 1; int countShownPrf = 1; int countShownFun = 1;
				
				while (it.hasNext() && (!finShowProcess || !finShowProInstance || !finShowBusEntPro || !finShowWidget || !finShowScenario || !finShowWidget || !finShowCbe || !finShowPrf || !finShowFun)) {
					Object obj = it.next();
					if (obj instanceof ProcessVo) {
						if (countShownProcess > Parameters.MAX_DEPENDENCIES_SHOWN && !finShowProcess) {
	 					...
	 					if (countShownWidget > Parameters.MAX_DEPENDENCIES_SHOWN && !finShowWidget) {
	 						out.print("<BR>" + LabelManager.getName(labelSet,"lblMasDep"));
	 						finShowWidget = true;
						} else if (!finShowWidget){
					...
					}else if (obj instanceof SimScenarioVo) { 
	 					if (countShownScenario > Parameters.MAX_DEPENDENCIES_SHOWN && !finShowScenario) {
	 						out.print("<BR>" + LabelManager.getName(labelSet,"lblMasDep"));
	 						finShowScenario = true;
						} else if (!finShowScenario){
							if (blnFor) { %><DIV class="subTit"><%=LabelManager.getName(labelSet,"lblEscAso")%></DIV><%
								blnFor = false;
							}
							out.print("<LI class=\"liDep\">" + ((SimScenarioVo) obj).getSimSceName()) ;
							countShownScenario++;
	 					}
