...
if (widVo.getWidKpiStyle() != null && widVo.getWidKpiStyle().intValue() <= 3){
	style = widVo.getWidKpiStyle();
}

//isFather = wBean.isFather(widId);  <------ DESCOMENTAR CUANDO MARTIN ARREGLE LOS BOTONES DE MOSTRAR HIJOS
...
%><trafficlight id="<%=widId%>" title="<%=widVo.getWidName()%>" valueStart="<%=widVo.getWidKpiMin().intValue()%>" valueEnd="<%=widVo.getWidKpiMax().intValue()%>" valueCurrent="<%=nextValue%>" seeTimer="<%=viewTimer%>" refreshTime="<%=refTime%>" url="<%=Parameters.ROOT_PATH%>/programs/biExecution/widgets/kpi/trafficLights/trafficLights.jsp?widId=<%=widId.intValue()%>" inconsistent="<%=inconsistent %>" style="<%=style.intValue()%>"><definition><% Collection widZneCol = widVo.getZones();
		   if (widZneCol!=null && widZneCol.size()>0){
			   ...
			   while (widZneIt.hasNext()){
				   ...
				   if (zoneVo.getWidZneColor()!= null && !"".equals(zoneVo.getWidZneColor())){%><color<%=colNum%> color="<%=zoneVo.getWidZneColor().replace("#","0x")%>" min="<%=zoneVo.getWidZneMin().toString()%>" max="<%=zoneVo.getWidZneMax().toString()%>"/><%}else{%><color<%=colNum%> color="<%=defaultColor%>" min="<%=zoneVo.getWidZneMin().toString()%>" max="<%=zoneVo.getWidZneMax().toString()%>"/><%}
				   colNum ++;
			    }
		   }
