...
if (errorCol == null || value != null){%><%@page import="com.dogma.vo.WidPropertiesVo"%><html xmlns="http://www.w3.org/1999/xhtml" xml:lang="es" lang="es"><head><meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1" /><title>semaforo</title><link href="<%=Parameters.ROOT_PATH%>/styles/<%= styleDirectory %>/css/workArea.css" rel="styleSheet" type="text/css" media="screen"><%@include file="../../../../../components/scripts/server/headInclude.jsp" %></head><body style="background:transparent;"><div><%	double nameSize = widVo.getWidName().length();
				...
				}else{%><DIV class="subTit" style="visibility:hidden"></DIV><%} %><!--URL utilizadas en la película--><!--Texto utilizado en la película--><object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,0,0" width="100%" height="200px" id="Semaforo" align="middle"><param name="allowScriptAccess" value="sameDomain" /><param name="movie" value="<%=Parameters.ROOT_PATH%>/programs/biExecution/widgets/kpi/trafficLights/semaforo.swf" /><param name="quality" value="high" /><param name="bgcolor" value="#ffffff" /><param name="wmode" value="transparent" /><param name="flashVars" value="xmlUrl=<%=Parameters.ROOT_PATH%>/programs/biExecution/widgets/kpi/trafficLights/trafficLights.jsp?widId=<%=widId.intValue()%>%26viewTimer=<%=viewTimer%>"/><embed src="<%=Parameters.ROOT_PATH%>/programs/biExecution/widgets/kpi/trafficLights/semaforo.swf" wmode="transparent" quality="high" bgcolor="#ffffff" flashvars="xmlUrl=<%=Parameters.ROOT_PATH%>/programs/biExecution/widgets/kpi/trafficLights/trafficLights.jsp?widId=<%=widId.intValue()%>%26viewTimer=<%=viewTimer%>" width="100%" height="200" name="Semaforo" align="middle" allowScriptAccess="sameDomain" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" /></object></div><DIV style="border-top:1px solid #999999"><% if (viewBtnChilds){%><span><img onclick="viewChilds()" style="cursor:pointer;cursor:hand;" width="25" height="25" title="<%=LabelManager.getName(uData.getLabelSetId(),uData.getLangId(),"lblVwWidChilds")%>" border="0" src="<%=Parameters.ROOT_PATH%>/images/dashboard/childs.png"></span><%}
...
function viewHistory(){
	var width = window.parent.getStageWidth()*.8;
	var height = window.parent.getStageHeight()*.8;
