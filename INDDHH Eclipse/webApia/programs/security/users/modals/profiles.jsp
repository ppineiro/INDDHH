...
function doXMLLoad(){
	var sXMLSourceUrl = "profilesXML.jsp?name=" + escape(document.getElementById("txtNom").value) + "&onlyEnv=<%=request.getParameter("onlyEnv")%>" + "&envId=<%=request.getParameter("envId")%>&exactMatch="+document.getElementById("chkExact").checked;
		...
		if (xmlRoot.childNodes.length == 0) {
			...
			for(i=0;i<xmlRoot.childNodes.length;i++){
				if (i >= <%=Parameters.MAX_RESULT_MODAL %>) {
					...
					oTd0.setAttribute("profileId",xRow.childNodes[0].firstChild.nodeValue);
					oTd0.setAttribute("profileName",xRow.childNodes[1].firstChild.nodeValue);
					//oTd0.setAttribute("profileAll",xRow.childNodes[2].firstChild.nodeValue);
...
function getSelected(){
	...
	if (oRows != null) {
		...
		for (i = 0; i < oRows.length; i++) {
			...
			arr = new Array();
			arr[0] = oTd.getAttribute("profileId");
			arr[1] = oTd.getAttribute("profileName");
			arr[2] = oTd.getAttribute("profileAll");
