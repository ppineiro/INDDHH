...
function btnConf_click(){
	if (verifyRequiredObjects()) {
		if(isValidName(document.getElementById("txtName").value)){
			document.getElementById("frmMain").action = "security.ProfilesAction.do?action=confirm";
...
function viewFnc(e){
	...
	var ret = openModal("/security.ProfilesAction.do?action=openModal&envId=" + envId + windowId,450,500);
}
