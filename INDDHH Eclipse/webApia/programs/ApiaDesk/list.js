function setList(div){
	...
	div.getRow=function(el){
		...
		element.icon.innerHTML="<img height='20px' width='20px' src='"+el.icon+"'>";
		element.data=el;
		return element;
	}
	...
	div.filter=function(str){
		if((this.filteredOut!=this.elements.length && this.filterStr.indexOf(str)==0 && this.filterStr.length<str.length)||
		( this.filterStr.indexOf(str)==0 && this.filterStr.length>str.length ) ||
		 ( this.filterStr.indexOf(str)!=0 && this.filterStr.length!=str )	){
			for(var i=0;i<this.elements.length;i++){
				if(this.elements[i].data.name.toUpperCase().indexOf(str.toUpperCase())==0 || str==""){
