...
function doXMLLoad(){
	var sXMLSourceUrl = "<%=Parameters.ROOT_PATH%>/programs/modals/viewColumnsXML.jsp?name=" + escape(document.getElementById("txtNom").value) + "&view=" + document.getElementById("hidView").value + "&spCols=" + '<%=spCols%>';
		...
		if (xmlRoot.childNodes.length == 0) {
			...
			for(i=0;i<xmlRoot.childNodes.length;i++){
				if (i >= <%=Parameters.MAX_RESULT_MODAL %>) {
					...
					oTd0.setAttribute("columnName",xRow.childNodes[0].firstChild.nodeValue);
					oTd0.setAttribute("columnType",xRow.childNodes[1].firstChild.nodeValue);
