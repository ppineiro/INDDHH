...
function doXMLLoad(){
	var sXMLSourceUrl = "querysXML.jsp?name=" + escape(document.getElementById("txtNom").value) ;
	<%if(request.getParameter("type")!=null){%>
	sXMLSourceUrl = sXMLSourceUrl + "&type=<%=request.getParameter("type")%>"  
	<%}%>
		...
		if (xmlRoot.childNodes.length == 0) {
			...
			for(i=0;i<xmlRoot.childNodes.length;i++){
				if (i >= <%=Parameters.MAX_RESULT_MODAL %>) {
					...
					oTd0.setAttribute("queryId",xRow.childNodes[0].firstChild.nodeValue);
					oTd0.setAttribute("queryName",xRow.childNodes[1].firstChild.nodeValue);
...
function getSelected(){
	...
	if (oRows != null) {
		...
		for (i = 0; i < oRows.length; i++) {
			...
			arr = new Array();
			arr[0] = oTd.getAttribute("queryId");
			arr[1] = oTd.getAttribute("queryName");
