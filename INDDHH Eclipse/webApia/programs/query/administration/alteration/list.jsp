...
%><%@page import="com.st.util.translator.TranslationManager"%><HTML><head><%@include file="../../../../components/scripts/server/headInclude.jsp" %></head><body onload="openFilterSection();"><TABLE class="pageTop"><COL class="col1"><COL class="col2"><TR><TD><%=LabelManager.getName(labelSet,"titEjeEnt")%><%if (blnSpecific) {%> : <%=queryVo.getQryTitle()%><%}%></TD><TD></TD></TR></TABLE><DIV id="divContent" class="divContent"><IFRAME name="idResult" id="idResult" height="0" width="0" frameborder="0"></IFRAME><form id="frmMain" name="frmMain" method="POST"><% if (queryVo.getWhereUserColumns() != null && queryVo.getWhereUserColumns().size() > 0) { %><DIV class="subTit"><table width="100%"><tr class="subTit"><td width="100%" align="left"><%=LabelManager.getName(labelSet,"sbtFil")%></td><td align="right"><button id="toggleFilter" title="<%=LabelManager.getToolTip(labelSet,"lblMonButFil")%>" class="btn" onclick="toggleFilterSection(<%=Parameters.SCREEN_LIST_SIZE - Parameters.FILTER_LIST_SIZE%>,<%=(Parameters.SCREEN_LIST_SIZE)%>)"><img src="<%=Parameters.ROOT_PATH%>/styles/<%= styleDirectory %>/images/openToc.gif" width="8" height="7"></button></td></tr></table></DIV><DIV id="listFilterArea" style="display:none"><DIV style="OVERFLOW:AUTO;HEIGHT:<%= Parameters.FILTER_LIST_SIZE - 32 %>px;"><table class="tblFormLayout"><COL class="col1"><COL class="col2"><COL class="col3"><COL class="col4"><%
						...
						if (! addedEndLine) {
							...
							if (qBean.queryHasEmptyRequieredFilters()) { %><tr><td colspan="<%= colsToShow %>"><br><br><%=LabelManager.getName(labelSet,"lblQryMustFil")%><br><br><br></td></tr><% 
									...
									for (int i = 0; i < showPosition.length; i++) amountOfVisibleColumns += showPosition[i] ? 1 : 0;
		 			  				...
		 			  				while (iteratorFilas.hasNext()) {
		 			  					row = (QryRowShowVo) iteratorFilas.next(); 
	 		 	  						if (row.getColumnas().size() != amountOfColumns) { %><tr><td colspan="<%= amountOfVisibleColumns %>"><%=LabelManager.getName(labelSet,"msgQryErrAm")%></td></tr><% } else { %><tr><td align="center" style="width:0px;display:none"><input type="hidden" id="chkSel<%=rowCount%>" name="chkSel<%=rowCount%>" /><input type="hidden" id="txtEntId<%=rowCount%>" name="txtEntId<%=rowCount%>" value="<%= ((posBusEntInstId>=0)?dBean.fmtInt(row.getValueInt(posBusEntInstId,true)):"") %>"><input type="hidden" name="txtBusEntId<%=((posBusEntInstId>=0)?dBean.fmtInt(row.getValueInt(posBusEntInstId,false)):"")%>" value="<%=dBean.fmtInt(busEntId)%>"><input type="hidden" name="txtBusEntAdm<%=((posBusEntInstId>=0)?dBean.fmtInt(row.getValueInt(posBusEntInstId,false)):"")%>" value="<%=((posBusEntAdminType>=0)?dBean.fmtStr(row.getValueStr(posBusEntAdminType,false)):"")%>"></td><%
			 									columnas = row.getColumnas().iterator();
		 										pos = 0;
				 											...
				 											if (showPosition[pos]) {
				 												if (pos == posProTitle){
						 											if(posProName != -1){
						 												...
						 												strValue=(dBean.fmtHTMLObject(TranslationManager.getProcTitle(proName, value.toString(), uData.getEnvironmentId(),uData.getLangId()),"&nbsp;"));
						 											}else{
						 											...
						 											if(posTskName != -1){
						 												...
						 												strValue=(dBean.fmtHTMLObject(TranslationManager.getTaskTitle(tskName, value.toString(), uData.getEnvironmentId(),uData.getLangId()),"&nbsp;"));
						 											}else{
						 												strValue=(dBean.fmtHTMLObject(value,"&nbsp;"));
						 											}
						 											...
						 											if(posBusEntName != -1){
						 												...
						 												strValue=(dBean.fmtHTMLObject(TranslationManager.getBusEntTitle(busEntName, value.toString(), uData.getEnvironmentId(),uData.getLangId()),"&nbsp;"));
						 											}else{
						 												strValue=(dBean.fmtHTMLObject(value,"&nbsp;"));
						 											}
						 											...
						 											if(posFrmName != -1){
						 												...
						 												strValue=(dBean.fmtHTMLObject(TranslationManager.getFrmTitle(frmName, value.toString(), uData.getEnvironmentId(),uData.getLangId()),"&nbsp;"));
						 											}else{
						 												strValue=(dBean.fmtHTMLObject(value,"&nbsp;"));
						 											}
						 											...
						 											if(posAttName != -1){
						 												...
						 												strValue=(dBean.fmtHTMLObject(TranslationManager.getAttTitle(attName, value.toString(), uData.getEnvironmentId(),uData.getLangId()),"&nbsp;"));
						 											}else{
						 												strValue=(dBean.fmtHTMLObject(value,"&nbsp;"));
						 											}
						 											...
						 											if(posAttName != -1){
						 												...
						 												strValue=(dBean.fmtHTMLObject(TranslationManager.getAttDesc(attName, originalValue, uData.getEnvironmentId(),uData.getLangId()),"&nbsp;"));
	 	 					...
	 	 					} %></tbody></table><IFRAME name="idResult" id="idResult" height="0" width="0" frameborder="0"></IFRAME></div><table class="navBar"><COL class="col1"><COL class="col2"><tr><% if (! queryVo.isFirstTime()) { %><%@include file="../../../includes/navButtonsQuery.jsp" %><% } else { %><td></td><% } %><td><input type="hidden" name="txtProId" value="<%= dBean.getProId() %>"><% if (queryVo.getButtonValue(QueryVo.FLAG_SHOW_BUTTON_ALTER)) { %><button onclick="btnAlt_click()" accesskey="<%=LabelManager.getAccessKey(labelSet,"btnAlt")%>" title="<%=LabelManager.getToolTip(labelSet,"btnAlt")%>"><%=LabelManager.getNameWAccess(labelSet,"btnAlt")%></button><% } %><% if (queryVo.getButtonValue(QueryVo.FLAG_SHOW_BUTTON_EXPORT)) { %><button onclick="btnExport_click()" accesskey="<%=LabelManager.getAccessKey(labelSet,"btnExport")%>" title="<%=LabelManager.getToolTip(labelSet,"btnExport")%>"><%=LabelManager.getNameWAccess(labelSet,"btnExport")%></button><% } %></td></tr></table></form></div><TABLE class="pageBottom"><COL class="col1"><COL class="col2"><TR><TD></TD><TD><button onclick="splash()" accesskey="<%=LabelManager.getAccessKey(labelSet,"btnSal")%>" title="<%=LabelManager.getToolTip(labelSet,"btnSal")%>"><%=LabelManager.getNameWAccess(labelSet,"btnSal")%></button></TD></TR></TABLE></body></html><script language="javascript" defer>
...
</script><script src="<%=Parameters.ROOT_PATH%>/programs/query/administration/alteration/list.js"></script><%@include file="../../../../components/scripts/server/endInc.jsp" %>
