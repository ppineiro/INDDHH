...
function btnNext_click(){
	if (verifyRequiredObjects()) {
		if(isValidLogin(document.getElementById("txtLogin").value)){
...
function btnAddPool_click() {
	...
	if(isAllEnv) {
		if(isGlobal){
			...
			rets = openModal("/programs/modals/pools.jsp?showAutogenerated=false&showGlobal=true&showOnlyEnv=true&envId="+envId,500,350);
		}
	} else {
		
		if(isGlobal){
			rets = openModal("/programs/security/users/modals/pools.jsp",500,350);
		} else {
			rets = openModal("/programs/modals/pools.jsp?showAutogenerated=false&showGlobal=false&showOnlyEnv=true&envId="+envId,500,350);
		}
	}
