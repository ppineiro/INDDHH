...
:: set CERTIFICATE=SelfSigned_2014.pfx
:: set SIGNING_OPTIONS=-storetype pkcs12 -keystore %CERTIFICATE%
set SIGNING_OPTIONS=-storetype JKS -keystore ../keystore_statum.jks -storepass statum434 
:: if not exist %CERTIFICATE% goto certificate

:: Output
if not exist air md air
set AIR_FILE=air/FacilisBPMN.air
...
echo Signing AIR setup using keystore ../keystore_statum.jks
