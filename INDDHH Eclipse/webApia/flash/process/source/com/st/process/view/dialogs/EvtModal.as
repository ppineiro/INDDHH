...
class com.st.process.view.dialogs.EvtModal extends MovieClip{
	...
	function EvtModal(Void){
		...
		custom = thisModal._parent.custom;
		eventClass_DP = new Array();
		
		p_type = thisModal._parent.p_type;
		this.confirm_btn.onPress = function() {
			var p_Array:Array = thisModal.getDataFromGrid(thisModal.eventClass_dg);
		...
		this.add_btn.onPress = function(){
			...
			//add to grid
			if(!((event_cmb.selectedItem.data=="" || class_cmb.selectedItem.data=="") ||
				(event_cmb.selectedItem.data==null || class_cmb.selectedItem.data==null) ||
				(event_cmb.selectedItem.data==undefined || class_cmb.selectedItem.data==undefined))){
				if(!isRepeated){
	...
	function onLoad(){
		...
		if(custom){
			trace("NOS MOSTRAR BOTOENES");
			this.add_btn._visible=false;
			this.del_btn._visible=false;
