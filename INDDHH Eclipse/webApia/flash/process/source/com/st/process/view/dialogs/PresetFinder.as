...
class com.st.process.view.dialogs.PresetFinder extends MovieClip{
			...
			loaderListener.onLoadComplete = function(success,loaderObj){
				...
				if(_global.isXMLexception(x)==true){
					...
					for (var e=0;e < x.firstChild.childNodes.length;e++) {
						var col_id = x.firstChild.childNodes[e].attributes.tskId;
						var col_name = x.firstChild.childNodes[e].attributes.tskName;
						var imgName = x.firstChild.childNodes[e].attributes.imgName;
						//objList.addItem(col_name,col_id);
						var d:Array=new Array();
						var events=x.firstChild.childNodes[e].firstChild;
						for(var i=0;i<events.childNodes.length;i++){
							d.push({event_id:events.childNodes[i].attributes.evtId,
								   event_name:events.childNodes[i].attributes.evtName,
								   class_id:events.childNodes[i].attributes.busClaId,
									class_name:events.childNodes[i].attributes.busClaName,
									order:events.childNodes[i].attributes.order});
						}
						d.sortOn("order",Array.NUMERIC);
						objList.addItem(col_name,{id:col_id,data:d,imgName:imgName});
					}
				}
