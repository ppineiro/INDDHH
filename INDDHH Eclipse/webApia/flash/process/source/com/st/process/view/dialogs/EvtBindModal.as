...
class com.st.process.view.dialogs.EvtBindModal extends MovieClip{
	...
	function EvtBindModal(Void){
		...
		this.att_btn.onPress = function(){
			...
			var configOBj = new Object();
				...
				configOBj.title =_global.labelVars.lab_eventBindingsAttributes;
			...
			var objEvt = new Object();
				objEvt.ok = function(evt){
					...
					thisModal.attName_txt.dispatchEvent({type:"change",att_id:att_id,att_label:att_label});
	...
	function onLoad(){
		...
		attTypeE_rad.label = _global.labelVars.lbl_BindEntity;
			...
			rowListener.change = function(event) {
				...
				if(i < tmp.bind_dg.dataProvider.length){
					...
					if(p_value!=null && p_value!="undefined" && p_value!=""){
						...
						tmp.frmOnGroup.selection.data=3;
						tmp.frmOnGroup.dispatchEvent({type:"click"});
		...
		var frmOnListener = new Object();
			frmOnListener.click = function (evt){
				...
				if(chkState==1){//val ON
					...
					tmp.attName_txt.dispatchEvent({type:"change",att_id:"",att_label:""});
			
					
				}else if(chkState==2){			//att ON
					trace("ATT-CHECK: ON")
