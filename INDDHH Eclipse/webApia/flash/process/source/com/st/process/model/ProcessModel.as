...
class com.st.process.model.ProcessModel {
	...
	public function addTask(att_id:Number,att_pro_ele_id:Number,att_task_id:Number,att_type:String,att_label:String,att_x:Number,att_y:Number):Object{
		var oTask = addElement(att_id,att_pro_ele_id,att_type,att_x,att_y);
			oTask.att_label = att_label;
			oTask.att_task_id = att_task_id;
		broadcastMessage("onTaskAdded",oTask.att_id,att_label,att_x,att_y);
		return oTask;
	};
	
	public function addCustomTask(att_id:Number,att_pro_ele_id:Number,att_task_id:Number,att_type:String,att_label:String,att_x:Number,att_y:Number,imgName:String):Object{
		var oTask = addElement(att_id,att_pro_ele_id,att_type,att_x,att_y);
			oTask.att_label = att_label;
			oTask.att_task_id = att_task_id;
			oTask.custom = true;
			oTask.imgName=imgName;
		broadcastMessage("onCustomTaskAdded",oTask.att_id,att_label,att_x,att_y,imgName);
		return oTask;
	};
	...
	public function taskHighlightComments(att_id:Number,hc_flag:Boolean):Void{
		if(hc_flag==true){
			elements[att_id].highligt_comments = hc_flag;
		}else{
			delete elements[att_id].highligt_comments;
		}
	};
	
	public function isTaskHighlightComments(att_id:Number):Boolean{
		return elements[att_id].highligt_comments;
	};
	
	public function addProcess (att_id:Number,att_pro_ele_id:Number,att_pro_id:Number,att_type:String,att_label:String,att_x:Number,att_y:Number):Void{
		var oProcess = addElement(att_id,att_pro_ele_id,att_type,att_x,att_y);
			oProcess.att_label = att_label;
			oProcess.att_pro_id = att_pro_id;
			oProcess.att_pro_type = ATT_PRO_TYPE_DEFAULT;
			
		broadcastMessage("onProcessAdded",oProcess.att_id,att_label,att_x,att_y);
	};
	...
	public function addElement(att_id:Number,att_pro_ele_id:Number,att_type:String,att_x:Number,att_y:Number):Object {
	...
	public function addLineElement(startId:Number,endId:Number,name:String,condition:String,conditionDoc:String):Void{
		if(!(startId==endId)){
			if((getVertexId(startId,endId)!=null) || (getVertexId(endId,startId)!=null)){
				...
				broadcastMessage("onLineAdded",startId,endId);
				if(name){
					addLineName(startId,endId,name);
				}
				if(condition){
					addLineCondition(startId,endId,condition,conditionDoc);
				}
			}
		}
	};
	...
	public function addLineCondition(id_a:Number,id_b:Number,condition:String,conditionDoc:String):Void{
		var oLine:Object = getVertex(id_a,id_b);
			oLine.condition = condition;
			oLine.conditionDoc = conditionDoc;
	...
	public function removeLineCondition(id_a:Number,id_b:Number):Void{
		...
		delete elements[id_a].vertices[id_b].conditionDoc;
		trace("LINE " + id_a + "_" + id_b + " CONDITION REMOVED: ");
		broadcastMessage("onDependencyConditionRemoved",id_a,id_b);
	};
	public function getLineCondition(id_a:Number,id_b:Number):Object{
		//var oLine:Object = getVertex(id_a,id_b);
			//return oLine.condition;
			var el=elements[id_a].vertices[id_b];
		return {conditionValue:el.condition,conditionDocValue:el.conditionDoc};
	};
	
	
	
	public function addLineName(id_a:Number,id_b:Number,name:String):Void{
		elements[id_a].vertices[id_b].name = name;
		trace("LINE " + id_a + "_" + id_b + " NAME ADDED: " + name);
		broadcastMessage("onDependencyNameAdded",id_a,id_b,name);
	};
	...
	public function updateProcessMultiplier(att_id:Number,multiplier_id:Number,multiplier_name:String){
		if(multiplier_id){
			elements[att_id].multiplier_id = multiplier_id;
			elements[att_id].multiplier_name = multiplier_name;
		}else{
	...
	public function updateProcessEntity(att_id:Number,entity_id:Number,entity_name:String){
		if(entity_id){
			elements[att_id].entity_id = entity_id;
			elements[att_id].entity_name = entity_name;
		}else{
			trace("DELETE MIGUEKLETE "+ att_id);
			delete elements[att_id].entity_id;
			delete elements[att_id].entity_name;
		}
	};
	...
	public function updateProcessIterationCondition(att_id:Number,b_flag:Boolean,condition:String,conditionDoc:String):Void{
		if(b_flag){
			if(isProcessIterated(att_id)){
				elements[att_id].condition = condition;
				elements[att_id].conditionDoc = conditionDoc;
			}
		}else{
	...
	public function getProcessIterateCondition(att_id:Number):Object{
		if(isProcessIterated(att_id)){
			var el=elements[att_id];
			return {conditionValue:el.condition,conditionDocValue:el.conditionDoc};
		}
	};
	...
	public function setProcessType(att_id:Number,att_pro_type:String):Void{
		if(att_pro_type == "m" || att_pro_type == "a" || att_pro_type == "s" || att_pro_type == "j" || att_pro_type == "k"){
			elements[att_id].type = att_pro_type;
			if(att_pro_type == "m" || att_pro_type == "j" || att_pro_type == "k"){
				updateProcessEntity(att_id);
			}
		}else{
	...
	//	TASK WEBSERVICE
	/////////////////////////////////////////////////////////////////
	public function setTaskWebService(att_id:Number,webService:Object){
		elements[att_id].webService = webService;
	};
	public function getTaskWebService(att_id:Number):Object{
		return elements[att_id].webService;
	};
	
	////////////////////////////////////////////////////////////////
	//	TASK 
	/////////////////////////////////////////////////////////////////
	public function setTaskCalendar(att_id:Number,calendar:Object){
		elements[att_id].calendar = calendar;
	};
	public function getTaskCalendar(att_id:Number):Object{
		return elements[att_id].calendar;
	};
	...
	//	PROCESS WEBSERVICE
	/////////////////////////////////////////////////////////////////
	public function setProcWebService(webService:Object){
		process_ws = webService;
	};
	public function getProcWebService():Object{
		return process_ws;
	};
	...
	//	TASK STATES [appends array/objects inside to element object]
	/////////////////////////////////////////////////////////////////
	public function getTaskCustom(att_id:Number):Array{
		return elements[att_id].custom;
	};
	...
	public function getTasks():Array{
		var tsks:Array=new Array();
		for (var z in elements) {
			if(elements[z].att_type == "T"){
				tsks.push(elements[z]);
			}
		}
		return tsks;
	};
	...
	public function getProcessMultiplier(att_id:Number):Object{
		if(elements[att_id].multiplier_id){
			var m_obj = new Object();
				m_obj.multiplier_id = elements[att_id].multiplier_id;
				m_obj.multiplier_name = elements[att_id].multiplier_name;
			return m_obj;
		}else{
			return null;
		}
	};
	public function getProcessEntity(att_id:Number):Object{		
		if(elements[att_id].entity_id){
			var m_obj = new Object();
				m_obj.entity_id = elements[att_id].entity_id;
				m_obj.entity_name = elements[att_id].entity_name;
			return m_obj;
		}else{
			return null;
		}
