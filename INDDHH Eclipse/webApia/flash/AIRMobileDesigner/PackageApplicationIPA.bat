...
set PATH=%PATH%;c:\Development\flex4.6\bin
...
set AIR_FILE=air/FacilisBPMN.ipa
...
::call c:\Development\AdobeAIRSDK\bin\adt -package -target ipa-test -provisioning-profile %IOS_PPROFILE% %SIGNING_OPTIONS% %AIR_FILE% %APP_XML% %FILE_OR_DIR%
call c:\Development\AdobeAIRSDK\bin\adt -package -target ipa-test-interpreter -provisioning-profile %IOS_PPROFILE% %SIGNING_OPTIONS% %AIR_FILE% %APP_XML% %FILE_OR_DIR%
