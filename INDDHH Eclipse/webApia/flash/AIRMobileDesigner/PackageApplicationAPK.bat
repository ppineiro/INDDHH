...
set PATH=%PATH%;C:\Development\flex\bin

:: Signature (see 'CreateCertificate.bat')
:: set CERTIFICATE=SelfSigned.pfx
set CERTIFICATE=ApiaKeystore
:: set SIGNING_OPTIONS=-storetype pkcs12 -keystore %CERTIFICATE%
set SIGNING_OPTIONS=-storetype JKS -keystore %CERTIFICATE%
if not exist %CERTIFICATE% goto certificate

:: Output
if not exist air md air
set AIR_FILE=air/FacilisBPMN.apk

:: Input
set APP_XML=mobile_application.xml
set FILE_OR_DIR=-C bin .

echo Signing AIR setup using certificate %CERTIFICATE%.
call C:\Development\AdobeAIRSDK\bin\adt -package -target apk %SIGNING_OPTIONS% %AIR_FILE% %APP_XML% %FILE_OR_DIR%
