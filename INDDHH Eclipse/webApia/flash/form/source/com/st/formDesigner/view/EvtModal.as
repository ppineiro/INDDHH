...
class com.st.formDesigner.view.EvtModal extends MovieClip{
	...
	function EvtModal(Void){
		...
		EVENTS_XML = _global.XML_FIELD_EVENTS;
		...
		p_type = thisModal._parent.fieldType;
		...
		this.add_btn.onPress = function(){
			...
			if(event_cmb.selectedItem.data==undefined || class_cmb.selectedItem.data==undefined || event_cmb.selectedItem.data=="" || class_cmb.selectedItem.data==""){
				return;
			}
	...
	function showEvtBinding(rowIndex:Number){
		...
		var configOBj = new Object();
			...
			configOBj.title =_global.labelVars.evtBindingsTit;
