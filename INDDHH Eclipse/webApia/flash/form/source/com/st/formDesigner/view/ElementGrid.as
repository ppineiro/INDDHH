...
class com.st.formDesigner.view.ElementGrid extends MovieClip{
	...
	function ElementGrid(Void){
		...
		for(var t=0;t<child_arr.length;t++){
			...
			colArray[t].onPress=function(){
				...
				if ((getTimer() - tmp.m_lastClick) < tmp.m_doubleClickSpeed) {
					//DBL CLICK
					tmp.dispatchEvent({type:" ",fieldId:this.fieldId,fieldType:this.fieldType});
				}else{
					if(!this.flag){
					...
					//CLICK
					tmp.dispatchEvent({type:"onColClicked",fieldId:this.fieldId,fieldType:this.fieldType});
					//fscommand("largalo",tmp._width);
					var startCoords:Object=new Object({x:_root._xmouse,y:_root._ymouse});
					if(tmp.colArray.length>1){
						this.onMouseMove = function(){
							if(this._parent._parent["aux"]==undefined){
								var aux=this._parent._parent.attachMovie("dropHereCur","aux",100);
								aux.startDrag(true);
								var tmp2=this;
								aux.onMouseUp=function(){
									tmp2.onMouseMove=[];
									aux.removeMovieClip(aux);
									if(tmp2._parent.hitTest(_root._xmouse,_root._ymouse)){
										for(var i=0;i<tmp.colArray.length;i++){
											if(tmp2==tmp.colArray[i]){
												var endPos=(Math.floor(tmp._xmouse/(tmp._width/tmp.colArray.length)));
												tmp.dispatchEvent({type:"onColMove",fieldId:tmp.colArray[i].fieldId,fieldType:tmp.colArray[i].fieldType,iniPos:i,endPos:endPos});
											}
										}
									}
								}
								//tmp.dispatchEvent({type:"onColMove",fieldId:this.fieldId,fieldType:this.fieldType});
								updateAfterEvent();
							}
						}
					}
			...
			colArray[t].onRelease=function(){
				...
				this.flag=false;
				if(this._parent._parent.aux!=undefined){
					this._parent._parent.removeMovieClip(this._parent._parent.aux);
				}
			}
	...
	function addColumn(newObjType:Number,id:Number){
		...
		for(var e=0;e<nCol;e++){
			if(colArray[e]){
				...
				for(var i in colArray[e]){
					//colArray[e][i]._width = cellWidth;
				}
				xPos = xPos + cellWidth;
			}
		};
	...
	function getColPosition(colFieldId:Number){
		for(var e=0; e < colArray.length; e++){
			if(colArray[e].fieldId==colFieldId){
				return e;
			}
	...
	function changeColType(colFieldId:Number,newType:Number){
		var col:MovieClip=getColObj(colFieldId);
		col._parent.fieldType=newType;
		var pos=getColPosition(colFieldId);
		col.clear();
		addElementToCol(newType,pos);
	}
