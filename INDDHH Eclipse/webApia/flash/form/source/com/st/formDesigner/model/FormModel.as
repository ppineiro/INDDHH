...
class com.st.formDesigner.model.FormModel {
	...
	function changeColGridType(gridId:Number,gridFieldId:Number,newType:Number){
		var col;
		var gridColChildren = elements[gridId].children_Array;
		for(var d=0;d<gridColChildren.length;d++){
			if(gridColChildren[d].fieldId == gridFieldId){
				col = gridColChildren[d];
			}
		}
		col.fieldType = newType;
		col.evt_Array = [];
		col.prp_Array = [];
		broadcastMessage("onColTypeChanged",gridId,gridFieldId,newType);
	}
...
//		delete elements[fieldId];
	...
	public function getGridElementType(gridId:Number,fieldId:Number):Array{
		//GET ELEMENT COL EVENTS IN GRID
		var gridColChildren = elements[gridId].children_Array;
		for(var d=0;d<gridColChildren.length;d++){
			if(gridColChildren[d].fieldId == fieldId){
				var fieldType = gridColChildren[d].fieldType;
				return fieldType;
			}
		}
	...
	public function removeBinds(fieldId:Number){
		var attId=elements[fieldId].attId;
		for(var i in elements){
			var bndArray:Array=elements[i].bnd_Array;
			for(var u=0;u<bndArray.length;u++){
				var element=bndArray[u];
				var mapping:Array=element["mapping"]
				for(var a=0;a<mapping.length;a++){
					var obj=mapping[a];
					if(obj["frm_att"]==attId){
						mapping.splice(a,1);
					}
				}
				if(mapping.length==0){
			...
			setEntityBinds(elements[i].fieldId,bndArray);
		}
	}
	///////////////////////////////////////////////////
	...
	public function getDefaultElementRowSpan(fieldType:Number):Number{
		...
		if(fieldType==10 || fieldType==6 || fieldType==14){	
			defaultRowSpan = 3;				//3 rows [elements 10,6,14]
		}else if(fieldType==13){
			defaultRowSpan = 4;				//4 rows [element 13]
		}else if(fieldType==16){
			defaultRowSpan = 5;				//5 rows [element 16]
		}else{										
	...
	public function getDefaultElementColSpan(fieldType:Number):Number{
		...
		if(fieldType==5 || fieldType==7 || fieldType==15){	
			defaultColSpan = 1;				//1 cols [elements 5,7]
		}else if(fieldType==8 || fieldType==14 || fieldType==16){ 				
			defaultColSpan = 4;				//4 cols [element 8,14,16]
		}else{										
	...
	public function getAttribute(fieldId:Number){
		return {attId:elements[fieldId].attId, attName:elements[fieldId].attName, fieldLabel:elements[fieldId].fieldLabel};
	};
	...
	public function setColAttribute(gridId:Number,fieldId:Number,attId:Number,attLabel:String,attName:String){
		...
		for(var d=0;d<gridColChildren.length;d++){
			if(gridColChildren[d].fieldId == fieldId){
				...
				gridColChildren[d].fieldLabel = attLabel;
	...
	public function setFormProperty(prop_id:Number,prop_type:String,prop_value:String):Void{
		...
		if(formProperties==null || formProperties==undefined){
			formProperties=new Array();
		}
