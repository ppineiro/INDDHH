...
class com.st.formDesigner.controller.FormController{
	...
	public function loadModelString(xmlModel:XML){
		reset();
		parseFormDefinition(xmlModel);
	}
	...
	public function getFormDefinition():Void{
		...
		if(_global.stringModel==null || _global.stringModel==undefined){
			...
			var strModel=_global.stringModel;
			strModel=strModel.split("\"&amp;lt;").join("\"&lt;");
			strModel=strModel.split("&amp;gt;\"").join("&gt;\"");
			
			var xml=strModel;
			var def_XML:XML=new XML();
			def_XML.parseXML(xml);
			parseFormDefinition(def_XML);
		}
	};
	...
	private function parseFormDefinition(oXML:XML):Void{
		...
		for(var e = 0; e < x.firstChild.childNodes.length;e++) {
			var currentNode:XMLNode = x.firstChild.childNodes[e];
			with (currentNode){
				//-------------------------------------------
				...
				if(nodeName == "EVENTS"){
					for(var j in childNodes ){
						...
						var p_events = new Object();
							...
							p_events.order= parseInt(childNodes[j].attributes.order);
						...
						for(var h in childNodes[j].childNodes){
							...
							var p_bind = new Object();
								...
								p_bind.att_label = childNodes[j].childNodes[h].attributes.att_label;
					...
					frm_events_array.sortOn("order",16);
				}
				...
				for(var j in thisNode.childNodes){
					...
					for(var h in thisNode.childNodes[j].childNodes){
						...
						var p_bind = new Object();
							...
							p_bind.att_label = thisNode.childNodes[j].childNodes[h].attributes.att_label;
...
//				if(fieldType != 5 && fieldType != 7 && fieldType != 8){
								...
								for(var p=0; p<children_Array[c].prp_Array.length;p++){
									...
									if(propertyHasChanged(c_pId,c_pValue)==true){
										var PROPERTY = myXML.createElement("PROPERTY");
											...
											if((c_pValue=="true" ||c_pValue==true) && c_pValue!=1 ){
	...
	function createPropertiesNode(prop_Array:Array):XMLNode{
		...
		for(var e=0;e < prop_Array.length;e++){
			if(prop_Array[e].type && prop_Array[e].prpId){
				...
				}else if(prop_Array[e].value==false || prop_Array[e].type!="checkbox"){
					FORM_PROPERTY.attributes.value =prop_Array[e].value;
					myXML.appendChild(FORM_PROPERTY);
				}
			}
		}
	...
	function propertyHasChanged(propID:Number,propValue:String){
		...
		if(((pElementType=="input" || pElementType=="modal" || pElementType=="color" || pElementType=="align") && (propValue!="")) 
	...
	function changeElementType(fieldId:Number,newType:Number){
		var name = __model.getElementProperty(fieldId,19);
		var label = __model.getElementLabel(fieldId);
		var x=__model.getElementGridX(fieldId);
		var y=__model.getElementGridY(fieldId);
		var att=__model.getAttribute(fieldId);
		__model.removeElement(fieldId);
		if(newType==1){
			fieldId=__model.addInputElement(null,label,newType,null,null,x,y,null,new Array(),new Array());
		}else{
			fieldId=__model.addElement(null,label,newType,null,null,x,y,new Array(),new Array());
		}
		__model.setElementProperty(fieldId,19,"S",name);
		__model.setAttribute(fieldId,att.attId,att.fieldLabel,att.attName);
		att=__model.getAttribute(fieldId);
	}
	
	function changeGridElementType(gridFieldId:Number,fieldId:Number,newType:Number){
		var label = __model.getElementLabel(fieldId);
		var x=__model.getElementGridX(fieldId);
		var y=__model.getElementGridY(fieldId);
		__model.changeColGridType(gridFieldId,fieldId,newType);
		//__model.setGridColElementProperty(gridFieldId,fieldId,19,"S",name);
