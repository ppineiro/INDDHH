...
if("definition".equals(acc)) {
	...
	values = dBean.getFormsList(request, response);
}

%><?xml version="1.0" ?><forms><data><level width="0" name="id" label="id" type="label" hidden="true" /><level width="40%" name="name" label="<%=LabelManager.getName(labelSet, langId, "flaNom")%>" type="label" /><level width="60%" name="label" label="<%=LabelManager.getName(labelSet, langId, "lblTit")%>" type="label" /></data><%=values%></forms>