...
String values = dBean.getEntitiesList(request, response);
%><?xml version="1.0" ?><forms><data><level width="0" name="id" label="id" type="label" hidden="true" /><level width="50%" name="name" label="<%=LabelManager.getName(labelSet, langId, "flaNom")%>" type="label" /><level width="50%" name="label" label="<%=LabelManager.getName(labelSet, langId, "flaEti")%>" type="label" /></data><values><%=values%></values></forms>