...
public class Query2 extends QueryAbstractClass {
	...
	protected void executeQuery(Query qry) throws BusClassException {
		...
		try {
			...
			String query =	
					"(SELECT 'Fuera de plazo' as motivo_no_adm, " + 
					"    (select count(*) " + 
					" " + 
					...
					" " + 
					"    where att.att_name = 'INDDHH_MOTIVO_NO_ADMISIBLE_STR' " + 
					"      and bei.bus_ent_id = 1379 and bei.att_value_10 = '2' " + 
					...
					") " + 
					"union all " + 
					"  (SELECT 'No competencia' as motivo_no_adm, " + 
					"    (select count(*) " + 
					" " + 
					"    from bus_ent_inst_attribute ent_att " + 
					"      join attribute att on ent_att.att_id = att.att_id_auto and ent_att.env_id = att.env_id " + 
					...
					" " + 
					"union all " + 
					"  (SELECT 'Inadmisibilidad manifiesta' as motivo_no_adm, " + 
					"    (select count(*) " + 
					" " + 
					"    from bus_ent_inst_attribute ent_att " + 
					"      join attribute att on ent_att.att_id = att.att_id_auto and ent_att.env_id = att.env_id " + 
					...
					" " + 
					"union all " + 
					"  (SELECT 'Falta de fundamentos' as motivo_no_adm, " + 
					"    (select count(*) " + 
					" " + 
					"    from bus_ent_inst_attribute ent_att " + 
					"      join attribute att on ent_att.att_id = att.att_id_auto and ent_att.env_id = att.env_id " + 
					...
					" " + 
					"union all " + 
					"  (SELECT 'Evidente mala fe' as motivo_no_adm, " + 
					"    (select count(*) " + 
					" " + 
					"    from bus_ent_inst_attribute ent_att " + 
					"      join attribute att on ent_att.att_id = att.att_id_auto and ent_att.env_id = att.env_id " + 
					...
					" " + 
					"union all " + 
					"  (SELECT 'Total de consultas' as motivo_no_adm, count(*) as ctd " + 
					" " + 
					"  from bus_ent_instance bei " + 
					" " + 
					"  where bei.env_id = 1 " + 
					"    and bei.bus_ent_id = 1379 " + 
					"    and bei.att_value_10 = '2' "
					+ "and bei.bus_ent_inst_create_data >= timestamp '" + fechaInicioStr + "' "
			...
			while (rs.next()) {
				...
				row.add(rs.getString("motivo_no_adm"));
